...
- The `## Story` and `## Acceptance criteria` sections of a ticket — with the `## Challenge` /
...
- Gap-analysis and findings, written as tickets rather than as a parallel report
...
2. Compare intended behavior (docs + stories + order/pay lifecycle in `CLAUDE.md`) against what the
   code actually does.
3. Report findings as one ticket per finding: missing flows, half-finished features
