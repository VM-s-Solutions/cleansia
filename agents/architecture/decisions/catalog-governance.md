...
2. **Silence is defined.** "The catalog was silent about X" = *no sentence covers X at any level of
   ...
   >
   > 🟡 **Round 2 is DRAFTED — `proposed`, not accepted, and no ticket may cite it.**
   > `backlog/adr/drafts/NNNN-what-makes-a-catalog-sentence-govern-round-2.md` (a sixth instance; did not
   > write ADR-0033, did not challenge it, did not adjudicate it, did not write round 1 or its challenge
   > or its verdict). What it proposes, in one line: **`governs` is REACH and carries no verdict term** —
   > *`S` governs `E` iff at least one member of `E`'s **exhibit** (the ticket's diff + the entry's own
   > `file:line` citations, a finite written list) falls under `S`'s **condition**, read at the narrowest
   > scope `S`'s own words support*; **quantifier ∃ over that finite list**; **widening `S` past its own
   > words — dropping a clause, swapping in a broader term, or reading a heading as extending it — is
   > unavailable**. The conflict half stays exactly as ADR-0033 accepted it, which is the diagnosis it
   > turns on: round 1 defined `governs` with a *conflict* notion and so duplicated the decidable second
   > conjunct into the undecidable first one. **Challenger and lead rounds pending; `deliberation.md` §6.**
   >
...
| …with **"governs" = REACH: one member of the entry's finite exhibit falls under `S`'s condition, read at `S`'s narrowest supported scope** ← **PROPOSED 2026-08-05 (round 2), NOT accepted** | one grep against the **pre-edit** catalog **+ the exhibit written as a file list**; firing it costs a quoted condition + an exhibit member + a named disjunct | `drafts/…-round-2.md`. Closes both degrees of freedom round 1 left open: **quantifier = ∃ over a finite written list** (the ticket's diff + the entry's own citations), so the negative is a bounded check; **scope = `S`'s own words, and widening is unavailable** — a reading that drops a clause of `S` or swaps a broader term for one `S` names loses, and a heading may narrow a sentence but never widen it. **`Governs` carries no verdict term at all**, which is the diagnosis: round 1 defined it with a *conflict* notion and duplicated the decidable second conjunct into the undecidable first one. **Coverage lemma:** a withdrawal either converted its violators (they are in the exhibit) or did not (**test 1** fires) — it cannot hide from both. Scored on **13 rows against parent commits**: 13 determinate, 9 agree, 3 diverge — **all three pre-existing** (ADR-0033's own two, plus T-0527/G1 where agreeing with history would be the error), **0 new divergences, 0 accepted-ADR rows flipped**. **Open costs, stated in the draft:** the exhibit is only as honest as the diff's characterization; a sentence with no limiting clauses reaches broadly and no row tests that; and **G3 is resolved permissively by construction** |
| …with **"governs" defined so that R-1…R-7 are met** ← **the open question** | unknown | The bar is in the rejected round-1 draft's §Verdict: declare the quantifier, define scope without the word "subject", **discriminate T-0449 (refinement — must not route) from T-0527 (withdrawal — must route)**, give the author an answer that can defeat the trigger, reproduce ADR-0033's rows 2 and 3, and **rebuild the corpus against the pre-edit catalog**. **One candidate is now on the table (row above) and is unratified** |
