...
> Companion to the **immutable** `agents/backlog/adr/0013-ios-app-architecture-and-port-strategy.md`
> (ADR-0013). The ADR is the frozen decision; this file is the *evolving* design notes, trade-off space,
> and current shape. Update this as the iOS port progresses; supersede the ADR for a real contract change.
> Cross-links: ADR-0011 (the born-canonical `ApiResult<T>` Swift contract this consumes), the Mobile API
> contract audit (security, 2026-06-22), the Android parity map (analyst, 2026-06-22), `patterns-mobile.md`
> (the iOS section), and `docs/architecture/*` once iOS lands.

## Current shape (as of ADR-0013, accepted 2026-06-23)
...
|---|---|---|
| Module layout | 1 workspace, `CleansiaCore` **SPM package** + 2 **app targets** | `:core` + `:partner-app` + `:customer-app` |
| State | `@Observable` VMs; sealed `UiState`(Loading/Error/Loaded) + `ActionState`(Idle/Submitting/Error) | `StateFlow<UiState>` (E1) + `ActionState` (E2) |
...
| Maps | **MapKit by default**, behind a `MapProvider` protocol; Mapbox iOS SDK = scoped fallback | Mapbox (no first-party map on Android) |
...
## Trade-off space (what was weighed — see ADR-0013 §Alternatives + the Challenge trail)
...
- **Q-IOS-01** (deployment target) — default **iOS 17** (enables `@Observable` + SwiftUI `Map`). Post-prod,
  non-blocking; only narrows device reach.
