...
|---|---|---|
| customer shell: `HorizontalPager` + floating-pill `CustomBottomBar` + center `BookFab` (`MainShell.kt:363-474`) | **CURRENT (ADR-0022 owner supersede, 2026-07-08): stock `TabView` + `.tabItem`** (same 4 tabs, SF symbols + titles) hosted as the root of the **ONE shell-level `NavigationStack`** over the type-erased `ShellRoute` `NavigationPath` (the iOS-16 crash fix — **unchanged** by the supersede); the Book FAB survives as a **solid-primary floating disc** (`ZStack` sibling, `BookFabMetrics` off the safe-area bottom) shown only while `path.isEmpty`. The pill/`.page`-pager composite is RETIRED (corrupted rendering on real iOS 26 hardware); tab-swipe given up, owner-accepted | same 4 tabs/order, ONE back stack, bar+FAB vanish on child push by construction (stack root = the `TabView`) |
...
> **Superseded in part (2026-07-08, ADR-0022 owner-directed supersede; D2-remnant closed 2026-07-19,
> T-0429):** the `.page` pager + `CustomerBottomBar` pill/FAB composite half of this record is
> RETIRED — the customer shell is the stock `TabView`/`.tabItem` bar + a solid-primary Book-FAB disc
> shown only while `path.isEmpty`; the partner shell is FINAL on the stock `TabView` + per-tab
> `NavigationStack`s (the D4 interim became the end state; T-0376 cancelled). What SURVIVES of this
> record: the deleted outer root stacks (D1), the customer's ONE shell `NavigationStack` +
> type-erased `ShellRoute` `NavigationPath` (D2 topology), the Gate-DP hardening (R4), and the
> ratified 74pt/34pt BookFab metrics erratum (T-0379). Current shape = the D3 table row above; the
> body below is the historical record.
...
| ADR (iOS shell = ONE shell-level `NavigationStack` + `.page` tab pager + custom pill-bar/FAB composite; outer root stacks DELETED both apps; partner this phase = minimal crash fix, pill/pager parity = PM-filed follow-up; supersedes §7.15 D6's pill→`TabView` swap + corrects the ADR-0018 D3 bottom-bar row; Gate-DP gains AR-DP-1a + AR-DP-4) | — | **accepted** (ADR-0022, 2026-07-02 — the phase/ios-fix1 owner-device iOS-16 defect ruling; reviewer #35). **Bar/pager half superseded in-ADR 2026-07-08** (owner-directed: stock `TabView` + FAB disc; pill/pager retired); D2 topology survives on customer; partner FINAL on per-tab stacks (T-0429, 2026-07-19); 74pt erratum ratified (T-0379) |
| Owner Q-IOS-01 (deployment target) | — | **ANSWERED — iOS 16** (old-device reach) |
| Owner **mobile-spec regen** (was the one hard blocker) | pre-Phase-2 | **DONE — verified 2026-08-14.** Specs carry `Device/Mine`, the device revoke and `GetPeriodPays`; both clients generated and wired into `project.yml`. A re-dump stays owner-run, but nothing is gated on it |
