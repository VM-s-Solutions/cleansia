...
|---|---|---|
| **F1** | **ADR-0032's "Number note" (`:23-25`) is false.** It says 0031 *"exists only in T-0439's worktree and has not reached `master`"*; `docs/decisions/adr-0031.md` is on disk. ADR-0032 is `accepted`, so `adr/README.md:16-26` requires a **signed erratum**, not an in-body edit. | Editing an accepted ADR from inside another ticket is the process violation T-0379 ratified the erratum lane to prevent. |
