...
**Status:** 📱 **PHASE/IOS-FIX1 COMPLETE — PR DRAFTED; remaining acceptance = the OWNER DEVICE PASS** (2026-07-03, on `phase/ios-fix1`, 11 commits pushed) — the on-device iOS-16 shakeout: the owner tested BOTH iOS apps on a real iOS 16 iPhone (the first human, first real-device pass) and reported **16 issues**; the 4-cluster diagnosis (booking-ux · brand-assets · navigation-shell · data-layer) grounded all 16 with file:line evidence → **the 6 slices T-0368…T-0373 are ALL DONE** (T-0367 absorbed into T-0370; the process leg **T-0374 → in_progress**: executed on every slice + the Gate-DP §G hardening landed, its durable `quality-gates.md` codification stays open). **PHASE GATE PASS on the final tree:** swiftformat 0.60.1 0/528 + swiftlint --strict clean tree-wide · Core 272/272 on BOTH iPhone 17 (iOS 26.3) AND the iOS 16.4 floor sim · Partner + Customer suites green (Customer 406) · dotnet 1714/1714 (incl. the new spec-enum guard) · Android compile green (Kotlin client regen from the new spec) · the 16.4 boot-install-launch smoke of BOTH apps with 0 NavigationAuthority/comparisonTypeMismatch hits. **ADR-0022 accepted** (`987f85f0`). **Follow-ups filed:** T-0375 (Android hardcoded greeting) · T-0376 (partner pill/pager — the ADR-0022 D4 mandate) · T-0377 (UILaunchScreen real-hardware re-probe) · T-0378 (partner branded splash) · T-0379 (architect ratification: the ADR-0022 in-body annotation + the SnackbarInset canonical-row replacement). **OWNER:** (1) the DEVICE PASS checklist is in the PR body — signed-in navigation (Plus from Home + Profile, order detail, dispute create→detail), tab swipe + bar-hidden-on-push, slide-to-pay feel, snackbar visibility over the bar, booking draft persistence, mascots/icons/splash, the membership screen with a SUBSCRIBED user, perceived lag; (2) the T-0370 spec re-dump + client regens ran IN-BRANCH — verify the spec diff at the PR or re-run your pipeline; (3) the owner commits these backlog edits + opens the PR (the PM commits nothing). See the STATUS-LOG 2026-07-03 (PHASE CLOSE) below + the INDEX banner. **T-0314 ADDENDUM: feature-complete ≠ device-verified — this phase paid the device-verification debt.** · (prior) 🧱 **HARDENING-1 DONE — PR drafted** on `phase/hardening-1` (2026-06-30; **5 commits, pushed; off master `3e7ce52` = the merged Phase-8 PR #100**) — the first post-iOS-port cleanup/hardening phase: it lands the deferred follow-ups the port surfaced (NO new features) across backend / iOS / Android + a CI/tooling pin. **8 tickets → `done` in 5 commits:** `239323b` (CI: pin SwiftFormat 0.60.1 — NO ticket, a CI hardening) · `d834e92` (**T-0349** — hoist the address-picker VM into `CleansiaCore`; Core 218/Partner 366/Customer 362; no new ADR) · `64f6525` (**backend trio** — **T-0346** GoogleAuth `email_verified` gate / **T-0348** PaymentIntent refund + the two CANCEL-path refundable-surface gate, NO schema change / **T-0350** NotificationPreferences rate-limit ×2 hosts; security review CLEAN; `Cleansia.Tests` 1685) · `e4e00b0` (**T-0341** — deterministic order status-history via `OrderStatusTrack.Sequence` + canonical `Order.CurrentStatus`; **pre-prod Initial migration REGENERATED in-branch, owner-authorized**; de-flake 20/20; IntegrationTests 97/97 + HostTests 60/60) · `1d99333` (**android parity-hygiene** — **T-0351** wire the dead customer SecurityScreen change-password / **T-0333** partner Register/Forgot i18n / **T-0337** the 7 partner profile VMs → sealed state + i18n + a `BankSectionViewModelTest`; verified by a LOCAL gradle build, JDK21/SDK35). **OWNER / manual note:** the T-0341 Initial-regen is **ALREADY DONE in-branch** — no pending migration; no NSwag regen (no DTO surface changed). **2 NEW non-blocking follow-ups filed `proposed`:** **T-0352** (cross-app password min-length policy drift — customer ≥12 vs partner ≥8) · **T-0353** (partner profile section Error-state has no retry affordance). The owner commits the backlog edits + opens the PR (the PM does not commit). · **(prior) 🎉 iOS PORT FEATURE-COMPLETE — PHASE 8 DONE** on `phase/ios-phase8` (2026-06-30; **7 commits, pushed; Phase-8 PR drafted**; off master `8d90104`) — **T-0314 (the customer TAIL, the LAST customer feature ticket, the broadest, `L → split` into 6 slices A–F) DONE → this COMPLETES the customer app AND the entire iOS port** (partner: Phases 0–5; customer: Phases 6–8). The feature port of both Kotlin/Compose apps to Swift/SwiftUI is **done**; the remaining iOS scope is the **App Store compliance/release pass + the owner provisioning + the deferred follow-ups** (no more feature tickets). **T-0314 (customer tail, DONE — 6 slices):** **A** (`6a587cf`) Home + Orders/OrderDetail (paged list, the 7-state OrderStatus incl. OnTheWay=3, lifecycle timeline/LiveProgressHero, cancel/review/receipt-via-QuickLook, the 5-min poller) + the T-0313 success→OrderDetail fold / **B** (`035c211`) Rewards/Loyalty/Referrals (tier/progress/perks + paged activity + referral copy/share) / **C** (`cae96dc`, Gate-SEC) Membership/Plus (the two-phase Stripe **SetupIntent** via the `StripePaymentController` seam extended with `PaymentIntentKind` — still the sole Stripe importer) + Recurring + the deferred ConfirmRecurring on OrderDetail — reviewer **APPROVE** + security **PASS** / **D** (`c10d819`, Gate-SEC) Disputes (list/create/detail + the generated **multipart** evidence upload, EXIF-strip, fail-closed file validation) + the customer's first camera/photo permission strings + `PrivacyInfo` — reviewer **APPROVE** + security **PASS** / **E** (`58d9d35`) Addresses (AddressManager 3-pane + saved-address CRUD on the Core map seam) / **F** (`4a15fbf`, Gate-SEC) Profile/Settings hub + **GDPR DeleteAccount** (`signOutLocal`-not-`logout`; blocked→stay-signed-in; the SIWA-revoke owner-deferred + the "remove in Settings" note) + Devices (T-0310 D6-8) + NotificationPreferences (optimistic+debounced) + the REAL change-password flow + Language/Appearance/Help — reviewer **APPROVE** (after the Profile-Subscribe-CTA fix) + security **PASS** (GDPR verdict **PASS**). §7.17 Understand-pass: the verified workflow ran (architect cluster plan + security design gate **PASS-WITH-REQUIREMENTS**, GDPR-delete **PASS**); §7.17 is **CONFIRMED-AS-SHIPPED**. Security posture (all PASS vs code, `security/ios-customer-auth.md`): the membership money path, the dispute multipart upload, the GDPR delete. **Tests: CleansiaCore 216 + CleansiaCustomer 362** (+ the CleansiaPartner 366 non-regression) on iPhone 17; swiftformat + swiftlint --strict clean. **OWNER / manual steps (for go-live/App Store, not code blockers — the PM never runs these):** the **Stripe publishable key** (live card + membership); **T-0344** (Apple SIWA capability); **T-0345** (Google client ids/IMP-1); the **customer-mobile spec + client regen**; the **camera/photo + customer-brand plist WORDING** sign-off; the **App Store compliance pass** (`ios-app-review-checklist.md`, ADR-0016 — 5.1.1 in-app delete ✓ shipped, SIWA ✓, Stripe-not-IAP framing, ASC privacy answers + demo account). **Follow-ups (proposed, NOT dispatched):** **T-0348** (mobile PaymentIntent refund) · **T-0349** (address-picker→Core harvest) · **T-0350** (NotificationPreferences rate-limit) · **T-0351** (NEW — the Android customer `SecurityScreen` dead-stub change-password catch-up); + notes (the customer brand asset + Google-"G" fidelity; the country-resolver → T-0334; the membership `idempotencyToken` regen — iOS carries the field; the dead `CreateOrderResponse.stripeSessionId` DTO cleanup at the next regen). · **(prior) 🍎 HARD AREA #1 CLEARED — PHASE 7 DONE** on `phase/ios-phase7` (2026-06-29; merged) — **the customer booking wizard + Stripe PaymentSheet** (ADR-0013's named HARD AREA #1, the single hardest feature in the port AND the customer **primary** flow), plus the **T-0347 backend money-safety fix**. **T-0313 (customer booking wizard + Stripe, 5 slices A–E) + T-0347 (one charge surface per card order) + T-0332 (the dual-use Bearer carve-out) DONE.** **T-0313:** A the 3-step modal anchored sheet (`.sheet`/`.presentationDetents`) + the shared `BookingViewModel` + the Book FAB action / B Step 1 Services + the server-authoritative pricing engine (debounced live `Quote` + the `BookingPricing` display port) + the first customer generated-client auth spine (`CustomerCoreSpineRequestBuilderFactory`, the ADR-0019 twin) / C Step 2 When&Where (the Core-map-seam address picker + lead-time slots) + the extras/promo/referral FSMs / D Step 3 Confirm + the CASH submit + the **T-0332 dual-use Bearer carve-out** (Bearer on the 3 booking endpoints iff a token exists; guest tokenless; pure-anon never; `CreatePaymentIntent` always authed) + the status-accurate cash success screen (Pending+New, NOT "Confirmed") / E the CARD branch + Stripe **PaymentSheet** (the `PaymentSheetPresenting`/`StripePaymentController` seam = the sole Stripe importer; publishable-key fail-closed). Reviewer **APPROVE** + **SECURITY PASS** on D+E (price-tampering NO; the dual-use Bearer scoping verified; `.completed` is UX-only / the webhook is the sole paid authority; no secret leak; fail-closed; single charge surface). **T-0347** (`afaa920`): a per-host `IOrderChannelProvider` so `OrderPaymentDispatcher` suppresses the Stripe Checkout Session for the mobile PaymentSheet channel (the mobile order's single charge surface is the PaymentIntent) — closes a **pre-existing double-capture defect** (the live Android card flow had it too); reviewer APPROVE + security PASS. **LIVE card payment is OWNER-gated on the Stripe publishable key (pk_); the code ships fail-closed.** New seams: the customer ADR-0019 spine; the `PaymentSheetPresenting`/`StripePaymentController` Stripe seam; the `BookingPricing` display port. Follow-ups kept as proposed: **T-0348** (mobile PaymentIntent refund path), **T-0349** (address-picker→Core harvest). Tests: **CleansiaCore 213 + CleansiaCustomer 156** (+ partner 366 non-regression) on iPhone 17; swiftformat + swiftlint --strict clean; the customer generated client is gitignored (CI regenerates); build-time verifications in `security/ios-customer-auth.md`. · **(prior) PHASE 6 DONE** on `phase/ios-phase6` (merged) — the FIRST customer feature: **T-0343 (backend AppleAuth, Sign in with Apple) + T-0312 (iOS customer shell + auth, 3 slices) DONE** — backend AppleAuth mirrors GoogleAuth 1:1 (RS256-pinned JWKS, fail-closed; reviewer APPROVE, security PASS — account-takeover NO); the customer shell/root/auth-core/social slices all reviewer-APPROVE with Gate-SEC PASS on the security-touching B+C slices (no parallel write path; the iOS↔backend nonce-encoding ALIGNED). New Core seams: `SocialSignInProviding` (Apple nonce flow + GoogleSignIn-iOS) + the construction-time `RegisterEndpoint` spine parameter (customer→`/api/Auth/Register`, partner→`RegisterEmployee`, byte-equivalent). LIVE social sign-in is OWNER-gated (T-0344 Apple + T-0345 Google + the EF migration for `User.AppleId` + the customer-mobile spec+client regen); the code ships fail-closed. Tests: **CleansiaCore 202 + CleansiaCustomer 42** (+ partner 366 non-regression) on iPhone 17; swiftformat + swiftlint --strict clean. CI: ios-ci now `build test`s CleansiaCustomer; backend-ci runs the AppleAuth tests. · 🎉 **PARTNER APP FEATURE-COMPLETE** (2026-06-28). PHASE 0 FOUNDATION DONE + MAC-VERIFIED + MERGED (2026-06-26) · **PHASE 1 (T-0303) DONE** — proving vertical green on `phase/ios-phase1` · **PHASE 2 — T-0304 + T-0305 DONE** on `phase/ios-phase2` · **PHASE 3 — T-0306 + T-0310 DONE** on `phase/ios-phase3` (merged #95) · **PHASE 4 — T-0307 (partner order work-loop, L→split, HARD AREA #3) + T-0308 (partner photo upload) DONE** on `phase/ios-phase4` (merged #96). **PHASE 5 — T-0309 (partner earnings + invoices + PeriodPay) + T-0311 (partner APNs push registration) DONE** on `phase/ios-phase5` (2026-06-28; merged #97) — **this phase makes the iOS PARTNER APP feature-complete** (every partner surface is now ported: auth → shell → dashboard → orders+photos → profile/devices/prefs → earnings/invoices → push). **T-0309:** 2 slices — A Earnings summary (reuses `getStats`) + PeriodPay (E1/E2 own-id) over the generated `PartnerEmployeePayrollAPI` + a Core `EarningsFormat`; the `.invoices` tab is now the Earnings surface (in-tab `NavigationStack`/`EarningsRoute`) / B invoices list+detail + the new Core `QuickLookPreview` seam (PDF) + the InvoicesStaleness silent-stale resume; `RefreshPhase` lifted to `CleansiaCore/State` (shared by Orders+Invoices). Reviewer **APPROVE**; **SECURITY PASS** (E1–E4 + TC-IOS-EARNINGS-OWNERSHIP; backend EmployeePayroll already JWT-scoped — **no T-0339-class gap**). **T-0311:** 2 slices — A the `PushRegistrar` Core seam + `PushTokenRegistrar` (`SessionScopedCache`) + the `Device/Register` client / B the `PushSessionObserver` (register on session×token) + the AppDelegate (`@UIApplicationDelegateAdaptor`) + the `Auth.setPreLogout` hook (logout unregisters BEFORE the token wipe) + the `aps-environment` entitlement + tap→OrderDetail. Reviewer **APPROVE**; **SECURITY PASS** (rules 1–4 + TC-IOS-PUSH-LOGOUT-CLEARS). End-to-end DELIVERY is OWNER-gated → **T-0342**. **PLUS a CI hardening:** ios-ci now runs the CleansiaPartner test suite (`build test`), not just `build` — the partner VM + security tests (366) now actually gate in CI for the first time. Tests: **CleansiaCore 194 + CleansiaPartner 366** (iPhone 17); swiftformat + swiftlint --strict clean. **T-0307:** 5 slices — A Core `SnapSheet` (ADR-0021 non-modal 3-snap) + `fullBleedMap` single-pin / B 3-pane OrdersList (sealed per-pane state + ported staleness cache) / C OrderDetail shell / D the OnTheWay lifecycle + the shared pure `OrderPrimaryAction` machine (**SECURITY PASS O1/O2/O4**) / E checklist/notes/issues/timeline; reviewer **APPROVE** all 5; security **PASS**. **T-0308:** 2 slices — A Core `CameraOrLibraryPicker` (first `UIViewControllerRepresentable`) + `ImageCompressor` (1920/0.7, EXPLICIT EXIF strip) / B `PhotosSection` (before/after rails, capture→upload) + Complete-unblock + bootstrapped `PrivacyInfo.xcprivacy` + camera/photo usage strings ×5; reviewer **APPROVE**; security **PASS** (P1–P5). Tests: **CleansiaCore 163 + CleansiaPartner 320** (iPhone 17); swiftformat + swiftlint --strict clean. **REQUIRED backend follow-up T-0339** (GetPagedOrders employeeId over-read — SECURITY, high, gates the GetPaged contract for go-live; iOS proceeded in parallel). · Phase 2+ tail (T-0309/0311/0312/0313/0314 + compliance T-0324…T-0329) proposed · follow-ups: **T-0334** (iOS ServiceAreaRow) / **T-0335** (iOS current-location FAB, gated on owner T-0325) / **T-0336** (iOS notifications-feed spike) / **T-0337** (Android partner profile sealed-state + i18n) / **T-0338** (Core catalog i18n ×5 + swappable bundle) / **T-0339** (backend GetPaged read-scoping — SECURITY) / **T-0340** (order-detail parity nits: checklist stable-id + Android status-label casing + placeholder-preview sweep); android i18n **T-0333** (Register/Forgot) prior · **owner items:** the camera/photo plist WORDING sign-off ×5 (T-0308 shipped en+cs/sk/uk/ru); T-0325 location string (owner, unused by Phase 4) · standing latent backend S8 (RefreshToken tenant read asymmetry the device-revoke kill rides on) tracked by **T-0236** (`done`) + `security/auth-sessions.md` + `security/ios-devices.md` — re-verify before any non-null-TenantId onboarding
**Created:** 2026-06-23
**Updated:** 2026-07-03
...
> **STATUS-LOG 2026-07-03 — 📱 PHASE/IOS-FIX1 CLOSE: ALL 6 SLICES DONE · PHASE GATE PASS · PR DRAFTED; the
> owner device pass is the remaining acceptance.** The 16 owner issues → 4 clusters → 6 slices + the process
> leg, all landed on `phase/ios-fix1` (11 commits, pushed; the PM commits nothing — backlog edits left
> unstaged for the owner):
> - **T-0368 done** (`d34eaf5e`; the architect ruling ACCEPTED as **ADR-0022**, docs `987f85f0`) — the
>   customer shell restructure: ONE shell `NavigationStack` (18-case `ShellRoute` on an erased
>   `NavigationPath`), the `.page` pager, the ported `CustomerBottomBar` pill/FAB composite via
>   `safeAreaInset`; the iOS-16 crash class eliminated BY CONSTRUCTION; the Slice-A review findings
>   (snackbar clearance, the 74pt FAB transcription) routed to the Slice-D carrier and folded in `fef5745c`.
> - **T-0369 done** (`4e38a93f`; reviewer **APPROVE clean**) — the partner mirror: outer stack deleted +
>   FIVE typed-path sites → `NavigationPath` (incl. the RegistrationLockView site the ruling's line refs
>   didn't name; gate #24 byte-unchanged). Ride-along: the Slice-B lint run exposed the ios-ci MASTER-PUSH
>   bypass → closed by `197352a9` (CI hardening, no ticket).
> - **T-0370 done** (`5252bfb9` + fold `ebf2fcfd`; ABSORBS T-0367) — `MembershipStatus`
>   `[SwaggerEnumAsInt]` + the spec re-dump; `apiResponseQueue` off main + the parallel prefetch;
>   offset-less date hardening; `ApiError.fromGenerated` → the ProblemDetails parse. **The manual steps ran
>   IN-BRANCH** (disposable-postgres spec dump; Kotlin client regen, Android compile green) — the owner
>   verifies the spec diff at the PR; `MobileSpecEnumGuardTests` (dotnet 1714/1714) pins every mobile enum
>   schema to integer.
> - **T-0371 done** (`fef5745c` + D+F fold `bfb1ca7a`) — the real Core `SlideToConfirm` (partner hoisted,
>   customer wired), in-sheet snackbar hosts + the shell `bottomInset` lift, `.profileIncomplete` →
>   dismiss+edit-profile, the session-lived draft. The **D-1 adversarial-verify catch:** the session-lived
>   draft opened a DUPLICATE-ORDER path (sheet swiped away over success → the FAB reopened step 3
>   re-armed) — fixed clear-first at the success outcome, test-pinned.
> - **T-0372 done** (`62d9495b` + fold `0be26d5d`) — mascots + the Core `Mascot`/`AnimatedMascotView`, app
>   icons + launch color BOTH apps, category icon meaning+tints; the fold fixed the animator restart,
>   REVERTED the accidental 33.6k-line xcstrings churn and pinned `SWIFT_EMIT_LOC_STRINGS: NO`
>   (SHA-proven no-churn). Recorded deviations stand: AC5 COLOR-ONLY launch (`UIImageName` sim-broken →
>   re-probe **T-0377**); AC6 no-broom. Partner splash follow-up **T-0378**.
> - **T-0373 done** (`e69a0283` + fold `bfb1ca7a`) — the Home upsell carousel ported (slide model as pure
>   data, Core `BrandGradient`, inner `.page` pager + custom dots, 6s rotate); the shell wiring landed
>   IN-slice (the pre-seeded recurring path). The **F-1 floor catch:** `BrandGradientTests` red ONLY on the
>   16.4 runtime; the reviewer's `performAsCurrent` suggestion was empirically WRONG on iOS 16
>   (`UIColor(Color)` flattens dynamic providers pre-iOS-17) — fixed structurally: hex stops as the single
>   source of truth, OS-independent assertions. Android's hardcoded ", Michael" greeting → **T-0375** (not
>   ported).
> - **T-0374 → in_progress (deliberately NOT closed):** the floor leg was EXECUTED on every slice (AC3 ✓ —
>   F-1 is the leg's proof-of-value) and the architect's Gate-DP §G hardening landed (`987f85f0`: AR-DP-1a
>   + AR-DP-4 = AC2 ✓); OPEN: AC1/AC4 — the durable codification (+ WHY paragraph) into
>   `agents/process/quality-gates.md` (grep-verified absent). qa+docs; doc-only.
>
> **PHASE GATE (final tree):** swiftformat 0.60.1 **0/528** + swiftlint --strict clean TREE-WIDE ·
> **Core 272/272 on BOTH iPhone 17 (iOS 26.3) AND the iOS 16.4 floor sim** · Partner + Customer suites
> green (**Customer 406**) · **dotnet 1714/1714** · Android compile green · the 16.4 boot-install-launch
> smoke of BOTH apps with **0 NavigationAuthority/comparisonTypeMismatch hits**. Reviewer-per-developer
> held on every slice (B APPROVE clean; C/E/D+F CHANGES all folded; A workflow-verified, findings routed).
> **Follow-ups filed `proposed` (NOT dispatched):** **T-0375** (Android greeting name, medium) · **T-0376**
> (partner pill/pager parity — the ADR-0022 D4 mandate, medium) · **T-0377** (UILaunchScreen re-probe on
> real hardware, low) · **T-0378** (partner branded splash, low) · **T-0379** (architect ratification: the
> ADR-0022 in-body 74pt annotation + the patterns-mobile SnackbarInset canonical-row replacement, medium —
> dispatch before T-0376). **REMAINING ACCEPTANCE — the OWNER DEVICE PASS** (checklist in the PR body):
> signed-in navigation (Plus from Home + Profile, order detail, dispute create→detail), tab swipe +
> bar-hidden-on-push, slide-to-pay feel, snackbar visibility over the bar, booking draft persistence,
> mascots/icons/splash, the membership screen with a SUBSCRIBED user, perceived lag. The owner commits the
> backlog edits + opens the PR.

> **STATUS-LOG 2026-07-03 — 📱 PHASE/IOS-FIX1 OPENED (the on-device iOS-16 shakeout — the device-verification
> debt of the "feature-complete" port).** The owner ran BOTH iOS apps on a **real iOS 16 iPhone** — the first
> time a human used the apps on the declared ADR-0014 floor hardware — and reported **16 issues**. A 4-cluster
> diagnosis (2026-07-02) grounded ALL 16 with file:line evidence; the diagnosis workflow is **verified**
> (every finding cause-rooted; the nested-stack ⚠️/crash topology **empirically reproduced on a booted iOS
> 16.4 simulator**; the "non-optional field" suspicion **DISPROVEN** by an executed decode test; repro
> artifacts in the session scratchpad). The clusters:
> - **navigation-shell** (iOS-16-specific, SHIP-BLOCKER): nested `NavigationStack`s (an outer pathless stack
>   over four sibling typed-path in-tab stacks) → the `comparisonTypeMismatch` **CRASH** on the Plus route +
>   the bare **yellow-⚠️** placeholder pushes on Profile sub-screens; the island/pill tab bar was **never
>   ported** (the stock TabView masked by the modern OS's own styling); the Book FAB overlaps/clips the bar on
>   0pt-inset floor hardware; tabs not swipeable. The partner app carries the **identical crash topology**.
> - **booking-ux**: SlideToConfirm was never implemented (static visual + tap with a SILENT guard); every
>   in-sheet error snackbars BEHIND the modal sheet (the only host is at the window root — also
>   promo/referral/Stripe-cancel); dismissing the wizard destroys the `@StateObject` VM → the whole draft lost.
> - **data-layer**: the `MembershipStatus` **contract lie** (backend writes int, spec says string → the whole
>   `GetMyMembership` fails to decode for ANY subscribed user — Android has the same latent bug); ALL
>   generated-client responses decode on the MAIN queue + a six-serial-await prefetch (the "lag");
>   offset-less date-times kill entire responses (latent); `ApiError.fromGenerated` drops the business error
>   code at 24/25 call sites (raw ProblemDetails JSON in snackbars).
> - **brand-assets**: ZERO mascots in the customer app (SF-symbol stand-ins everywhere), **no app icon in
>   EITHER app** (`ASSETCATALOG_COMPILER_APPICON_NAME: ""`), blank launch screens, the Home upsell carousel
>   never ported, category icons with wrong meaning/no tints — Gate-DP **misses** structural to the gate's
>   `.kt`-file citation unit (raster `res/` assets + app-level chrome were owned by no screen ticket).
>
> **Filed (T-0368…T-0374; ticket files carry the full evidence + AC):** **T-0368** Slice A customer shell
> RESTRUCTURE — single shell stack + page pager + the custom pill-bar/FAB composite (`in_progress`, **architect
> ruling in flight** on the ADR-0020 shell-pattern change; fallback = the minimal outer-stack delete +
> `NavigationPath` conversion; SHIP-BLOCKER high) · **T-0369** Slice B partner crash mirror — minimal fix
> (`ready`, high) · **T-0370** Slice C data-layer fixes (`in_progress`, dev dispatched; **ABSORBS T-0367**;
> the regen-gated halves ON HOLD — see owner steps; high) · **T-0371** Slice D booking wizard UX — Core-hoisted
> SlideToConfirm + in-sheet snackbar host + `.profileIncomplete`→edit-profile + the draft-surviving VM
> (`ready`, high; serialize `CustomerShellView.swift` edits with T-0368) · **T-0372** Slice E brand/asset
> parity — mascots/Core enum/animator + app icons + launch screens BOTH apps + category icon meaning
> (`in_progress`, dev dispatched; medium-high) · **T-0373** Slice F Home upsell carousel (`proposed`, deps
> T-0368+T-0372; medium) · **T-0374** PROCESS: the **iOS 16 floor verification leg** — every iOS slice smokes
> on the iOS 16.4 simulator (runtime now installed locally) + the Gate-DP §G hardening the architect is
> folding in (the AR-DP-1 asset-counterpart sub-check + the one-time app-chrome item) (`proposed`, high).
>
> **OWNER / manual steps (held; the PM never runs these):** the **T-0370 mobile spec re-dump
> (`scripts/refresh-mobile-spec.sh`) + both-client regen (`scripts/generate-api-clients.sh` + Android
> `openApiGenerate`)** — the Android `IntEnumSerializersModule` entry + the iOS int-enum verification are ON
> HOLD until the owner confirms. **T-0314 record ADDENDUM (also in its INDEX row):** *feature-complete ≠
> device-verified* — the port's tests/gates all ran on latest-runtime simulators; every floor-only defect
> (crash/⚠️/island) was invisible there, and no gate asserted branding/app-chrome. This phase pays that debt;
> T-0374 makes the floor leg permanent. Reviewer-per-developer holds on every slice; no gate is skipped.
> The owner commits these backlog edits with the phase (the PM does not commit; edits left unstaged).
...
> **STATUS-LOG 2026-06-30 — 🧱 HARDENING-1 DONE; the post-iOS-port follow-up tail is cleared (PR drafted).**
> `phase/hardening-1` (5 commits, pushed; off master `3e7ce52` = the merged Phase-8 PR #100). This is the first
> **post-port** phase — it adds NO features; it lands the deferred `proposed`/`draft`/`ready` follow-ups the iOS
> port surfaced, across backend / iOS / Android, plus a CI/tooling pin. **8 tickets → `done`:**
> - **`239323b` — CI/tooling hardening (NO ticket):** pinned **SwiftFormat 0.60.1** (the GitHub release binary)
>   to stop the recurring local-vs-CI version drift — CI's brew-latest kept adding default rules (`docComments`,
>   `redundantEmptyView`) that failed CI on locally-clean code. Lint clean at 0.60.1. Recorded as a CI hardening;
>   no ticket needed (the §3 table / INDEX banner note it).
> - **`d834e92` — T-0349** (iOS harvest, `proposed → done`): public framework-pure
>   `CleansiaCore/Location/AddressPickerViewModel` (`searchBias=["cz","sk"]` default, load-bearing for the partner
>   non-regression); both app-local copies deleted + both apps repoint; Views stay app-local (the sanctioned
>   `pickerMap`/`fullBleedMap` MapKit binding). **Core 218 / Partner 366 / Customer 362** green; swiftformat
>   0.60.1 + swiftlint --strict clean; reviewer **APPROVE**; **no new ADR** (a home change via the ADR-0013
>   escape clause); harvest recorded in `patterns-mobile.md` + `ios-app-architecture.md`.
> - **`64f6525` — the backend trio (T-0346 + T-0348 + T-0350, all `proposed → done`).** **T-0346** gate
>   `GoogleAuth` provisioning on `email_verified` (fail-closed, parity with the AppleAuth gate; the
>   `AuthenticationType != Google` takeover guard stays; + the unverified-token-rejected test). **T-0348**
>   `RefundPaymentIntentAsync` on `IStripeClient`/`StripeClient` + `RefundService` routes by charge surface
>   (Session→web XOR `StripePaymentIntentId`→mobile) — **NO schema change** (`StripePaymentIntentId` already
>   existed); + a money-correctness fold-in: the refundable-surface gate extended to the **two CANCEL paths**
>   (`CancelOrder` + `AdminCancelOrder`) via `Order.HasRefundableChargeSurface` so a cancelled paid
>   mobile/recurring card order refunds; the architect "both-surfaces" finding **refuted/dropped** (one charge
>   surface per card order, by T-0347). **T-0350** `[EnableRateLimiting("auth")]` on `NotificationPreferences`
>   GetMine+Update on **both** hosts + a `RateLimitCoverageGuardTests` for the lazy-create GET. **Security review
>   CLEAN**; correctness/test findings folded (cancel-refund coverage + stale-test retarget + GetMine guard).
>   Build 0 errors; **`Cleansia.Tests` 1685**.
> - **`e4e00b0` — T-0341** (backend, `proposed → done`): the architect ruling shipped verbatim — new
>   `OrderStatusTrack.Sequence` (`int`) assigned at append from the aggregate history + canonical
>   `Order.CurrentStatus` (`OrderByDescending(CreatedOn).ThenByDescending(Sequence)`) routed through ALL
>   in-memory handlers + `OrderMappers` + the 2 SQL sites (`OrderSpecification` + `OrderRepository`); the
>   `.Any()`-existence checks untouched (out of scope). **manual_step done in-branch — the pre-prod Initial
>   migration was REGENERATED (owner-authorized class):** `Sequence` folded into `20260623112626_Initial`,
>   timestamp preserved; NO new incremental migration, NO NSwag impact. **De-flake gate met:**
>   `AdminOverrideOrderStatusHandlerTests` **20/20**; **IntegrationTests 97/97 + HostTests 60/60**; reviewer
>   **APPROVE**.
> - **`1d99333` — the android parity-hygiene commit (T-0351 + T-0333 + T-0337, all → `done`).** **T-0351** wire
>   the dead customer `SecurityScreen` "Update password" stub to the existing reset-code flow
>   (`AuthViewModel.requestPasswordChange`/`changePassword`; the `CleansiaNavHost.kt:432` call site now passes the
>   action; no backend change). **T-0333** i18n the partner Register/Forgot validation strings (inject
>   `@ApplicationContext Context` + `R.string.*` ×5; behavior identical except language). **T-0337** migrate the
>   **7 partner profile VMs** (Profile + the 6 section VMs) to sealed `Loading`/`Error`/`Loaded` + `ActionState`
>   + `R.string.*` ×5 + a `BankSectionViewModelTest`. **Verified by a LOCAL gradle build** (JDK21/SDK35 —
>   partner + customer compile + the new test pass) since `android-ci` runs only on PR; reviewer **APPROVE**.
>
> **OWNER / manual note:** the **T-0341 pre-prod Initial-migration regeneration is ALREADY DONE in-branch** (the
> owner-authorized regen class) — there is **no pending migration step** for this phase, and **no NSwag regen**
> (no DTO/endpoint surface changed). **2 NEW non-blocking follow-ups (the android review surfaced them), filed
> `proposed`, NOT dispatched:** **T-0352** (cross-app password min-length policy drift — customer-app ≥12 vs
> partner-app ≥8; pick one canonical policy, ideally aligned with the backend `BaseAuthValidator`; layers
> `[android]`, low) · **T-0353** (the partner profile section-form `Error` state renders an empty form with no
> retry affordance — the screens consume the sealed state via `is Loading` + `as? Loaded`, not an exhaustive
> `when`; behavior-preserved from before T-0337, a small UX enhancement; layers `[android]`, low). **The owner
> commits these backlog edits + opens the HARDENING-1 PR (the PM does not commit).** **Reconciliation note (the
> squash-merge caveat carries forward):** master is now `3e7ce52` (the squash-merged Phase-8 PR #100) — verify
...
> **STATUS-LOG 2026-06-28 — PHASE 6 DONE; the iOS CUSTOMER AUTH FOUNDATION landed on `phase/ios-phase6`
> (6 commits, pushed; Phase-6 PR drafted).** This phase opens the **customer app** — the first customer feature
> (the partner app shipped feature-complete in Phases 0–5). The branch is off master **`c898e79`** (the merged
> Phase-5 PR #97); commits in order: `8243657` (Q-IOS-04 resolution docs), `a689d03` (T-0343 backend AppleAuth),
> `7c4c7a3` (T-0312 §7.15 Understand-pass docs), `2cf0f1e` (T-0312 Slice A), `6f9c1de` (T-0312 Slice B), `2ae5982`
> (T-0312 Slice C). Both tickets passed the full workflow (architect Understand-pass → dev slices →
> reviewer/Gate-DP + security on the touching slices). **T-0343 (backend AppleAuth — Sign in with Apple) → `done`**
> (`a689d03`): a new `AppleAuth` CQRS + `IAppleTokenVerifier`/`AppleTokenVerifier` (**RS256-PINNED** JWKS via
> `JsonWebTokenHandler` + `ConfigurationManager`; `aud == Apple:BundleId` native, iss/exp/nonce, fail-closed, no
> SSRF) + `AppleConfig` + `User.AppleId`/`AuthenticationType.Apple`/`CreateWithApple` + `[AllowAnonymous] POST
> /api/Auth/AppleAuth` on the Customer Mobile host + `InvalidAppleUserToken` ×5 i18n. **Mirrors GoogleAuth 1:1.**
> Reviewer **APPROVE**; **SECURITY PASS** (account-takeover **NO** — the RS256-pin + the handler takeover-guard
> verified vs code). Ships **fail-closed**; LIVE Apple sign-in is gated on **T-0344** + the owner EF migration
> (`User.AppleId`) + the `customer-mobile-api` regen. **T-0312 (iOS customer shell + auth — 3 slices, §7.15) →
> `done`** (`2cf0f1e`+`6f9c1de`+`2ae5982`): **Slice A** = `CustomerRootView` (flat-enum, COPIES the ADR-0020
> pattern, the simpler customer gate — no RegistrationLock) + the 4-tab shell + the inert Book FAB (the
...
> (emits `AuthOutcome`, the router maps) + the **Core spine `RegisterEndpoint` fix** (a construction-time parameter:
...
> **Gate-DP divergences (recorded):** the Android `HorizontalPager` + floating-pill `CustomBottomBar` → native
> `TabView` + the FAB-as-overlay (native TabView has no center-action slot — ADR-0018 D3 sanctioned swap); the
> official Apple button + the recreated Google "G". **Tests:** **CleansiaCore 202 + CleansiaCustomer 42** (+ the
> CleansiaPartner 366 non-regression) on the iPhone 17 simulator; swiftformat + swiftlint --strict clean. The
> build-time security verifications are recorded in `security/ios-customer-auth.md`. **T-0314 follow-ups surfaced by
> the reviewers (recorded so they aren't lost):** (a) ship the customer brand asset to replace the SF-Symbol
> `AuthHeaderImage` + add the Android SignIn brand wordmark; (b) a brand-fidelity check of the recreated Google "G"
> vs Google's official asset before App Store submission — both folded into the T-0314 scope/notes (T-0314 has no
> ticket file — it is a §3 row). **Owner / manual steps (the PM never runs these), all gating LIVE social sign-in:**
> **T-0344** (Apple SIWA capability + `Apple:BundleId`), **T-0345** (Google client ids + `Google:ClientId` — IMP-1),
> the **EF migration for `User.AppleId`**, the **customer-mobile-api spec + client regen** (also needed for the
> T-0314 business endpoints + live social e2e). **Follow-up:** **T-0346** (backend Google `email_verified`
> hardening, depends_on T-0343). Resulting transitions: **T-0343 → `done`, T-0312 → `done`** (§3 + INDEX Wave-10
> roster); the owner tickets **T-0344/T-0345/T-0346** stay `proposed`/owner. The owner commits these backlog edits
> + opens the Phase-6 PR (the PM does not commit). **Reconciliation note for future phases (the squash-merge
> caveat):** Phase 5 was merged to master as the squash commit `c898e79` (PR #97) — `git merge-base --is-ancestor`
> against the original Phase-5 commits would misread NO, so verify any prior-phase status by master **TREE**
> content, not the ancestor check.

> **STATUS-LOG 2026-06-30 — 🎉 PHASE 8 DONE → the iOS PORT [partner + customer] is FEATURE-COMPLETE. T-0314
> (the customer TAIL — the LAST customer feature ticket, the broadest, `L → split` into 6 slices A–F)
> `proposed` → `done` on `phase/ios-phase8` (7 commits, pushed; Phase-8 PR drafted; off master `8d90104`).**
> This phase fills the four placeholder tabs T-0312 scaffolded plus every remaining customer surface — and so
> **completes the customer app AND the entire iOS port** (partner: Phases 0–5, feature-complete 2026-06-28;
> customer: Phases 6–8). The verified workflow ran: the §7.17 Understand-pass (parallel Android customer-tail
> + backend tail-contracts/security + cross-cutting iOS-mechanisms surveys) → architect cluster plan (§7.17,
> every choice composes an accepted ADR — **NO new ADR**) → the security design gate
> (**PASS-WITH-REQUIREMENTS**; GDPR-delete verdict **PASS**) → 6 dev slices each with a concurrent reviewer +
> Gate-SEC on the security-touching C/D/F. **§7.17 is CONFIRMED-AS-SHIPPED.** **T-0350** (NotificationPreferences
> rate-limit, the one LOW backend finding) was filed from the design gate and kept `proposed`. **T-0314
> (customer tail, DONE — 6 slices):** **A** (`6a587cf`) Home + Orders/OrderDetail — the paged list, the
> 7-state OrderStatus mirrored incl. OnTheWay=3, the lifecycle timeline / LiveProgressHero, cancel/review/
> receipt-via-QuickLook, the 5-min poller; folds in the T-0313 success→OrderDetail nav. **B** (`035c211`)
> Rewards/Loyalty/Referrals — tier/progress/perks + the paged activity feed + referral copy/share. **C**
> (`cae96dc`, **Gate-SEC**) Membership/Plus — the two-phase Stripe **SetupIntent** (Phase-1
> `paymentMethodConfirmed=false` → `{setupIntentClientSecret, stripeCustomerId, ephemeralKey}` → PaymentSheet;
> Phase-2, replaying the SAME idempotency token, → `{membershipId}` → re-read `membershipGetMine`) via the
> T-0313 `PaymentSheetPresenting`/`StripePaymentController` seam extended with a `PaymentIntentKind` SetupIntent
> path — **still the SOLE Stripe importer**; fail-closed on `StripeConfig` — plus Recurring + the deferred
> ConfirmRecurring PaymentIntent branch on OrderDetail. **Reviewer APPROVE + SECURITY PASS** (`.completed` is
> UX-only; the webhook is the sole active-state authority; no secret logged; idempotency-token replay; own-
> membership-only). **D** (`c10d819`, **Gate-SEC**) Disputes — list/create/detail + the **generated multipart**
> evidence upload (`disputeUploadEvidence(file:URL)` — the generated `FormDataEncoding` does REAL multipart),
> compress + **EXIF/GPS strip** (reuses the T-0308 `ImageCompressor`) + write-to-temp-file, fail-closed client
> validation mirroring the backend (≤ 10 MiB; image/jpeg|png|webp + application/pdf) + the customer target's
> first `NSCameraUsageDescription`/`NSPhotoLibraryUsageDescription` ×5 + the `PrivacyInfo.xcprivacy` photos
> entry. **Reviewer APPROVE + SECURITY PASS** (own-dispute server-scoped via `DisputeNotOwnedByUser`; the blob
> name is server-controlled; no secret in multipart). **E** (`58d9d35`) Addresses — the AddressManager 3-pane +
> saved-address CRUD on the Core `MapProvider`/`GeocodingService` seam (country-bias deferred → T-0334; the
> picker-VM harvest → T-0349). **F** (`4a15fbf`, **Gate-SEC**) Profile/Settings hub + **GDPR DeleteAccount**
> (the destructive confirm → `gdprDeleteMyAccount()` → the canonical local wipe `AuthApiClient.signOutLocal()`
> — **NOT `logout()`** — → route→login; **blocked mid-transaction → stay-signed-in** + the localized error
> (in-flight order / pending invoice); the Apple SIWA `/auth/revoke` **owner-deferred** per §7.14 D4
> identity-token-only + Apple TN3194, with the localized "remove Cleansia in Settings → Apple ID" note ×5) +
> **Devices** (the T-0310 D6-8 pattern verbatim) + **NotificationPreferences** (optimistic + debounced writes)
> + the **REAL change-password flow** + Language/Appearance/Help. **Reviewer APPROVE** (after a Profile-
> Subscribe-CTA fix) + **SECURITY PASS** (GDPR verdict **PASS** — anonymize-not-resurrect, own-account-only,
> the no-resurrect test green; `signOutLocal`-not-`logout`; the SIWA-revoke correctly deferred). **The
> Slice-F review caught an iOS-right/Android-stub parity gap** → the NEW follow-up **T-0351** (the Android
> customer `SecurityScreen` "Update" button is a dead no-op — wire it to the existing reset-code flow). **Tests:
> CleansiaCore 216 + CleansiaCustomer 362** (+ the CleansiaPartner 366 non-regression) on the iPhone 17
> simulator; swiftformat + swiftlint --strict clean. Build-time security verifications in
> `security/ios-customer-auth.md`. **OWNER / manual steps (for go-live/App Store — NOT code blockers; the PM
> never runs these):** the **Stripe publishable key** (`pk_`, now live card AND membership); **T-0344** (Apple
> SIWA capability + `Apple:BundleId`); **T-0345** (Google client ids/IMP-1); the **customer-mobile spec +
> client regen** (the membership `idempotencyToken` is cross-platform only after it — iOS already carries the
> field); the **camera/photo + customer-brand plist WORDING** sign-off; and the **App Store compliance pass** —
> the `ios-app-review-checklist.md` (ADR-0016) must go green per app: 5.1.1 in-app delete ✓ (shipped), SIWA ✓,
> Stripe-not-IAP framing, the ASC privacy answers + a demo account. **Follow-ups kept proposed (NOT
> dispatched):** **T-0348** (mobile PaymentIntent refund), **T-0349** (address-picker→Core harvest), **T-0350**
> (NotificationPreferences rate-limit), **T-0351** (the Android `SecurityScreen` catch-up); + the recorded
> notes (the customer brand asset to replace the SF-Symbol `AuthHeaderImage` + the Google-"G" pre-submission
> fidelity check; the `LiveCountryResolver`/country-bias fold into T-0334; the membership `idempotencyToken`
> customer-mobile NSwag regen; the dead `CreateOrderResponse.stripeSessionId` DTO cleanup at the next regen,
> S9 hygiene). **Remaining iOS scope after this merges:** the App Store compliance/release pass + the owner
> provisioning + the deferred follow-ups (T-0334/0337/0338/0340/0348/0349/0350/0351 + the brand assets) — **the
> FEATURE port is done.** Resulting transitions: **T-0314 → `done`** (§3 + INDEX Wave-10 roster); **§7.17 →
> CONFIRMED-AS-SHIPPED**; **T-0350 filed `proposed`; T-0351 filed `proposed`.** The owner commits these backlog
> edits + opens the Phase-8 PR (the PM does not commit). **Reconciliation note (the squash-merge caveat,
> restated):** the prior phases were squash-merged to master (master is now `8d90104`), so
> `git merge-base --is-ancestor` against the original phase commits misreads NO — verify any prior-phase status
> by master **TREE** content, not the ancestor check.

> **STATUS-LOG 2026-06-29 — 🍎 PHASE 7 DONE → HARD AREA #1 CLEARED. T-0313 (customer booking wizard + Stripe
> PaymentSheet) + T-0347 (backend money-safety) + T-0332 (the dual-use Bearer carve-out) `proposed`/`draft`/`ready`
> → `done` on `phase/ios-phase7` (7 commits, pushed; Phase-7 PR drafted; off master `c47f34a`).** This phase ships
> the **single hardest feature** in the port — the Bolt-style 3-step booking wizard + Stripe PaymentSheet, the
> customer **primary** flow (ADR-0013's named HARD AREA #1) — plus the T-0347 backend money-safety fix it surfaced.
> Commits in order: `f2113da` (the §7.16 Understand-pass + the T-0332 resolution + T-0347/0348/0349 docs), `afaa920`
> (T-0347 backend), `db4a12f`/`67f12e2`/`c42679d`/`4e30aff`/`8a2b4c7` (T-0313 Slices A–E). The verified workflow ran:
> a parallel Android/backend/Stripe survey (the Understand pass) → architect ruling (§7.16, every choice composes an
> accepted ADR — NO new ADR) → the security design gate (CHANGES-REQUESTED → surfaced the HIGH backend defect T-0347
> + 4 iOS ACs, all satisfied) → the 5 dev slices each with a concurrent reviewer + Gate-SEC on D+E. **T-0347
> (backend money-safety, DONE)** (`afaa920`): one charge surface per card order — a per-host `IOrderChannelProvider`
> so `OrderPaymentDispatcher` suppresses the Stripe **Checkout Session** for the mobile **PaymentSheet** channel
> (the mobile order's single surface is the PaymentIntent). Closes a **pre-existing double-capture defect** — the
> live **Android** card flow had it too (different idempotency keys; the webhook made order STATE idempotent but not
> the captures). Host-based discriminator (the audience couldn't tell web from mobile — both register
> `JwtAudiences.Customer`); NO contract change, NO regen, NO EF migration. Reviewer **APPROVE** + security **PASS**.
> Residual follow-up **T-0348** (the mobile PaymentIntent refund path — admin-only, latent; gates LIVE mobile
> refunds, NOT this fix). **T-0313 (customer booking wizard + Stripe, DONE — 5 slices):** **A** (`db4a12f`) the
> 3-step modal anchored sheet (`.sheet`/`.presentationDetents` — ADR-0018 D3 modal mapping, NOT the ADR-0021
> SnapSheet) + the shared `BookingViewModel` + the Book FAB action (replaces the inert T-0312 FAB). **B** (`67f12e2`)
> Step 1 Services + the server-authoritative pricing engine (debounced live `Quote` + the `BookingPricing` display
> port; iOS echoes the raw `TotalPrice` verbatim, `PriceMatchesAsync` re-validates, the charge reads
> `order.TotalPrice` — NO client-trusted price on the money path) + the first **customer generated-client auth
> spine** (`CustomerCoreSpineRequestBuilderFactory`, the ADR-0019 twin of the partner spine). **C** (`c42679d`)
> Step 2 When&Where (the Core-map-seam address picker + lead-time time slots) + the Confirm extras/promo/referral
> FSMs. **D** (`4e30aff`) Step 3 Confirm + the CASH submit + the **T-0332 dual-use Bearer carve-out**
> (`HeaderAdapter` attaches the Bearer on the 3 booking endpoints **iff a token exists**; guest tokenless; pure-anon
> never; `CreatePaymentIntent` always authed) + the server-authoritative price echo + the double-submit debounce.
> **Reviewer APPROVE + SECURITY PASS** (no pure-anon Bearer leak; no under-charge; guest path + partner
> non-regression preserved). The cash success screen is **status-accurate** ("Booking received", Pending+New per
> §7.16 D4 — NOT "Confirmed"). **E** (`8a2b4c7`) the CARD branch + Stripe **PaymentSheet** (`StripePaymentSheet`
> SPM dep, **customer target only** @25.17.0; the `PaymentSheetPresenting`/`StripePaymentController` seam = the
> **sole Stripe importer**; publishable-key **fail-closed**). **Reviewer APPROVE + SECURITY PASS** (`.completed` is
> **UX-only** — the webhook is the sole paid authority; no secret leak; fail-closed; single charge surface).
> **Resolves T-0332** (`draft` → `done`/RESOLVED — the dual-use carve-out shipped in Slice D). **Gate-DP
> divergences (recorded):** the Android anchored bottom-sheet → native `.sheet`/`.presentationDetents`; the official
> Stripe PaymentSheet (vs the Android PaymentSheet); the address picker on the Core map seam. **Tests: CleansiaCore
> 213 + CleansiaCustomer 156** (+ the CleansiaPartner 366 non-regression) on the iPhone 17 simulator; swiftformat +
> swiftlint --strict clean. The **customer generated client is gitignored** (CI regenerates). Build-time security
> verifications in `security/ios-customer-auth.md`. **Owner / manual steps (the PM never runs these):** the **Stripe
> publishable key** (`pk_`, the sole LIVE-card provisioning — the code ships fail-closed; the T-0347 fix is landed);
> the standing owner items carry forward (**T-0344** Apple, **T-0345** Google, the **customer-mobile regen**, the
> **camera/photo plist wording** ×5). **Follow-ups kept proposed:** **T-0348** (mobile PaymentIntent refund),
> **T-0349** (address-picker→Core harvest), + the **T-0314** items (success→OrderDetail nav + the lifecycle
> timeline/`OrderSummaryCard` when the customer Orders feature lands; the customer brand asset to replace the Phase-6
> SF-Symbol `AuthHeaderImage` + the Google "G" brand-fidelity check; saved-address CRUD; the `LiveCountryResolver`
> fold into the T-0334 `ServiceAreaProvider` seam; a minor backend DTO cleanup — `CreateOrderResponse.stripeSessionId`
> is a dead legacy field iOS never reads, to drop at the next NSwag/spec regen, S9 hygiene). Resulting transitions:
> **T-0313 → `done`, T-0347 → `done`, T-0332 → `done`/RESOLVED** (§3 + INDEX Wave-10 roster). The owner commits
> these backlog edits + opens the Phase-7 PR (the PM does not commit). **Reconciliation note (the squash-merge
> caveat, restated):** the prior phases were squash-merged to master (master is now `c47f34a`), so
> `git merge-base --is-ancestor` against the original phase commits misreads NO — verify any prior-phase status by
> master **TREE** content, not the ancestor check (Phase 6's AppleAuth + customer-auth files confirmed present in
> the master tree this session).
...
| **T-0312** | **Customer app shell SCAFFOLD + FULL auth** (the FIRST customer feature) — `CustomerRootView` flat-enum root-switch (the ADR-0020 *pattern*, copied) splash-gated, **NO RegistrationLock** (the simpler customer gate) → `CustomerShellView` (4-tab `TabView` Home·Orders·Rewards·Profile + center **Book FAB**, tabs as placeholders, FAB present-but-INERT) + the FULL auth surface (SignIn/SignUp/EmailVerify/ForgotPassword + the event-driven `CustomerAuthViewModel` + **Google via GoogleSignIn-iOS + Apple via `ASAuthorizationAppleIDButton`/SIWA**). **Acceptance scope + the 7 Understand-pass rulings fixed in §7.15 (no new ADR):** D1 (the `CustomerRootView` PATTERN-copy of ADR-0020 D5, no registration gate — Android customer `CleansiaNavHost.kt:137-147` parity), D2 (the T-0305 chain + the Android `AuthViewModel` event contract + SIWA-above-Google below the `OR` divider, AR-ACCT-2/4.8), D3 (provider-ACQUISITION app-local in `CleansiaCustomer`; provider-CONSUMPTION = 2 new `AuthApiClient` methods reusing the shared empty-token gate + the single Keychain mutation; `+/api/auth/appleauth` to the allow-list), D4 (the T-0304 shell-scaffold deferral map — Book FAB action → T-0313, tab content + soft onboarding → T-0314, LIVE social → owner-gated), D5 (build shell+root+acquisition+VMs AHEAD of the regen against a `SocialSignInProviding` protocol + fakes; bind the live AppleAuth POST after T-0343+regen), D6 (Gate-DP: pager+floating-pill → native `TabView`+FAB-overlay, official SIWA button, GoogleSignIn-iOS SPM dep + the `com.apple.developer.applesignin` entitlement in `project.yml`), D7 (3 slices A scaffold / B auth-core / C social; **B+C SECURITY-TOUCHING → Gate-SEC parallel**). TC-IOS-CUSTOMER-ROUTER-SEED/-SPLASH-RESOLVE/-SHELL/-AUTH-OUTCOME/-EMPTYTOKEN/-VERIFY-EMAIL-ARG/-ANON + TC-IOS-SOCIAL-OUTCOME/-NONCE. Gate-DP + Gate-AR. **Android parity: customer `features/main/MainShell.kt`, `navigation/CleansiaNavHost.kt`, `features/auth/{AuthViewModel,SignInScreen,SignUpScreen,EmailVerifyScreen}.kt`, `core/auth/GoogleSignInController.kt`.** | M (**3 slices**) | **done ✅** `2cf0f1e`+`6f9c1de`+`2ae5982` (`phase/ios-phase6`, off master `c898e79`; the §7.15 ruling. **Slice A** = `CustomerRootView` (flat-enum, COPIES ADR-0020, the simpler customer gate — NO RegistrationLock) + the 4-tab `TabView` shell + the inert Book FAB (the FAB-as-overlay Gate-DP swap) + the new `CleansiaCustomerTests` target; ios-ci now `build test`s CleansiaCustomer. **Slice B** = the email auth chain (SignIn/SignUp/EmailVerify/Forgot) + the event-driven `CustomerAuthViewModel` (emits `AuthOutcome`, the router maps) + the Core spine **`RegisterEndpoint` fix** (a construction-time param: customer→`/api/Auth/Register`, partner→`RegisterEmployee`, byte-equivalent). **Security PASS** (no parallel write path; partner non-regression). **Slice C** = social — the official `ASAuthorizationAppleIDButton` + the real multicolor Google "G"; the **`SocialSignInProviding` Core seam** (the Apple nonce flow + GoogleSignIn-iOS, the sole framework consumers); two spine methods `googleAuth`/`appleAuth` (reuse the one gate + persist; appleauth anon-allow-listed); the GoogleSignIn SPM dep + the `com.apple.developer.applesignin` entitlement + the reversed-client-id placeholder; fail-safe when unconfigured. Reviewer **APPROVE**; **SECURITY PASS** (the iOS↔backend nonce-encoding ALIGNED; no parallel write path). Gate-DP divergences: pager+pill → native `TabView`+FAB-overlay; official Apple button + the recreated Google "G"; the `AuthHeaderImage` SF-Symbol PLACEHOLDER → T-0314 brand asset. **CleansiaCore 202 + CleansiaCustomer 42** (+ partner 366 non-regression) on iPhone 17; swiftformat/swiftlint --strict clean; build-time verifications in `security/ios-customer-auth.md`. T-0314 follow-ups recorded (brand asset + Google-"G" fidelity). LIVE social sign-in OWNER-gated → T-0344/T-0345 + the EF migration + the customer-mobile regen) | ios | T-0302✓, T-0306✓, T-0343✓ | **owner (gates LIVE social, code ships fail-closed): EF migration (`User.AppleId`) + the `customer-mobile-api` spec+client regen + T-0344 (Apple capability/`Apple:BundleId`) + T-0345 (Google client ids/IMP-1) — the T-0311-gated-by-T-0342 pattern** | **6 (customer; the FIRST customer feature; `phase/ios-phase6`)** |
| **T-0313** | **Customer booking wizard + Stripe** — the 3-step Bolt-style anchored sheet (Services / WhenWhere / Confirm), client-side pricing, **cash→success vs card→`stripe-ios` PaymentSheet** | **L → split** (**5 slices A–E**) | **done ✅** `db4a12f`+`67f12e2`+`c42679d`+`4e30aff`+`8a2b4c7` (`phase/ios-phase7`, off master `c47f34a`; the §7.16 ruling. **HARD AREA #1 CLEARED.** **A** = the 3-step modal anchored sheet (`.sheet`/`.presentationDetents`) + the shared `BookingViewModel` + the Book FAB action. **B** = Step 1 Services + the server-authoritative pricing engine (debounced live `Quote` + the `BookingPricing` display port) + the first customer generated-client auth spine (`CustomerCoreSpineRequestBuilderFactory`, the ADR-0019 twin). **C** = Step 2 When&Where (the Core-map-seam address picker + lead-time time slots) + the Confirm extras/promo/referral FSMs. **D** = Step 3 Confirm + the CASH submit + the **T-0332 dual-use Bearer carve-out** (`HeaderAdapter` attaches Bearer on the 3 booking endpoints iff a token exists; guest tokenless; pure-anon never; `CreatePaymentIntent` always authed) — server-authoritative price echo + double-submit debounce; the cash success screen is status-accurate ("Booking received", Pending+New per §7.16 D4 — NOT "Confirmed"). **E** = the CARD branch + Stripe **PaymentSheet** (`StripePaymentSheet` SPM dep customer-only @25.17.0; the `PaymentSheetPresenting`/`StripePaymentController` seam = the sole Stripe importer; publishable-key fail-closed). Reviewer **APPROVE** + **SECURITY PASS** on D+E (price-tampering NO; the dual-use Bearer scoping verified — no pure-anon Bearer leak; `.completed` is UX-only / the webhook is the sole paid authority; no secret leak; fail-closed; single charge surface). **CleansiaCore 213 + CleansiaCustomer 156** (+ partner 366 non-regression) on iPhone 17; swiftformat + swiftlint --strict clean. The customer generated client is gitignored (CI regenerates). Build-time verifications in `security/ios-customer-auth.md`. **Resolves T-0332** (Slice D) + **T-0347** (the backend money-safety prerequisite, landed `afaa920`). **LIVE card is OWNER-gated on the Stripe publishable key (pk_); the code ships fail-closed.** Follow-ups: **T-0348** (mobile PaymentIntent refund) + **T-0349** (address-picker→Core harvest) + the T-0314 items | ios | T-0312✓ | — | 2 (**HARD AREA #1 — hardest; `phase/ios-phase7`**) |
| **T-0314** | Customer parity tail — Home, Orders+OrderDetail, Rewards/loyalty, Membership (SubscribePlus→Stripe), Recurring, **Disputes (multipart `IFormFile` evidence)**, Addresses (map seam), Profile/Settings (incl. **DeleteAccount/GDPR**, Devices, Notification prefs). **+ Two T-0312-reviewer follow-ups folded in (recorded 2026-06-28):** (a) ship the customer **brand asset** to replace the T-0312 SF-Symbol `AuthHeaderImage` placeholder **+ add the Android SignIn brand wordmark** to the iOS auth header (Gate-DP parity); (b) a **brand-fidelity check of the recreated Google "G"** (shipped in T-0312 Slice C) vs Google's official asset **before App Store submission**. | **L → split (6 slices A–F)** | **done ✅** `6a587cf`+`035c211`+`cae96dc`+`c10d819`+`58d9d35`+`4a15fbf` (`phase/ios-phase8`, off master `8d90104`; the §7.17 ruling — **CONFIRMED-AS-SHIPPED**. **🎉 THE LAST CUSTOMER FEATURE — the iOS PORT [partner + customer] is now FEATURE-COMPLETE.** **A** (`6a587cf`) Home + Orders/OrderDetail (paged list, the 7-state OrderStatus incl. OnTheWay=3, lifecycle timeline/LiveProgressHero, cancel/review/receipt-via-QuickLook, the 5-min poller) + the T-0313 success→OrderDetail fold. **B** (`035c211`) Rewards/Loyalty/Referrals (tier/progress/perks + paged activity + referral copy/share). **C** (`cae96dc`, Gate-SEC) Membership/Plus (the two-phase Stripe **SetupIntent** via the `StripePaymentController` seam extended with `PaymentIntentKind` — still the sole Stripe importer) + Recurring + the deferred ConfirmRecurring on OrderDetail. Reviewer **APPROVE** + security **PASS**. **D** (`c10d819`, Gate-SEC) Disputes (list/create/detail + the generated **multipart** evidence upload, EXIF-strip, fail-closed file validation) + the customer's first camera/photo permission strings + `PrivacyInfo`. Reviewer **APPROVE** + security **PASS**. **E** (`58d9d35`) Addresses (AddressManager 3-pane + saved-address CRUD on the Core map seam). **F** (`4a15fbf`, Gate-SEC) Profile/Settings hub + **GDPR DeleteAccount** (`signOutLocal`-not-`logout`; blocked→stay-signed-in; the SIWA-revoke owner-deferred + the "remove in Settings" note) + Devices (T-0310 D6-8) + NotificationPreferences (optimistic+debounced) + the REAL change-password flow + Language/Appearance/Help. Reviewer **APPROVE** (after the Profile-Subscribe-CTA fix) + security **PASS** (GDPR verdict **PASS**). Security posture (all PASS vs code, `security/ios-customer-auth.md`): membership money path (`.completed` UX-only / webhook-authoritative / fail-closed / idempotency-token replay), dispute multipart upload (own-dispute server-scoped / fail-closed validation / EXIF-strip / no-secret), GDPR delete (anonymize-not-resurrect / own-account-only / signOutLocal / SIWA-revoke deferred per §7.14 D4 + TN3194). **Tests: CleansiaCore 216 + CleansiaCustomer 362** (+ the CleansiaPartner 366 non-regression) on iPhone 17; swiftformat + swiftlint --strict clean. **OWNER / manual steps:** the Stripe publishable key (live card + membership); **T-0344** (Apple SIWA capability); **T-0345** (Google client ids/IMP-1); the customer-mobile spec+client regen; the camera/photo + customer-brand plist WORDING sign-off; the **App Store compliance pass** (`ios-app-review-checklist.md`, ADR-0016 — 5.1.1 in-app delete ✓, SIWA ✓, Stripe-not-IAP framing, ASC privacy answers + demo account). **Follow-ups:** **T-0348** (mobile PaymentIntent refund) + **T-0349** (address-picker→Core harvest) + **T-0350** (NotificationPreferences rate-limit) + **T-0351** (the NEW Android customer `SecurityScreen` dead-stub catch-up); + notes (the customer brand asset + the Google-"G" pre-submission fidelity check; the `LiveCountryResolver`/country-bias → T-0334; the membership `idempotencyToken` customer-mobile NSwag regen — iOS already carries the field; the dead `CreateOrderResponse.stripeSessionId` DTO cleanup at the next regen) | ios | T-0312✓, T-0306✓, T-0313✓ | **owner: Stripe pk_ (live card + membership) + T-0344 (Apple) + T-0345 (Google/IMP-1) + customer-mobile spec+client regen + camera/photo + customer-brand plist WORDING sign-off + the App Store compliance/ASC pass** | 2 (customer; **THE LAST customer feature — iOS PORT FEATURE-COMPLETE**; `phase/ios-phase8`) |
...
> `agents/archive/2026-08/backlog/security/ios-devices.md`. The backend Device surface was traced on this Mac and is **server-scoped
...
> `agents/archive/2026-08/backlog/security/ios-earnings.md`. This is the index pointer + the headline ruling.
...
`agents/archive/2026-08/backlog/security/ios-earnings.md`.
...
| **T-0329** | **Pre-submission audit** — create + run `agents/archive/2026-08/backlog/ios-app-review-checklist.md`: every AR-* item per app (partner/customer) + the App-Store-Connect prerequisites (App Privacy answers, demo account, export-compliance, screenshots, age rating). **Release gate** — the first submission waits on it green | M | **proposed** | ios, qa, docs | T-0323..T-0328 | **owner: ASC submission prerequisites** (App Privacy, demo account, export-compliance, screenshots, signing) | **Pre-submission** (the final gate) |
...
#### MANUAL_STEPs (owner)
1. ~~**EF migration** for the new `User.AppleId` column~~ — **DONE 2026-06-28 (owner-authorized):** regenerated into the single `Initial` migration (pre-prod, no deployed DB), original timestamp preserved; `Cleansia.IntegrationTests` 97/97 + `Cleansia.HostTests` 60/60 green. (No further migration action needed.)
...
### 7.16 T-0313 (Phase-7 — HARD AREA #1: the customer booking wizard + Stripe PaymentSheet) — acceptance scope + the Understand-pass rulings + the T-0332 resolution (recorded 2026-06-28, architect)

> **CONFIRMED-AS-SHIPPED + CLOSED 2026-06-29 — T-0313 is `done`; HARD AREA #1 is CLEARED** (§3 / status-log).
> Shipped in **5 slices A–E** on `phase/ios-phase7` (`db4a12f`+`67f12e2`+`c42679d`+`4e30aff`+`8a2b4c7`, off
> master `c47f34a`): A the 3-step modal anchored sheet + the shared `BookingViewModel` + the Book FAB; B Step 1
> Services + the server-authoritative pricing engine (debounced live `Quote` + the `BookingPricing` port) + the
> customer ADR-0019 spine (`CustomerCoreSpineRequestBuilderFactory`); C Step 2 When&Where (the Core-map-seam
> address picker + lead-time slots) + the extras/promo/referral FSMs; D Step 3 Confirm + the CASH submit + **the
> T-0332 dual-use Bearer carve-out** + the status-accurate cash success screen (Pending+New, §7.16 D4 — NOT
> "Confirmed"); E the CARD branch + Stripe **PaymentSheet** (the `PaymentSheetPresenting`/`StripePaymentController`
> seam = the sole Stripe importer; publishable-key fail-closed). Reviewer **APPROVE** + **SECURITY PASS** on D+E
> (price-tampering NO; the dual-use Bearer scoping verified; `.completed` is UX-only / webhook is the sole paid
> authority; no secret leak; fail-closed; single charge surface). **CleansiaCore 213 + CleansiaCustomer 156** (+
> partner 366 non-regression) on iPhone 17; swiftformat + swiftlint --strict clean. **Resolves T-0332** (Slice D)
> + the backend money-safety prerequisite **T-0347** (`afaa920`, reviewer APPROVE + security PASS). **LIVE card is
> OWNER-gated on the Stripe publishable key (pk_); the code ships fail-closed.** Follow-ups: **T-0348** (mobile
> PaymentIntent refund), **T-0349** (address-picker→Core harvest), + the T-0314 items. The customer generated
> client is gitignored (CI regenerates); build-time verifications in `security/ios-customer-auth.md`.

T-0313 is the **single hardest iOS feature** — ADR-0013's named **HARD AREA #1** (the Bolt-style 3-step anchored booking sheet, cash→success vs card→Stripe, server-authoritative pricing). It is the **action behind the inert Book FAB** T-0312 already shipped (`CustomerShellView.swift:10` `func book() {}`); T-0313 makes it open the wizard. `depends_on` T-0312 (shell+auth, landed) + T-0302 (codegen — the generated `CustomerOrderAPI`/`CustomerPaymentAPI` are **already on disk**, see below). It also **resolves T-0332** (the dual-use Bearer checkpoint, `depends_on: [T-0313]`, carried here as ACs). Surfaced by the T-0313 Understand pass against the Android booking survey + the backend payment contract + the Stripe-iOS research.

> **Every ruling APPLIES accepted ADRs + prior records — NO new ADR.** (1) the sheet shape APPLIES **ADR-0018 D3** (the `ModalBottomSheet → .sheet + .presentationDetents` MODAL mapping) — explicitly the modal case, NOT the **ADR-0021** `SnapSheet` (reserved for the non-modal-over-a-live-map partner OrderDetail; ADR-0021 D3's modal/non-modal discriminator resolves the booking sheet to **native**). (2) Stripe APPLIES **ADR-0013 D7** (the `stripe-ios` PaymentSheet, customer target only) + the §7.13/ADR-0014 D6′ **Core-seam family** idiom (system SDK behind one app-local seam) + the §7.14/T-0311 **ship-vs-owner-gated** + fail-closed pattern. (3) the network APPLIES **ADR-0019** (the generated client on the `RequestBuilderFactory` spine) + **ADR-0011 D4** (`ApiResult`). (4) pricing APPLIES the backend's server-authoritative `IOrderPricingCalculator` + `PriceMatchesAsync` contract. (5) the T-0332 carve-out APPLIES **ADR-0013 §D4.4** / header-parity §3 (the allow-list single-source intent, untouched). **A genuinely-new trade-off requiring a new ADR was looked for and NOT found** — every choice composes an accepted decision; the L→split + the payment-slice Gate-SEC carry the rigor.

> **On-disk reality (verified 2026-06-28 — the contract is COMPLETE, no regen needed):** `CustomerOrderAPI.orderQuote` (`/api/Order/Quote`) + `orderCreateOrder` (`/api/Order/CreateOrder`) and `CustomerPaymentAPI.paymentCreatePaymentIntent` (`/api/Payment/CreatePaymentIntent`) all exist in the generated `CleansiaCustomerApi`, riding the ADR-0019 `requestBuilderFactory` (`CleansiaCustomerApiAPI.requestBuilderFactory.getBuilder()`). **`CreatePaymentIntentResponse` already carries `clientSecret`, `paymentIntentId`, `stripeCustomerId`, `ephemeralKey`** (`CreatePaymentIntentResponse.swift:15-18`) — the backend is **PaymentSheet-ready**, so T-0313 needs **no backend DTO change and no spec/client regen** (unlike T-0312, which rode the AppleAuth regen). The dual-use endpoints are on `AnonymousAllowList.customerGuestBooking` (`AnonymousAllowList.swift:35,38`) and `HeaderAdapter.apply` withholds the Bearer for any allow-listed path regardless of token (`HeaderAdapter.swift:29`) — this is the exact T-0332 surface. The Stripe SDK + the wizard/VM/views are **net-new**.

#### Decision 1 — Scope vs T-0314: T-0313 = the FULL 3-step wizard + server pricing + cash/card + Stripe PaymentSheet; Apple Pay + the recurring path + the tab content are T-0314 (APPLIES the §7.15 D4 deferral-map precedent; NO new ADR)

**RULING (CONFIRMED): T-0313 ships the complete booking front-to-back — the 3-step modal anchored sheet, the server-authoritative pricing, BOTH the cash and card branches, and the Stripe PaymentSheet (code-complete, fail-closed). DEFERRED → T-0314: Apple Pay, the recurring/`ConfirmRecurringOrder` path, deep promo/referral UX beyond the one-shot dialogs, and the tab CONTENT (already T-0314).** Booking is shipped **whole** because a half-wizard is not a shippable commerce flow and it is the primary customer surface. A **scope record**, not a new trade-off.

- **IN:** Step 1 Services (services/packages multi-select sets, rooms/bathrooms steppers, catalog categories), Step 2 When&Where (address row → AddressManager overlay on the `MapProvider`/`GeocodingService` seam, day strip today+7, 1h slots 08:00–19:00 lead-time-band-gated), Step 3 Confirm (extras, receipt, promo dialog, card/cash picker, special instructions, Plus preferred-cleaner, cancellation policy + trust badges); the shared `BookingViewModel` mirroring the Android 5 StateFlows + the sealed `BookingSubmitOutcome` (Success | CardPending | Failed | ProfileIncomplete); the debounced live re-quote + the cash and card submit paths; the Stripe SPM dep + publishable-key config + the PaymentSheet flow; the T-0332 carve-out.
- **DEFERRED → T-0314:** **Apple Pay** (Decision 4 — non-trivial Merchant-ID/cert provisioning; additive `config.applePay` later with **no flow change**, `config.applePay = nil` ships card-only); the **recurring** cash-auto-confirm path (`ConfirmRecurringOrder` — one-off only here); saved-payment-method UX beyond what PaymentSheet's `CustomerConfiguration` gives free; the **Home/Orders/Rewards/Profile tab content** (already T-0314).

#### Decision 2 — T-0332: SEND the Bearer on the dual-use Order/Payment endpoints when a session exists; withhold ONLY for a true guest (APPLIES ADR-0013 §D4.4 / header-parity §3 — the allow-list single-source intent UNCHANGED; NO new ADR)

**RULING (CONFIRMED, decisive): iOS attaches `Authorization: Bearer …` on `/api/Order/CreateOrder`, `/api/Payment/CreateOrder`, and `/api/Order/Quote` whenever a non-expired session token exists, and withholds it ONLY for a genuine tokenless guest — a per-request "attach-Bearer-iff-a-token-exists" carve-out, NOT an allow-list deletion.** Grounded in the backend survey: all three are `[AllowAnonymous]` but **dual-use** (`GetUserId() ?? string.Empty`, CreateOrder.cs:215 / QuoteOrder.cs:109). WITH a Bearer the order binds to the user (`order.UserId`, OrderFactory.cs:56/111), the best-of-three discount snapshot resolves, SavedAddressId ownership (OrderAddressResolver.cs:63) + PreferredEmployeeId eligibility (CreateOrder.cs:128) are enforced, and the webhook fires the OrderConfirmed push (HandlePaymentNotification.cs:266). WITHOUT it the same call **silently** creates a guest order (UserId=null, Order.cs:285): no discounts, never in GetMyOrders, no push. A naive port would downgrade every signed-in booking to a guest order — a real parity defect. The fix is a **dual-use set the `HeaderAdapter` consults** so these paths stay tokenless for a true guest but attach the Bearer when a token is present; the `customerGuestBooking` entries **stay** (ADR-0013 §D4.4, reviewer check #4 not regressed). `CreatePaymentIntent` is the exception — strictly `[Authorize]` + ownership-checked (PaymentController.cs:31), already Bearer'd via the ADR-0019 spine; a true guest cannot complete a **card** booking in-app (guest card is web-only). T-0332 AC1–AC4 land on Slice D.

#### Decision 3 — Pricing: the SERVER is authoritative on the charged amount; iOS computes only a DISPLAY quote over the server's numbers (APPLIES the backend `IOrderPricingCalculator` + `PriceMatchesAsync` contract; NO new ADR)

**RULING (CONFIRMED): there is no client-trusted price on the money path.** iOS calls `POST /api/Order/Quote` (the generated `orderQuote`) as the authoritative source — a debounced (400ms, distinctUntilChanged) live re-quote on every input change + a submit-time reuse-cached-or-refetch — and echoes the quote's **RAW `totalPrice`** (the pre-discount subtotal incl. the server-folded express surcharge) **verbatim** as `CreateOrder.TotalPrice`, which the backend re-recomputes and validates via PriceMatchesAsync (CreateOrder.cs:147-164; mismatch → `TotalPriceNotMatch`). iOS computes a **display** quote only — the discount-then-surcharge **ordering** math (the `BookingPricing.kt` port: `max(tier,promo)` first, then +20% express surcharge on the discounted subtotal) over the server's line-items for the receipt + the slide-to-confirm label. The **charged** amount is sourced authoritatively from the persisted Order on the backend (`CreatePaymentIntent` uses `order.TotalPrice`/`order.Currency.Code`, never the client). Pass the SAME `CleaningDate` to Quote and Create. Promo is NOT in the quote (applied at create-time); referral is sent raw (backend fail-soft).

#### Decision 4 — Stripe consumption: the StripePaymentSheet SPM dep in an app-local Core-seam-family controller; CreatePaymentIntent→client_secret→PaymentSheet→result; the cash branch short-circuits (APPLIES ADR-0013 D7 + the §7.13 seam-family + the §7.14/T-0311 ship-fail-closed pattern; NO new ADR)

**RULING (CONFIRMED): the official `StripePaymentSheet` SPM product is added to the `CleansiaCustomer` target ONLY (ADR-0013 D7 — not Core, not partner), consumed by a thin app-local `PaymentSheetPresenting`/`StripePaymentController` that is the SOLE importer of the Stripe SDK (the §7.13 `PushRegistrar` / §7.6 `MapProvider` seam-family idiom). The booking VM + views import neither Stripe nor any card UI; the controller rides a fakeable protocol so the VM unit-tests against a fake (the T-0312 `SocialSignInProviding` precedent).** Add only the `StripePaymentSheet` library product (not the legacy umbrella `Stripe`); min iOS 15 < the ADR-0014 16.0 floor. The publishable key is set once at launch from a build-injected `STRIPE_PUBLISHABLE_KEY` (xcconfig→Info.plist, `$(...)`-collapsed-to-empty like `AppConfig.googleClientID`, AppConfig.swift:22-25) — the iOS analogue of Android's `PaymentConfiguration.init` in `CleansiaApp.onCreate`; only the publishable key ships, the secret key stays server-only.

- **Card branch:** the VM submits → generated `orderCreateOrder` (`paymentType=2`, raw `totalPrice` echoed) creates a **Pending** order → generated `paymentCreatePaymentIntent({orderId})` (`[Authorize]`, Bearer via the spine) returns `{clientSecret, ephemeralKey, stripeCustomerId, paymentIntentId}` → `CardPending(PaymentSheetParams)` → the view presents PaymentSheet via the SwiftUI `.paymentSheet(...)` modifier (`merchantDisplayName="Cleansia"`, `customer=.init(id:ephemeralKeySecret:)`, `applePay=nil`, `allowsDelayedPaymentMethods=false`); **SCA/3DS is in-sheet** (backend `AutomaticPaymentMethods.Enabled=true`). Result: `.completed` → navigate to success (**a UX signal ONLY — the order moves Pending→Confirmed solely via `/api/Payment/webhook`, never by trusting the client**); `.canceled` → snackbar, order stays Pending (swept by the backend cleanup ~1h — iOS must NOT cancel it), allow retry/cash; `.failed` → error, retry.
- **Cash branch:** `paymentType=1`, Create returns Success (StripeSessionId=null) → success screen, **no Stripe call**. (Correcting CLAUDE.md: a one-off cash order is **not** auto-confirmed at create — it stays Pending+New until a cleaner takes it, OrderPaymentDispatcher.cs:47-57; iOS must not show "Confirmed" for a fresh cash booking.)
- **Fail-closed (the §7.14/T-0311 pattern):** before offering card, check for a non-empty publishable key; if empty/missing, do NOT init STPAPIClient or present PaymentSheet, hide/disable "Pay by card", offer only Cash — never crash, never present an unconfigured sheet, never appear to take an un-completable card payment. Card is purely additive over the always-available cash path.

#### Decision 5 — The slice plan (L → 5 slices; D + E are SECURITY-touching → Gate-SEC in PARALLEL) + the ship-vs-owner-gated split

**RULING: 5 slices. A/B/C are the wizard + pricing (Gate-DP). D (cash submit + the T-0332 carve-out) and E (the card/PaymentSheet path) are SECURITY-TOUCHING → Gate-SEC runs in PARALLEL on D + E.** This record fixes the architectural seams; the security enforcement is the security charter's Gate-SEC. **Slice A** = the modal 3-step sheet scaffold + step-nav + the shared `BookingViewModel`/5-state shape (no network). **Slice B** = ServicesStep + the server-authoritative pricing engine (debounced `orderQuote` + the `BookingPricing` display math). **Slice C** = WhenWhereStep + address (on the `MapProvider` seam) + extras/promo/referral. **Slice D (Gate-SEC)** = ConfirmStep + the cash submit (CreateOrder echoing the raw price → Success) + the **T-0332 dual-use Bearer carve-out** + the double-submit debounce. **Slice E (Gate-SEC)** = the Stripe SPM dep + publishable-key fail-closed config + the `PaymentSheetPresenting` seam + CreatePaymentIntent → PaymentSheet → result. Ordering: A→B→C (the wizard + pricing prove out), then D (cash, the always-available path) before E (card, additive).

**Ship-vs-owner-gated (the T-0342-gates-T-0311 shape):** the **wizard + cash + ALL the card CODE + the T-0332 carve-out SHIP** in T-0313 against an empty/placeholder publishable key (reviewer/Gate-SEC verify the seam + fail-closed, not a live charge); cash is fully functional with no Stripe key. **LIVE card payment is OWNER-GATED on the single provisioning step — the Stripe publishable key (pk_live_ in Release).** No backend change, no webhook, no regen. **Apple Pay is DEFERRED to T-0314** (Merchant-ID + processing cert + capability; `config.applePay = nil` ships card-only with no later flow change).

#### New CRC role (added with the T-0313 wiring)

- **`ios-stripe-payment-controller`** — the app-local `PaymentSheetPresenting`/`StripePaymentController` (in `CleansiaCustomer/Features/Booking/`, the next member of the §7.13/§7.6 Core-seam family): *responsibility:* present the Stripe PaymentSheet for a **server-created** PaymentIntent and return the typed `PaymentSheetResult`. *Collaborators:* `StripePaymentSheet`, the publishable-key config, the booking VM (via a fakeable protocol). *Does NOT know:* the order/charge amount (the server owns it — the controller only holds the `client_secret`), how to create or confirm a PaymentIntent (it never does), the token store, or navigation (it returns a result; the view/VM navigates). **If this controller ever trusts `.completed` to flip order state, or sends/computes a price, the responsibility is wrong — the webhook is the source of truth and the amount is the server's.**

(The booking VM rides the existing `ios-generated-client-auth-bridge` spine for Quote/Create/CreatePaymentIntent; the T-0332 carve-out extends the `ios-header-adapter` role with a dual-use set — the single-token-source invariant is unchanged.)

#### Open question for the owner (non-blocking)

- **None that blocks T-0313's code.** The one LIVE-card gate is the **Stripe publishable key** (owner provisioning, Decision 5) — code ships fail-closed against an empty key; the live acceptance is the owner's once `pk_live_` is set. Apple Pay's provisioning is **deferred to T-0314**, not a T-0313 blocker. No spec/client regen, no backend change.
#### Security design gate (Gate-SEC, T-0313) — CHANGES-REQUESTED → the iOS card code ships fail-closed; LIVE card gated on T-0347 + the Stripe key

**Price-tampering: PASS** (NO under-charge — server-authoritative; iOS echoes the raw `TotalPrice`, `PriceMatchesAsync` re-validates, the charge reads `order.TotalPrice`/`order.Currency.Code`). The verdict is **CHANGES-REQUESTED** on ONE HIGH backend defect + 4 iOS implementation ACs:

- **HIGH (backend defect → T-0347, a LIVE-card prerequisite):** `OrderPaymentDispatcher` ALWAYS creates a Stripe **Checkout Session** for Card orders (`OrderPaymentDispatcher.cs:26-33`, no web-vs-mobile discriminator), while the PaymentSheet flow ALSO mints a **PaymentIntent** on the same order → **TWO capturable charge surfaces** (different idempotency keys; the webhook makes the order STATE idempotent but not the captures). The architect's "CreateOrder returns `StripeSessionId=null` for mobile" claim is **CONTRADICTED by the code**. **Pre-existing** — the live Android customer card flow uses `CreatePaymentIntent` too, so this is a latent double-capture surface in prod. Fix in **T-0347** (suppress the Checkout Session for the mobile PaymentSheet path — a host/channel discriminator); until then LIVE iOS card is BLOCKED (already owner-gated on the Stripe key regardless).
- **MEDIUM (iOS AC, Slice D):** CreateOrder has NO server idempotency → iOS MUST debounce/disable slide-to-confirm for the whole submit round-trip (single in-flight; TC: double-trigger → one order).
- **MEDIUM (iOS AC, Slice E):** PaymentSheet `.completed` is a UX signal ONLY — never flips order state; paid/Confirmed is read from the server (the webhook is the sole authority). `StripePaymentController` returns the typed result only.
- **LOW (iOS AC, Slice E):** never log/persist the `client_secret`/`ephemeralKey`/`stripeCustomerId` (S6) — straight to PaymentSheet, in-memory only.
- **LOW (iOS AC, Slice E):** empty publishable key → hide/disable card, no `STPAPIClient` init, no sheet, cash-only (fail-closed; TC).

---

### 7.17 T-0314 (Phase-8 — the customer TAIL: the last customer feature ticket; the four tab contents + every remaining customer surface) — acceptance scope + the cross-cutting rulings (recorded 2026-06-29, architect)

T-0314 is the **customer TAIL** — the **last** customer feature ticket and the **broadest** in the port (an `L → split`, on par with T-0300/T-0307/T-0313 for effort). It fills the four placeholder tabs T-0312 scaffolded (Home/Orders/Rewards/Profile) plus every customer surface that is neither the booking wizard (T-0313, `done`) nor auth (T-0312, `done`): Home, Orders/OrderDetail, Rewards/Loyalty/Referrals, Membership/Plus, Recurring, Disputes, Addresses, and Profile/Settings (incl. GDPR-delete, Devices, NotificationPreferences). `depends_on` T-0312 (shell+auth) + T-0313 (the booking + Stripe `PaymentSheetPresenting` seam) + T-0302 (codegen — the customer generated client is on disk for the business endpoints). Surfaced by the parallel **Android customer-tail survey** + the **backend tail-contracts/security survey** + the **cross-cutting iOS-mechanisms research**.

> **Every ruling APPLIES accepted ADRs + prior records — NO new ADR.** (1) the architecture is dead-consistent for a 1:1 port: every cluster = `@HiltViewModel`/VM + sealed `*UiState` (Loading/Error/Loaded — NEVER a flag-bag, the §7.7 D5 iOS-canonical shape) + shared `ActionState` for mutations + one-shot effects for nav/snackbar/PaymentSheet handoff; `@Singleton` repos returning `ApiResult<T>`, most `SessionScopedCache`. iOS mirrors this exactly (the §7.7 D5 born-sealed-state rule). (2) Membership card APPLIES the **T-0313 §7.16 D4** Stripe-seam ruling (sole importer), extended with a SetupIntent variant. (3) GDPR delete APPLIES **ADR-0016 AR-ACCT-1** + the **§7.14 D4** identity-token-only ruling (revoke not forced). (4) Dispute upload APPLIES the **T-0308 §7.10 D1** capture-seam + the generated-multipart contract. (5) Devices APPLIES **T-0310 §7.7 D6-8** verbatim. (6) the AddressManager APPLIES the **ADR-0021/§7.6 D6** map seam + the **§7.7 D3** service-area deferral. **A genuinely-new trade-off requiring a new ADR was looked for and NOT found** — every choice composes an accepted decision; the L→split + the four security-touching slices carry the rigor.

> **On-disk reality (verified):** the customer generated clients ALREADY exist for the tail — `CustomerMembershipAPI` (Subscribe → `{setupIntentClientSecret, stripeCustomerId, ephemeralKey}` / CreateCheckoutSession → `{checkoutUrl}`), `CustomerGdprAPI.gdprDeleteMyAccount`, `CustomerDisputeAPI.disputeUploadEvidence(disputeId:file:URL)` (the generated `FormDataEncoding` does REAL multipart — file from URL), plus Order/Loyalty/Referral/Recurring/SavedAddress/Device/NotificationPreferences. The T-0313 `PaymentSheetPresenting`/`StripePaymentController` seam + `StripeConfig.isCardPaymentAvailable` fail-closed gate are on disk. The local-wipe seam `AuthApiClient.signOutLocal()` (tokenStore.clear() + sessionScopedCaches.clearAll()) and the `CameraOrLibraryPicker` capture seam are on disk. The tab VMs/views are **net-new**.

#### Decision 1 — Scope vs deferrals: T-0314 ships ALL the customer-tail clusters; Apple Pay, the country-resolver (T-0334), the address-picker harvest (T-0349), and SIWA-revoke are DEFERRED (APPLIES the §7.15 D4 / §7.16 D1 deferral-map precedent; NO new ADR)

**RULING (CONFIRMED): T-0314 ships the four tab contents + every remaining customer surface — Home · Orders/OrderDetail · Rewards/Loyalty/Referrals · Membership/Plus · Recurring · Disputes · Addresses · Profile/Settings/GDPR-delete/Devices/NotificationPreferences. DEFERRED: Apple Pay (owner-gated, additive); the `LiveCountryResolver`/country-bias → T-0334; the address-picker→Core harvest → T-0349; SIWA `/auth/revoke` (not forced now, owner-gated); the in-app notifications FEED (the customer app has no Room feed — only prefs are in scope, the §7.7 Scope-B precedent).** A **scope record**, not a new trade-off.

The three T-0313-deferred fold-ins land in their enabling cluster: the **success→OrderDetail nav + lifecycle timeline** in Slice A (Orders now exists), the **saved-address CRUD** in Slice E (alongside AddressManager), and the **brand asset + Google-'G' fidelity** on the auth/profile surfaces (the §7.15-recorded T-0314 items).

#### Decision 2 — Membership/Plus payment: native PaymentSheet via the Subscribe SetupIntent (NOT web CreateCheckoutSession), reusing + extending the T-0313 Stripe seam (APPLIES §7.16 D4; NO new ADR)

**RULING (CONFIRMED): the iOS Plus flow uses `POST /api/Membership/Subscribe` with the native Stripe PaymentSheet in SetupIntent mode — NOT the web CreateCheckoutSession URL.** Phase 1 (`paymentMethodConfirmed=false`) → `{setupIntentClientSecret, stripeCustomerId, ephemeralKey}` → PaymentSheet; Phase 2 (`paymentMethodConfirmed=true`, **replaying the same idempotency token**) → `{membershipId}` → MembershipSuccess + re-read `membershipGetMine`. The T-0313 `PaymentSheetPresenting`/`StripePaymentController` seam gets ONE new SetupIntent initializer (~15 lines) — it stays the SOLE Stripe importer (§7.16 D4 / AR-PAY-1). **Fail-closed** on `StripeConfig.isCardPaymentAvailable` (two-gate: hide the CTA AND make the branch unreachable). **`.completed` is UX-only — the webhook is the SOLE active-state authority** (carry the §7.16 D4 + Gate-SEC rule); iOS re-reads `membershipGetMine`, never client-mutates the paid state. Swap = instant proration (no sheet); cancel = period-end, in-app reachable from Settings (sibling to delete). `CreateCheckoutSession` stays the documented web-only fallback (if ever used → `SFSafariViewController`, never a bare WKWebView). **WATCH-ITEM (AR-PAY-1):** Plus stays external-Stripe (NOT IAP) only as a real-world-service benefit — an owner/ASC framing item.

#### Decision 3 — GDPR account-deletion: in-app destructive confirm → gdprDeleteMyAccount() → signOutLocal() wipe → route→login; Apple /auth/revoke DEFERRED + owner-gated (APPLIES AR-ACCT-1 + §7.14 D4; NO new ADR)

**RULING (CONFIRMED): the delete flow = a Settings destructive-confirm row → `gdprDeleteMyAccount()` → the canonical local wipe `AuthApiClient.signOutLocal()` (NOT `logout()` — the account is gone) → route root→login.** Satisfies App Store 5.1.1(v) + AR-ACCT-1. **Signing in again must NOT resurrect the account** (the backend `GdprDeletionService` anonymize-not-resurrect guarantees this; own-account-only — parameterless command, userId from JWT, S1/S3). **The Apple `/auth/revoke` call is NOT needed now and is DEFERRED + owner-gated** — per §7.14 D4 (identity-token-only SIWA: the backend AppleAuth validates the identityToken via JWKS, never exchanges the authorizationCode, holds NO Apple token) + Apple TN3194 ('if you hold no token you must still delete the data and direct the user to manually revoke'). So NO `.p8`/Key ID/Team ID; **best-practice a localized ×5 'remove in Apple ID Settings' note** (a string, not a code dependency). The `.p8` + a backend revoke become mandatory ONLY if a future feature exchanges the authorizationCode — a recorded deferred open risk.

#### Decision 4 — Dispute evidence upload: the GENERATED multipart client + the CameraOrLibraryPicker seam + write-to-temp-file + client-side validation (APPLIES the T-0308 §7.10 D1 capture-seam + the generated-multipart contract; NO new ADR)

**RULING (CONFIRMED): the upload uses the generated `disputeUploadEvidence(disputeId:file:URL)` directly — NO raw-multipart spine path is needed (the generated `FormDataEncoding` does REAL multipart, unlike the T-0308 base64-BY-CONTRACT partner-photo path).** The signature takes `file: URL`, so the picked image is compressed + EXIF/GPS-stripped (reuse the T-0308 `ImageCompressor`) + written to a temp file (correct extension) before the call, cleaned up after. The picker REUSES `CameraOrLibraryPicker` (the partner `PhotosSection` pattern), with a `UIDocumentPicker`/PHPicker variant added for PDF evidence. **Client validation is fail-closed and mirrors the backend exactly:** size ≤ 10 MiB, type ∈ {image/jpeg, image/png, image/webp, application/pdf}, EXIF stripped, per-file sequential upload gated on the disputeId. Dispute PDFs view via the §7.12 QuickLook seam. **PURPOSE STRINGS (AR-PRIV-4, Gate-AR, IN THIS TICKET):** the customer target gets `NSCameraUsageDescription` + `NSPhotoLibraryUsageDescription` (×5) + the `PrivacyInfo.xcprivacy` photos entry if not already present.

#### Decision 5 — Customer Devices: REUSE the partner T-0310 D6-8 pattern verbatim (APPLIES §7.7 D6-8; NO new ADR)

**RULING (CONFIRMED): the customer Devices surface is the T-0310 D6-8 pattern byte-for-byte** — the same `Device/Mine` + `Device/{rowId}` revoke contract (the customer host shares it). D6: `currentDeviceId` from the ONE `DeviceIdProvider` (cz.cleansia.customer.device). D7: hide-revoke-on-current + the defensive self-revoke→sign-out branch (TC-IOS-DEVICES-SELF-REVOKE, red-first). D8: server-scoped (`GetByIdAndUserAsync`, cross-user → DeviceNotFound, NotFound-not-Forbidden, S3); the DTO leaks no foreign PII. iOS adds NO client ownership check and sends only a `deviceRowId` from its own `deviceMine()` response. SECURITY-owned (Gate-SEC slice F), same as §7.7.

#### Decision 6 — The slice plan (L → 6 clusters in dependency order; C/D/F SECURITY-touching → Gate-SEC in PARALLEL)

**RULING: 6 slices, grouped by feature cluster in dependency order.** **A** Home + Orders/OrderDetail (+ the T-0313 success→OrderDetail nav + lifecycle timeline; the 7-state OrderStatus enum mirrored incl. OnTheWay=3) — reads, Gate-DP. **B** Rewards/Loyalty/Referrals — reads, Gate-DP. **C (Gate-SEC)** Membership/Plus (native SetupIntent PaymentSheet, fail-closed, webhook-authoritative) + Recurring (+ the deferred ConfirmRecurring PaymentIntent branch on OrderDetail). **D (Gate-SEC)** Disputes + the multipart evidence upload. **E** Addresses (3-pane on the map seam + saved-address CRUD; country-bias deferred → T-0334; picker harvest → T-0349) — Gate-DP. **F (Gate-SEC)** Profile/Settings + GDPR-delete + Devices + NotificationPreferences. Ordering: A→B (the read surfaces prove the canonical VM/UiState shape) → C (money) → D (upload) → E (map) → F (the security-dense settings tail). This record fixes the architectural seams; the security enforcement is the security charter's Gate-SEC on C/D/F.

#### New CRC roles (added with the T-0314 wiring)

- **`ios-customer-tab-vm`** (one per tab/cluster) — a `@HiltViewModel`-parity VM: *responsibility:* observe its singleton repo(s), expose a sealed `UiState`/`ActionState`, emit one-shot effects (nav/snackbar/PaymentSheet handoff). *Does NOT know:* navigation (effects only), the Keychain/token, or how the audience root switches. **If a tab VM navigates directly or carries a flag-bag UiState, the responsibility is wrong** (the §7.7 D5 born-sealed-state rule).
- **`ios-delete-account-vm`** — *responsibility:* present the destructive confirm, call `gdprDeleteMyAccount()`, hand off to `signOutLocal()`, emit `accountDeleted`. *Does NOT know:* the Keychain internals, how the backend anonymizes, or the route reset (the root reacts to the session going empty). **If it calls `logout()` post-delete, or trusts a client flag to confirm deletion, the responsibility is wrong.**
- (The membership flow rides the existing `ios-stripe-payment-controller` (§7.16) extended with a SetupIntent path; the dispute upload rides the existing capture seam; Devices rides the §7.7 D6-8 SECURITY-owned device contract — no new role.)

#### Open question for the owner (non-blocking)

- **None that blocks T-0314's code.** The LIVE-card gate is the **Stripe publishable key** (already provisioned in T-0313). Apple Pay is deferred/additive. The `idempotencyToken` field needs the owner's customer-mobile NSwag regen (the iOS client already carries it; the Android adapter has it commented out). SIWA `/auth/revoke` is NOT needed (the identity-token-only design). The minor backend consistency gap (NotificationPreferences lacks `[EnableRateLimiting("auth")]`) is a small PM follow-up, not an iOS blocker.
#### Security design gate (Gate-SEC, T-0314) — PASS-WITH-REQUIREMENTS → the per-cluster ACs land on the security-touching slices C/D/F

**GDPR delete verdict: PASS** — genuine anonymize-not-resurrect deletion (User.Anonymize nulls Google/Apple/password + rewrites email; satellite rows hard-deleted), own-account-only (parameterless, userId from JWT), `signOutLocal()`-not-`logout()`, and the SIWA-revoke **correctly DEFERRED** (the §7.14 D4 identity-token-only design holds no Apple token → Apple TN3194 = deletion-of-data + the localized "remove in Settings → Apple ID" note satisfies 5.1.1(v)). No new exploitable breach; every security-touching cluster composes an already-verified backend + an accepted iOS seam.

**Binding per-cluster requirements (the per-slice Gate-SEC enforces; red-first where noted):**
- **GDPR (Slice F):** in-app destructive confirm → `gdprDeleteMyAccount` → on SUCCESS `signOutLocal()` (Keychain clear + `sessionScopedCaches.clearAll()` + the push-unregister hook) → route→login; NEVER `logout()`. The **no-resurrect** test (sign-in→delete→sign-in = a fresh account). The localized "remove Cleansia in Settings → Apple ID → Sign in with Apple" note ×5. **The VM MUST branch on the ApiResult** — deletion is BLOCKED mid-transaction (in-flight order / pending invoice → `GdprDeletionBlockedByOrder/ByInvoice/AlreadyPending`); on failure show the localized error + STAY signed in (do NOT wipe). No client-side delete logic / no client flag.
- **Membership (Slice C):** PaymentSheet `.completed` (SetupIntent) is UX-only → re-read `membershipGetMine`; the webhook is the sole paid authority (never client-mutate active). Fail-closed on an empty publishable key (hide the CTA + unreachable branch, reuse `StripeConfig`). Never log/persist `setupIntentClientSecret`/`ephemeralKey`/`stripeCustomerId`. Replay ONE `idempotencyToken` across Phase-1/Phase-2. swap/cancel/getmine own-membership-only; cancel in-app reachable.
- **Disputes (Slice D):** own-dispute scoping is server-enforced in the handler (`DisputeNotOwnedByUser`); client file validation fail-closed + mirrors the backend (size ≤ 10 MiB AFTER compression; type image/jpeg|png|webp + application/pdf); the blob name is server-controlled (no path traversal); no secret in multipart; the generated `disputeUploadEvidence(file:URL)` (the generated URLSession does real multipart) + EXIF-strip + temp-file.
- **Devices (Slice F):** the T-0310 D6-8 rules VERBATIM (the ONE `DeviceIdProvider`; hide-revoke-on-current; the self-revoke→sign-out branch).
- **Notification-prefs (F) + Saved-address (E):** own-only by construction/validator (VERIFIED) — no client ownership check needed.

**Findings:** **LOW (backend, not an iOS blocker) → T-0350** — NotificationPreferences GetMine/Update lack `[EnableRateLimiting("auth")]` (own-prefs-only, so no leak; an un-throttled per-user write). **INFO** — the membership Phase-1 `stripeCustomerId` is in-flight only (no-log). The membership `idempotencyToken` is fully cross-platform only after the owner's customer-mobile NSwag regen (the iOS generated client already carries the field; until then the backend reconcile collapses double-taps — acceptable).
