...
manual_steps: []
sprint: 6
source: T-0188 security-gate note 1; recorded in agents/archive/2026-08/backlog/security/auth-sessions.md (Wave-3 close)
---

## Context
Latent multi-tenant gap found by the T-0188 security gate (device/session management, merged
`5d631f8c`/`8ddfef9d`), recorded in `agents/archive/2026-08/backlog/security/auth-sessions.md`: refresh tokens are
...
- 2026-06-14 — **review** (backend, test-first). Lane Auth-token.
  - **AC1 (contract-lock — architect call):** chose **read-side `IgnoreQueryFilters()` with a
    ...
    `agents/archive/2026-08/backlog/security/auth-sessions.md` (2026-06-14 entry). Mirrors the T-0245 fix shape and the
