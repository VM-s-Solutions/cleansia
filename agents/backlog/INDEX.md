...
| T-0662 | Angular framework packages sit at 19.2.14 while patches exist to 19.2.25, and `~19.2.0` already admits them — clears ~18 runtime highs. They must move as a set and an incremental `npm install` hits a peer-resolution conflict, so this wants an `ng update` on a clean tree with a full frontend verification behind it | M | `todo` | — | — |
| T-0663 | Seven runtime advisories have **no 19.x patch at all** — i18n XSS via event-handler attributes, hydration DOM clobbering, SSR raw-content escape, HttpTransferCache key ambiguity. Angular 19 is past active support, so the fix is an Angular 20 upgrade. Owner decision: real risk, real cost, and no third option | L | `blocked` | — | — |
...
| T-0665 | Four commands return `ICommand<bool>` whose every success path is `BusinessResult.Success(true)` — a payload that cannot be false, since failures use the error channel. `check-consistency` B1 is right, and its own comment names the cure: bare `ICommand`. But the wire body changes for Logout / Register / RegisterEmployee / ResendConfirmationEmail across three web apps and four mobile apps, and needs `manual_step: nswag-regen`. HandlePaymentNotification's `string` is a Stripe webhook body nothing reads. Owner decision before any client breaks | M | `blocked` | — | — |
