...
> | **T-0403** | **iOS Firebase Messaging / FCM-token integration** — the backend dispatches via FCM (needs FCM registration tokens) but the iOS apps have NO Firebase SDK and register a RAW APNs token FCM can't target, so iOS push cannot deliver even with the APNs key uploaded. Add FirebaseCore+Messaging (both apps) + GoogleService-Info.plist, register the FCM token instead of raw APNs, and fold the T-0398 customer push wiring. Depends on T-0342 (owner APNs key). **HIGH** | M | **proposed** (high) | T-0342 | ios | no | owner: add iOS apps to Firebase + GoogleService-Info.plist | phase/ios-fix3 push-enablement investigation |
...
> | **T-0413** | **ResolveDispute refund push lacks orderNumber** — the alert body renders with an empty arg (tolerated by design). `DisputeRepository.GetForUpdateAsync` has no `.Include(d => d.Order)`, so the producer can't read `DisplayOrderNumber` without a repo/query change | S | **proposed** (low) | — | backend, db | no | — | T-0404 backend lane skip note |
> | **T-0414** | **Self-revoke should sign out immediately** — deleting YOUR OWN device from the Devices page leaves a zombie session until the ≤30-min TTL bites (server side is correct; the client ignores what it just did). Compare revoked deviceId to the local one → on match run local sign-out + route to login; badge "This device" + confirm copy. Both platforms, both apps | S | **proposed** (medium) | — | ios, android | no | — | owner self-revoke report; companion logout-hang fix shipped separately |
>
