...
> | **T-0339** | **SECURITY (backend)** — scope `GetPagedOrders` "mine" views to the JWT caller (client `Filter.EmployeeId` over-read leaks foreign-assigned order coords/codes/pay). Reachable, MEDIUM; pre-existing | S | **in-review** — landed in the Phase-4 PR `d688d30` (reviewer APPROVE + security PASS, closes D2b); pending backend-ci | — | backend | **yes** | — | T-0307 security gate §7.8 (`security/ios-orders.md` D2b) |
...
> | **T-0342** | **OWNER TASK** — iOS APNs auth key (`.p8`) in the backend push provider + Push Notifications capability/provisioning on the App ID (gates end-to-end push DELIVERY; T-0311 ships the code + the `aps-environment` entitlement without it) | S | **proposed** (owner) | — | ios | no | **owner: APNs key + Push capability** | T-0311 §3B |
>
