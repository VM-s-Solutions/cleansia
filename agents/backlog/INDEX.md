...
| T-0666 | **Dependabot has never been allowed to fix anything.** `repos/.../automated-security-fixes` reads `false` and there is no `.github/dependabot.yml`, so alerts accumulate and no update PR is ever opened — which is how 165 piled up. Enabling it is one API call or one settings toggle, but every PR it opens spends GitHub Actions minutes and the owner is already against a quota, so this is a decision about that budget rather than a defect to fix | S | `blocked` | — | — |
