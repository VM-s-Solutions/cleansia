...
| **T-0588** | **19 money-coercion sites remain, in the repositories the T-0582 sweep did not reach** — `ReferralApi` (9), `DisputeApi` (5), `RecurringBookingApi` (2), `SavedAddressApi`, `PromoCodeApi`, `NotificationPreferencesApi` (1 each). None is partially mapped: the tell is the return type, still a generated `*Dto` | M | **ready** — same five rules and the same wire-pin shape as `af288efa`/`02a02957`/`f2273bab`. **The rollup ruling is per surface and must be made per surface**, not inherited: refuse where the list IS the addends, drop the row where a total is supplied independently | — | android | no | — | T-0582 residual, 2026-08-10 |
| **T-0589** | **The two apps refuse broken money differently, and one loses the field name.** Partner refuses through `WireContract.required()` → `ApiError.Server`, so the offending field reaches triage; customer refuses through its native `?: return null` → a 200 with a null body, which does not | S | **ready** — each app used its own idiom deliberately rather than duplicating `WireContract` into `customer-app` or moving it into the serialized `:core` lane mid-ticket. The decision owed is **which idiom wins**, and it is worth making before the iOS port copies one: a refusal nobody can attribute is a support ticket that ends in "reinstall the app" | — | android, ios | no | — | T-0582 divergence, 2026-08-10 |
