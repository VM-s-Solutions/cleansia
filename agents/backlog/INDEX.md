...
> |----|----|----|
> | **T-0531** | **RESCOPED** — `ready`, `S` → **`XS`**, **AC1′ only** (the *activation* consequence into `multi-tenancy-and-region.md`). AC2/AC3/AC4 withdrawn — landed elsewhere, or withdrawn on principle/decay | that file still contains no "unique index" / "NULLS" / "DISTINCT". **AC5 stands: nothing is fixed, no migration, no index change** |
> | **T-0532** | **STANDS**, `ready`, unblocked, three AC sharper | `PromoCodeRedemptionRepository.cs:22` **still** carries `// INTERIM(ADR-0038 §D3 → T-0532)` — live interim, non-orphan marker. **Now the highest-value unshipped `ready` ticket in the queue** |
> | **T-0471** | **STANDS UNCHANGED**, `ready`, `S` | ADR-0033 `:3` still `proposed`; `adr/challenges/` still has **no** 0033 file while 0034–0042 all got one. **Now the oldest `ready` ticket in the queue** |
>
