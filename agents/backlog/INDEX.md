...
| T-0636 | Partner DEV API 500 from /health — **root cause found 2026-08-28**. `System.BadImageFormatException`: *"Illegal or unimplemented ELEM_TYPE in signature. The format of the file '/home/site/wwwroot/Cleansia.Infra.Database.dll' is invalid."*, thrown from `Microsoft.Extensions.DependencyInjection` → `ResolveService`, 2026-08-25 18:43:57 UTC on `api-cleansia-partner-weu-dev`. **Not a code defect**: `azure/webapps-deploy@v3` extracts the zip file-by-file over `/home/site/wwwroot`, `WEBSITE_RUN_FROM_PACKAGE` is set nowhere in `deploy/` or `.github/`, and dev has no staging slot (`slot-name` is `production` unless env is prod, and a B-series plan cannot host slots) — so the app restart raced the extraction and DI loaded a half-written assembly. It clears when extraction completes, which is the "4 minutes" the original report measured. Prod is immune by construction: it lands in the staging slot, warms it, proves /health, then swaps. Fix is `WEBSITE_RUN_FROM_PACKAGE=1` on the app hosts — App Service then mounts the package read-only instead of overwriting files in place, so a partial DLL cannot exist | S | `todo` | — | — |
