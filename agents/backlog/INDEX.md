...
| T-0636 | Partner DEV API 500 from /health — **cause identified 2026-08-28**: `System.BadImageFormatException` ("Illegal or unimplemented ELEM_TYPE in signature") at `Global.ResolveService`, twice at 2026-08-25 18:43:57.749 / .768 UTC under one operation_Id. That is a CLR metadata/signature parse failure — an assembly-load problem, not a database or cold-start one, which is exactly why it answered **500** rather than the **503** a failed `AddDbContextCheck` produces (both ordinary 503s sit in the same window: partner 08-23 21:22, admin 08-24 22:27). Grounded: `ResolveService` appears nowhere in `src/`, and the codeless App Insights agent is off by design (main.bicep:725), so it is neither our code nor profiler IL rewriting. `traces` for the window is empty, which is expected — RequestLoggingMiddleware filters /health. Needs the stack's assembly to name the culprit | S | `todo` | — | — |
