...
> | **T-0427** | **iOS Live Activity for an in-progress clean** — Wolt/Uber-style lock-screen + Dynamic Island live status. Real feature: a Widget Extension + ActivityKit (16.1) client, AND a SEPARATE APNs Live-Activity push channel (distinct from the T-0403/04 FCM alert path — likely direct-APNs, FCM can't target activity tokens) driven by order-status transitions + a per-order activity-token registration. ADR-gated | L | **ADR drafted — awaiting owner ratification** (ADR-0029 iOS Live Activity, `draft`: D1 direct-APNs token-auth client behind ILiveActivityPushClient (the T-0342 .p8 serves both channels — but needs delivering as backend secrets, a NEW step); D2 OnTheWay=start / InProgress=update / Completed=end+30min linger / Cancelled=dismiss, on the existing status seam via a new `live-activity-dispatch` queue + sibling consumer (FCM path byte-unchanged); D3 NEW `LiveActivityToken` entity (Device-overload rejected on cardinality); D4 customer-only, no names in payload. **Owner gates:** .p8 second custody location, the timing/floor product choices, the v1 exclusions. Ships inert until T-0342) | T-0403, T-0404 | architect, ios, backend | no | — | owner remark + remarks-sweep |
...
> | **T-0429** | **iOS partner — D2 single-NavigationStack/ShellRoute topology on the STOCK bar** — the surviving half of retired T-0376 (owner approved the re-scope 2026-07-17): the ADR-0022 supersede killed the pill/pager but only retired the D2 topology by association; the partner still carries the crash-fix interim per-tab NavigationPath stacks. Converge on ONE shell-level stack + merged ShellRoute UNDER the stock TabView; deep links + the cold-launch buffered-tap path must survive; architect signs the ADR status-log entry (AC4) | M | **proposed** (medium) | — | ios, architect | no | — | re-scope of T-0376, owner-approved |
> | **T-0430** | **Partner apps — notifications feed UI** — the T-0393 v1 backend ships the partner-host endpoint so rows accumulate from day one; this lands the partner UIs: Android partner migrates its Room feed to the server inbox (delete Room after parity), partner iOS gains the inbox (mirror the customer feed UI). Badge + watermarked mark-all per FD-AC5/6 | M | **proposed** (medium) | T-0393 | android, ios | no | — | T-0393 panel rebuttal condition |
> | **T-0431** | **Partner-targeted notification events** — feed v1 ships partner = order.new_available only (the only dispatch that exists). Add accepted-job cancellation (highest impact — a cleaner learns nothing today when their accepted job is cancelled), assignment confirmation, invoice-ready: catalog key + producer + loc-keys ×5 + feed rows via the shared notify seam; analyst keyset/copy first | M | **proposed** (medium) | T-0393 | analyst, backend, android, ios | no | — | T-0393 Q-FEED-02 default |
>
