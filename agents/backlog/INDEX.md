...
> | **T-0427** | **iOS Live Activity for an in-progress clean** — Wolt/Uber-style lock-screen + Dynamic Island live status. Real feature: a Widget Extension + ActivityKit (16.1) client, AND a SEPARATE APNs Live-Activity push channel (distinct from the T-0403/04 FCM alert path — likely direct-APNs, FCM can't target activity tokens) driven by order-status transitions + a per-order activity-token registration. ADR-gated | L | **architecture hardened + implementation plan ready — awaiting owner ADR ratification** (ADR-0029 → `proposed (ratification-ready)` 2026-07-19, re-verified vs the shipped code: producer seam reshaped to a sibling `ILiveActivityProducer` beside the INotificationProducer consolidation (+ its own tripwire); AdminOverrideOrderStatus corrections (forward-only, Cancelled unreachable, gains its FIRST producer call); cancel end-push pinned outside the refund conditional; poison pair added; exact `APNS:*` config keys pinned. **The .p8 gate is RESOLVED by conduct** — key uploaded, in backend secret custody, ordinary push confirmed working on live dev; same team key signs `liveactivity`, residual = APNS:* binding. **Still OPEN for owner:** Q2 timing defaults (OnTheWay start / 30-min linger / instant cancel dismiss / 17.2 remote-start floor) + Q3 v1 exclusions — recommended defaults recorded. Slices LA-1…LA-5 specced in the ticket (entity+endpoint / queue+producer seam / direct-APNs client / consumer+poison+janitor / widget extension+ActivityKit); MANUAL_STEPs: owner-applied customer project.yml widget-target diff (owner-modified working tree), APNS:* binding, migration fold + spec regen. Backend ships inert `APNS:Enabled=false` ahead of the iOS lane) | T-0403, T-0404 | architect, ios, backend | no | — | owner remark + remarks-sweep |
...
> | ID | Title | Size | Status | depends_on | Layers | sec | manual_step | Source |
> |----|-------|------|--------|-----------|--------|-----|-------------|--------|
> | **T-0334** | iOS `ServiceAreaProvider` Core seam + the advisory `ServiceAreaRow` (+ forward-geocode country-bias) | M | **in_progress** — the endpoint-free half **shipped** on `feature/payroll-invoice-paid-notify` (countries-only `ServiceAreaProvider` actor in Core — lazy-cached, failure-never-cached, single-flight, `refresh()` — + per-app `ServiceAreaDataSource`s on the ADR-0011 `ApiResult` contract; the partner `AddressSectionViewModel` moved off direct `getServicedCountries` onto the provider with the advisory tri-state + degrade-to-Unknown law preserved; **AC3 forward-geocode country-bias LIVE** — `servicedCountryCodesProvider` wired at BOTH picker construction sites, partner + customer). **Gated remainder:** the AC2 city-level 3-state `ServiceAreaRow` + the cities half of the provider need the serviced-cities endpoint the partner mobile spec doesn't expose → owner spec+client regen | T-0310✓, T-0306✓ | ios | no | **owner: partner-mobile spec+client regen (serviced-cities) for the AC2 row** | sprint-12 §7.7 D3 (architect) |
...
> | **T-0320** | Dev smoke + verification (5 APIs + SSR + 2 SPAs + Functions; queue→Functions live) — **needs the env up** | M | **in_progress** (reachable smoke run 2026-07-19: 5 APIs + SSR green, DB round-trip green; **Functions root 503**, SWA URLs + AC2 outstanding — see ticket) | 2 | T-0318✓, T-0319✓ | infra, backend, qa | no | — |
