...
| T-0669 | Home: one title system (headings `Sky700`), neutral shadows, hero bubbles + wave + uniform scroll-fade removed | M | `in_progress` | — | — |
...
| T-0676 | Promo e-mail rendered from `email-templates/` rather than a hosted SendGrid template — the folder becomes the runtime source, so repo and sent HTML cannot drift | M | `blocked` | — | AC1–AC5 shipped; AC6 waits on the owner-run NSwag regen |
| T-0677 | Migrate the six existing e-mails off hosted templates and delete the six `*TemplateId` settings | M | `ready` | — | — |
