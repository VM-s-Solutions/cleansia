...
> | **T-0431** | **Partner-targeted notification events** — feed v1 ships partner = order.new_available only (the only dispatch that exists). Add accepted-job cancellation (highest impact — a cleaner learns nothing today when their accepted job is cancelled), assignment confirmation, invoice-ready: catalog key + producer + loc-keys ×5 + feed rows via the shared notify seam; analyst keyset/copy first | M | **partly done ✅ — `order.assignment_cancelled` shipped; `payroll.invoice_ready` deferred to owner** on `feature/i18n-cluster-3` (the headline gap closed: a cleaner is now told when a job they ACCEPTED is cancelled — shared notifier in CancelOrder + AdminCancelOrder gated on assigned crew, non-mutable, distinct partner key; push loc-keys ×5 in all 4 catalogs client-first, partner render+deep-link both apps; 13-event pins, 1979/1979 + mobile builds green). invoice_ready held on owner OPEN-2/3 (push-vs-feed, generation-vs-paid trigger) + a missing partner invoices deep-link target) | T-0393 | analyst, backend, android, ios | no | — | T-0393 Q-FEED-02 default |
>
