...
| T-0668 | Nx 21.6.11 → 23.1.2 — two `nx` advisories remain open (high < 22.7.7, medium < 22.7.2). Dev-scope: the CLI, not shipped code. Dependabot's #244 proposed it as a drive-by bot merge; the transitive `brace-expansion` half is already fixed by an override, so what is left is a genuine two-major migration of the build tooling and wants the verified treatment Angular 20 got. Angular 22 (#241) is deliberately NOT filed alongside it: no `@angular/*` advisory is open at all after T-0667 | L | `todo` | — | — |
