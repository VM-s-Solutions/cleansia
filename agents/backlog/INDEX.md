...
| **T-0598** | **The iOS customer app coerces the price the customer agrees to pay.** `BookingCodeStates.swift:48-58` — nine `?? 0` across `totalPrice`, `originalSubtotal`, `servicesSubtotal`, `packagesSubtotal`, `extrasSubtotal`, both discounts and `expressSurchargeAmount`. Plus `OrderDetailSummary.swift:66-67,119-120`, `OrdersTab.swift:196`, `OrderDetailDetailsCards.swift:18,122`, `MembershipPerks.swift:26-28,127`, `SubscribePlusScreen.swift:192-193`, `OrderDetailViewModel.swift:218-220` | M | **ready** — the customer half of T-0596, which shipped the partner half and the catalog. **The quote is the highest-value coercion left in the codebase**: a broken wire field renders the booking total as **0 Kč** on the screen where the customer commits. Likely §D2 *refuse* — the breakdown lines are the addends of a total the same screen prints — but **every row needs its own ruling against its own screen**, written into the mapper's doc comment. Two already framed by the T-0596 lane: the list total is per-row rather than a rollup, so it is *refuse the row's own money* with no page question; and `upgradesRemaining ?? 0` currently reads a broken wire as **"quota exhausted"**, which silently withdraws a benefit the member paid for. ⚠️ `LoyaltyClient`, `RewardsReferralClient`, `DisputeClient`, `SavedAddressClient`, `RecurringBookingClient`, `NotificationPreferences` and `UserProfileClient` were **never read line-by-line** — T-0596's sweep was scoped to money, quantity and identity | — | ios | no | — | T-0596 residual, 2026-08-11 |
| **T-0599** | **Delete iOS's now-redundant `isUpcoming` conjunct — but ONLY after the backend is DEPLOYED, not merged** | XS | **blocked-by: ADR-0049 backend DEPLOYED to the target environment.** ⚠️ **The constraint is asymmetric and that is the whole ticket**: a shipped iOS binary cannot be redeployed while the backend can, so deleting early reopens the defect for an App Store review window plus the update tail. **The live risk is the CARRIER, not the deletion.** `PreferredOfferPresentation.swift:16-19` today argues the *opposite* — *"the narrowing is made here"* — with no ADR reference and no precondition, and a lane deleting a conjunct reads the file, not the ADR. So the constraint must be carried by **both** the Swift doc comment and this row. Until both exist, deleting the conjunct is a **review finding** | — | ios | no | — | ADR-0049 §D6 + C4, 2026-08-11 |
| **T-0600** | **Three shipped doc comments carry a premise ADR-0047 got wrong** — `OrderDetail.swift:133`, `OrderDisclosurePresentation.kt:21`, `OrderDetailRedactionGateTests.swift:17` all repeat *"the server redacts to `string.Empty` and `[]`"* | XS | **ready** — the **code is correct and the reason is false**, which is the more dangerous half: the next reader derives a `!= ""` test from it. `OrderPiiRedaction.cs:40-53` **nulls** seven fields — `AccessInstructions`, `Notes`, `SpecialInstructions`, `CompletionNotes`, `Address`, `Review`, `PreferredOffer` — and `:30-31` nulls the coordinates, *inside the very line ranges the ADR cited*. Only string scalars go to `""`. So the roster spans **both** forms and neither `!= null` nor `!= ""` alone covers it | — | android, ios | no | — | ADR-0047 ratification A2, 2026-08-11 |
| **T-0601** | **A new `OrderStatus` member does not force a disclosure decision** | S | **ready** — measured, not suspected: I added a temporary `MutationProbe = 7` to the enum and ran both suites. `ErasureBlockingOrderStatusTests` **reddened** — it states its membership positively, so a new status forces a ruling. `PreferredOfferDisclosureTests` **stayed green**: `IsDisclosable`'s limb (a) is a negative test (`not (Completed or Cancelled)`), so a new status silently defaults to *disclosable*. That is the safe direction for this rule and the wrong shape for a gate — the point of the erasure pin is that agreement is a decision **re-made on every change**, and this one is inherited | — | backend | no | — | ADR-0049 C1(v) probe, run 2026-08-11 |
| **T-0602** | **ADR-0048's tier cannot claim `T1-CI` yet — six repositories are unmigrated and one is half-migrated** | M | **ready** — the ratification pass **refused to flip this token**, and refusing was correct: the claim would simply not have been true. `MembershipApi`'s cancel and swap responses still fabricate `membershipId = ""`, and the six customer repositories T-0588 swept still carry no wire test of their own, so §D3's roster is a **closed list** that a seventh repository joins by nobody noticing. Close the roster, then flip the token — in that order, per `conventions.md` §*"add enforcement behind the cleanup, never in front of it"* | — | android | no | — | ADR-0048 ratification, 2026-08-11 |
