...
## Done when
- [ ] `GoogleTokenVerifier` extracts `email_verified`; `GoogleAuth` provisions/links only on a verified email.
- [ ] An unverified-email Google token is rejected (no provision); the test proves it.
- [ ] Both providers' auto-provision/link rest on a verified email (parity).
