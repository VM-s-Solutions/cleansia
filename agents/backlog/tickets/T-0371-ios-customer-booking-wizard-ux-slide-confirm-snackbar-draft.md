...
## Acceptance criteria
- [ ] **AC1 (real slide control, hoisted)** — `SlideToConfirm` lives in `CleansiaCore` (single
...
- [ ] **AC2 (in-sheet snackbar host)** — Per ADR-0021's modal-sheet pattern, `.snackbarHost(snackbar)` is
...
- [ ] **AC3 (profileIncomplete parity)** — `.profileIncomplete` no longer snackbars: `BookingSheetView` gains
...
- [ ] **AC4 (draft survives)** — `BookingViewModel` is hoisted to the presenting container
...
- [ ] **AC5 (non-regression)** — Core + Customer + Partner suites green (the partner repoint is
