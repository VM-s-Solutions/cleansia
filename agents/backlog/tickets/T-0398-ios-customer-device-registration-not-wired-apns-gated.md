...
- 2026-07-17 — AC2 implemented by ios (with the backend token-less `RegisterDevice` half in the same tree): the shared Core `PushSessionObserver` now maps a live session with no token to the canonical token-less register `""` (`session ? (token ?? "") : nil`), so the install self-registers at session start and after fresh login even before push permission/APNs provisioning, and a later real token re-registers to upgrade the row. Both apps inherit it (the observer is shared Core — a customer-only fork would duplicate Core). Tests: 5 new `PushSessionObserverTests` (session-start token-less register, fresh-login register, token upgrade, signed-out no-register, logout no-register) + `PushTokenForwarderTests` sequence expectations updated to the new contract; Core 328/328 green, customer 527/529 (the 2 known local Stripe-key artifacts). Harvest: the token-less divergence folded into `patterns-mobile.md` push Lifecycle (b). AC3 remains owner-gated (T-0342 provisioning).
- 2026-07-19 — **frontmatter reconciled to reality (proposed → done)** — code shipped on feature/i18n-cluster-3 (merged); on-device verification remains APNs-gated (T-0342 owner).
