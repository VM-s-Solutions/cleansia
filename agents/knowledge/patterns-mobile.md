...
> **Brand-mark geometry — the ONE way (T-0443).** The iOS 1024 masters are the source; Android reproduces them as **vectors**, never as an imported PNG. Two platform constraints decide the numbers and neither is optional: (a) an iOS icon is authored full-bleed for a fixed rounded-rect mask, so a naive import is sheared by a **circle**-mask launcher — the Android mark is scaled to the iOS mark's fraction of the icon (measured from the master, e.g. 822/1024 = 80.3%) **of the inner 72dp**, which puts its furthest ink corner ~29dp from centre, inside the **66dp (radius 33) safe circle**; (b) the background gradient runs **(18,18)→(90,90)**, not corner-to-corner, because the launcher crops 18dp of bleed off every edge — a 0→108 gradient shows only its middle third and reads flat. The **notification small icon is the exception that is never a straight copy**: Android keeps only a small icon's alpha, so it must be a flat single-fill silhouette (any colour or gradient flattens to a white blob), and an 8-letter wordmark is illegible at 24dp — both apps therefore ship the same Poppins-Bold **"C" monogram** filling 20 of the 24dp canvas. `BrandIconCatalogTest` (`:core`) pins all of it, because every one of these failures is silent at build time.
...
(reviewer #27). **Current-location/the my-location FAB SHIPPED in T-0335** (T-0325's
`NSLocationWhenInUseUsageDescription` landed ×5 first): the Core **`LocationProvider`** protocol
(`CleansiaCore/Location` — `authorizationStatus`/`requestWhenInUseAuthorization() async`/`currentLocation()
async -> Coordinate?` best-effort-nil; default `CLLocationManagerLocationProvider`, the ONLY CoreLocation
consumer besides `CLGeocoderGeocodingService`; injected via the Core `\.locationProvider` environment key, DEBUG
`PreviewLocationProvider`) + the FAB/auto-center flows as **`AddressPickerViewModel.autoCenterOnOpen`/
`recenterOnMyLocation`** with a one-shot `locationFailed` (.denied/.unavailable) the views map to app-local
snackbar strings. **Already-denied/restricted on OPEN stays silent** on the Prague default (iOS never re-prompts
— no nag loop); a fresh prompt answered denied and every explicit FAB tap DO report. The picker still centers on
the **Prague default** until a fix arrives; **pan-to-place + search at full parity**. **No Mapbox token, no map SDK, no
