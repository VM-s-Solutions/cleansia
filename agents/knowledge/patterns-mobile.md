...
**iOS shell navigation — the ONE way (ADR-0022, 2026-07-02):** inside the signed-in audience there is exactly
**ONE `NavigationStack`, at the SHELL level**, over **one** typed route enum whose values are held in a
**type-erased `NavigationPath`** (never `@Published var path: [SomeRoute]` — homogeneous typed sibling paths,
multi-element sets, and `navigationDestination(isPresented:)` mixing are all documented iOS-16 crash/glitch
sources on the ADR-0014 floor). The tab ROOTS (and only they) live in a `TabView(selection:)` with
`.tabViewStyle(.page(indexDisplayMode: .never))` (Android `HorizontalPager` swipe parity); the custom
pill-bar/FAB composite mounts via `.safeAreaInset(edge: .bottom)` (the clearance contract — no ad-hoc bottom
spacers, no `.overlay` + fixed-offset FAB); pushed children cover the whole shell — bar hidden on push (the
Android NavHost-above-shell parity). **Deviations a reviewer rejects (#35):** a `NavigationStack` nested inside
another — in particular a root view wrapping its audience `switch` in a stack (the `CustomerRootView.swift:17` /
`PartnerRootView.swift:17` crash class); per-tab sibling `NavigationStack`s bound to typed `[Route]` arrays; a
shell bar built as a stock `TabView`/`.tabItem` bar (the pill+FAB is BRANDING per ADR-0018/ADR-0022, not a
component swap). *Partner interim (recorded in ADR-0022 D4): per-tab stacks over `NavigationPath` remain
sanctioned until the PM-filed partner pill/pager parity ticket migrates the partner shell to this shape.*
