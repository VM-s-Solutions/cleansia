...
> Introduced by **ADR-0034** (`proposed`). A domain service in `Cleansia.Core.AppServices.Services`,
...
modulo-11 on a domestic prefix/account) — and **failing closed** when it cannot decide.
...
- **The backfill script (T-0518)** — runs this *same* validator over each legacy `Employee.IBAN` value.
  That is the backfill principle: *anything a fresh write would pass, we accept.*
...
- Tests assert: `"totally not an iban!!"` rejected · valid-structure/bad-check-digit IBAN rejected ·
  ...
  valid IBAN for an **unconfigured** country **accepted**.
- The CZ/SK modulo-11 weight vector and the CZ/SK IBAN composition carry a **cited primary source** in a
  code comment, with a test-vector table (ADR-0034's honesty note — these are reads until T-0519
  verifies them, and no agent asserts a banking legal requirement).
