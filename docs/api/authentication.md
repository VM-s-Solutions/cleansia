...
Certain partner-facing endpoints are additionally protected by the `[RequireCompleteProfile]` action filter. This filter checks whether the authenticated employee has completed their registration (profile, at least one active document, and admin approval — `ContractStatus` `Approved` or `Active`). If the employee's profile is incomplete, the filter short-circuits the request and returns **403 Forbidden**.
