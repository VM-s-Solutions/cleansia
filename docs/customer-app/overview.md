...
| Checkout | Stripe card payments, or cash for a signed-in customer's one-cleaner booking ([the cash rule](/product/business-rules#cash)); `/checkout/success` and `/checkout/cancel` are the two URLs Stripe returns to |
| Order tracking | `/track-order` — the page a booking's e-mail link lands on. Anonymous, keyed on the `?token=` access token alone; there is no lookup form. It also lists the bookings this browser still holds tokens for, and cancels one → [Order tracking](/customer-app/order-tracking) |
...
| Recurring bookings | `/membership/recurring`, `…/create`, `…/:id` — a member's schedules (create and edit gated by `customerMembershipGuard`; list, pause and delete are not). The wizard starts on card and offers cash only when the quote says one cleaner; a cash schedule the server now skips is badged *Needs a change*, with the fix → [Recurring bookings](/flows/booking-and-pricing#recurring-bookings) |
