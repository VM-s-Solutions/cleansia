---
title: ADR-0001 — Authorization model
adr: ADR-0001
status: accepted
---
...
> ### ⚠️ Correction banner — 2026-09-08 (T-0689): the Feature Flag policy rows are gone
>
> **§D2 below still lists five `Can*FeatureFlag` policies. They no longer exist.** The endpoints they
> guarded — a CRUD surface over a feature-flag table that, once the data-retention switch moved to
> configuration (T-0685), gated nothing at all — were deleted along with the table, the entity, the
> repository and the five policies themselves.
>
> **The individual rows are not rewritten**, per this repo's rule that an accepted ADR is changed by
> superseding rather than editing: the D2 decision about how those endpoints *should* have been guarded
> was correct and nobody has re-decided it. This banner is the correction. No surviving route changed
> its physical policy, and no permission was widened.
>
> `FrozenPermissionMapTests` — the test that pins §D2 against the live map — was updated in the same
> change and records the removal at its Feature Flags entry.
>
> **Retires when:** §D2 is superseded by a later ADR that restates the table.
...
This **adapts** S2 (`docs/architecture/security-rules.md §S2` — "Authorization on every endpoint"):
...
> **What shipped since — 2026-09-21.** These rows name constants that no longer exist in `Policy.cs`: `CanUpdateOrder`, `CanViewOrderReview`, `CanAddPhoneNumber`, the Country Configuration View/Create/Update/Delete quartet and `CanCreateTenantConfiguration` (no `[Permission]` carried them on any host), `CanCalculateOrderPay` and `CanViewPayPeriod` (their partner-host actions `EmployeePayrollController.CalculateOrderPay` and `PayPeriodController.GetPayPeriodById` were removed with no caller). The five Feature Flag constants went earlier with their table (T-0689); their rows stay, per the banner at the top. The table is the 2026-06 record; `Policy.cs` and `FrozenPermissionMapTests` are the map today.
...
Role files in `docs/domain/roles/` created/updated by this decision:
...
- Update `docs/architecture/security-rules.md` companion note under S2 to cite this ADR as the
...
> `docs/architecture/platform-expandability.md` §7b/§8. The A1 ruling (Option A for `MembershipPlan`) is unchanged
