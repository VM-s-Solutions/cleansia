...
> ### ⚠️ Correction banner — 2026-09-08 (T-0689): the "feature flag" §A5 names is gone
>
> **§A5 and the CH-G row in §A9 reject hosting dead-letter retention in `DataRetentionBackgroundService`
> partly because the whole job short-circuits on its master switch.** §A5 was updated in T-0685 to name
> that switch correctly; §A9's CH-G row still says "feature-flag short-circuit" and is not edited here.
> The objection was right — more right than it knew. The flag was a row no migration ever inserted, so the sweep had never run on any
> deployed database (T-0685). The switch is now `DataRetention:Enabled`, bound from configuration and
> defaulting to true, which is exactly the shape §A6.1 prescribes for `DeadLetterRetention`.
>
> The feature-flag table itself was then deleted outright (T-0689): once this switch left it, nothing in
> the platform read it.
>
> **The rejection stands and its wording is not edited** — only the mechanism it cites has changed name.
> Read "feature flag" in §A5/§A9/§A6.1 as "the job's master switch".
>
> **Retires when:** §A5 is superseded by a later ADR.

> ### ⚠️ Correction banner — 2026-09-12 (T-0709): one `generate-invoice` delivery now yields one invoice per currency
>
> **D2.1's key table reads `generate-invoice` | `invoice:{PayPeriodId}:{EmployeeId}` | "one invoice per
> employee per period".** The key is unchanged — `MessageKeys.Invoice` is still
> `invoice:{payPeriodId}:{employeeId}` — but the effect column no longer describes what one delivery
> produces. `GenerateInvoice` groups the pair's un-invoiced pay by currency, claims a payout reference
> per group, writes one `EmployeeInvoice` per currency and flushes them together, so the honest effect
> is *one generation per employee per period, one invoice per currency the period's pay holds*. A
> cleaner who worked a CZK job and a EUR job in one period receives two documents; the unique index
> is `(EmployeeId, PayPeriodId, CurrencyId)`. → /flows/pay-and-payouts#the-path
>
> **D2.2 names the invoice consumer's target-state guard as "`EmployeeInvoice` for `(PayPeriodId,
> EmployeeId)` does not exist".** The guard that shipped is per currency: `GenerateInvoice.Validator`
> refuses `payroll.invoice.already_exists` only when an un-invoiced pay row would join an invoice the
> pair already holds in that row's currency. A redelivery after a full run is a no-op because no
> un-invoiced row remains (`payroll.no_unpaid_order_pays`), not because a `(PayPeriodId, EmployeeId)`
> row exists.
>
> **D3.4 describes the invoice-side reconciliation candidate as "`PayPeriod`s with employees who have
> no `EmployeeInvoice`".** The anti-join is per currency
> (`PayPeriodRepository.GetInvoiceReconciliationCandidatesAsync`): a pair whose CZK pay is invoiced and
> whose EUR pay is not is still a candidate. The message key and the re-enqueue path are unchanged, and
> the consumer invoices every currency still open.
>
> **The contract stands and its wording is not edited** — the frozen key formula, the guard-first rule
> and the reconciliation backstop all survive. Only the cardinality of the effect behind one key moved.
>
> **Retires when:** D2.1, D2.2 and D3.4 are superseded by a later ADR.
...
This **adopts** S7 (`docs/architecture/security-rules.md:100` — "Idempotency on side-effecting
...
- **Hosting the sweep in `DataRetentionBackgroundService`.** Rejected on two mechanics: it runs
  **weekly** (`Functions/DataRetentionTimerFunction.cs:10`, `0 0 3 * * 0`), which turns a 7-day body
  window into a worst case of 14; and the whole job short-circuits on a master switch
  (`DataRetentionBackgroundService.cs:31-39`, `DataRetention:Enabled`) — a PII-minimization clock
...
| `Cleansia.Core.AppServices/Features/DataRetention/DataRetentionBackgroundService.cs` | **pointer comment only** above `RunAllRetentionTasksAsync`: dead-letter retention lives in `PruneDeadLetters` (daily, not weekly; not behind `DataRetention:Enabled`) — §A5 |
...
`docs/domain/roles/dead-letter-record.md` (new).
