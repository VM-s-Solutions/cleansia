...
- **Distributed confirmation-code guessing (worse than passwords).** Verified: `ConfirmUserEmail`
  ...
  unchanged by that rename; the citation is dated here rather than rewritten.)* A 6-digit code is a 10^6 space; distributed across a botnet it is brute-forceable
