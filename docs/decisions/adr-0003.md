---
title: ADR-0003 — Partitioned rate limiting
adr: ADR-0003
status: accepted
---
...
This **adapts** S5 (`docs/architecture/security-rules.md §S5` — "Rate limiting on auth + side-effecting
...
- **Distributed confirmation-code guessing (worse than passwords).** Verified: `ConfirmUserEmail`
  ...
  `[AllowAnonymous]`. *(2026-09-13: `ExistsWithConfirmationCodeAsync` no longer exists and the legacy
  read is `GetByConfirmationCodeIgnoringTenantAsync` — ADR-0061 D4. The exposure this bullet describes is
  unchanged by that rename; the citation is dated here rather than rewritten.)* *(2026-09-15: the
  "code alone" sentence is no longer true of the 6-digit path. `ConfirmUserEmail.Command` is
  `(Email, Code)`: a 6-digit OTP is **never resolved by the bare code** — the e-mail names the single
  account whose stored hash the code is compared against, and every attempt that resolves an account
  spends one unit of that account's **per-code attempt budget** (5 tries per issued code; a resend
  resets it) before validity is evaluated, so a guessing run cannot convert a late hit. Only the legacy
  22-character link token resolves by code alone, and it is a 128-bit secret. The residual below is
  therefore the legacy link's, and the distributed-guessing figure for the OTP no longer applies; the
  per-account budget is the account-bound throttle this bullet asked for. Off a customer host the
  command confirms an Employee or Administrator only — T-0754.)* A 6-digit code is a 10^6 space; distributed across a botnet it is brute-forceable
...
New role file in `docs/domain/roles/`:
...
Catalog edit (same change): `docs/architecture/security-rules.md §S5` already records that auth/side-
...
Questions opened (same change): `agents/archive/2026-08/backlog/questions/open.md` —
