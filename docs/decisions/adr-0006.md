---
title: ADR-0006 — Refund dispute money path
adr: ADR-0006
status: accepted
---
...
> **What shipped — 2026-09-22.** `RefundStatus.Failed` (D5's third status) has **no producer**:
> `Refund.Create` writes `Pending`, `MarkSucceeded` is the only transition, and a Stripe failure
> inside `RefundService` leaves the row `Pending` for the keyed re-drive (confirm-then-record, D7) —
> no `refund.failed` webhook case exists. The value stays on the enum and the wire — Q-UI-05, default
> in force since 2026-09-20: neither build the producer nor strike the state until the owner rules.
