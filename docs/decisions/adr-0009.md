...
> **not**: `agents/archive/2026-08/backlog/adr/` already contains 0001–0008, with **0007 = soft-delete-policy** and
...
> ### ⚠️ Correction banner — 2026-09-12 (T-0709): the catalogue price shape and the loyalty divisor this ADR cites are gone
>
> **Fact 8 and D5.1 describe a price authored once on the catalogue entity.** Fact 8 cites
> "`Service.BasePrice` / `PerRoomPrice` (`Service.cs:19,21`) are the *standalone* catalog prices", and
> D5.1 rules that "an `OrderService`'s gross basis is its standalone catalog price (`Service.BasePrice +
> PerRoomPrice × (rooms+bathrooms)`, matching `OrderPricingCalculator.cs:30`) and an `OrderPackage`'s
> gross basis is `Package.Price` (`OrderPricingCalculator.cs:26`)". Neither column exists any more.
> `Service`, `Package` and `Extra` carry no price; a price is one row per (entry, currency) in
> `ServicePrices` (`BasePrice`, `PerRoomPrice`), `PackagePrices` and `ExtraPrices`, nothing converts,
> and an entry with no row in a currency is not offerable in it. The formula D5.1 names is unchanged —
> `BasePrice + PerRoomPrice × (rooms + bathrooms)` — but it is evaluated once, at creation, against the
> row in the order's currency, and frozen as `OrderService.LineTotal` / `OrderPackage.LineTotal` (with
> the bundled split frozen as `OrderPackageService.LineGross`). The allocator's ratio weights are those
> frozen line figures, never a re-read of the live catalogue, so an admin price edit cannot move the
> denominator of a refund on an order placed months earlier. D2's invariant — the share is multiplied by
> the frozen `Order.TotalPrice`, discount and surcharge never re-applied — is untouched.
> → /product/business-rules#price-stages
>
> **Fact 7, D6, D8 (TC-REFUND-LOYALTY) and CH-5 fix the divisor at 10.** D6 rules "Revoke
> `floor(refundNet / 10)` points per refund … symmetric with the earn `floor(order.TotalPrice / 10)`
> (`LoyaltyService.cs:46`)". The divisor is now a property of the order's currency,
> `Currency.LoyaltyPointsDivisor`, authored by the admin on the currency form; CZK is seeded at 10, so
> every number in this ADR still holds for a CZK order. One helper (`LoyaltyService.PointsForAsync`)
> serves both the earn and the clawback, so the symmetry D6 wanted is structural rather than a matter of
> two literals agreeing. A currency with no divisor earns nothing, claws back nothing, and logs — it is
> never scaled from another currency's rate. → /product/business-rules#money-constants
>
> **The policy stands and its wording is not edited** — the 14-day soft window, share-of-frozen-
> `TotalPrice` allocation, the fee-bearer switch, the per-refund-keyed clawback and the weight model all
> survive. Only the source of the ratio weights and the value of the divisor moved.
>
> **Retires when:** D5.1 and D6 are superseded by a later ADR.
...
rules / fee bearer). The owner has now answered both (`agents/archive/2026-08/backlog/questions/open.md`, both RESOLVED
...
Role files in `docs/domain/roles/` (these layer on ADR-0006's `refund-service.md` / `refund.md`,
