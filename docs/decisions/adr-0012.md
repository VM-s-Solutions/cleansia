...
- **Q-AUDIT-01 (`pre-prod`, owner/legal)** — the regulatory **minimum-retention window** for
  ...
  *(Never re-filed after the 2026-08 backlog reset; **re-filed 2026-09-13 as Q-AUD-L1** by
  [ADR-0062](/decisions/adr-0062), whose customer table takes a three-years-per-row window. The admin
  table's "append-only, no auto-delete" default is unchanged by that ADR.)*
