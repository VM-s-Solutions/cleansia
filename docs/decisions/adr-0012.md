---
title: ADR-0012 — Admin action audit log
adr: ADR-0012
status: accepted
---
...
*(**Extended 2026-09-13 by [ADR-0062](/decisions/adr-0062):** the predicate below is now the first arm of
`AuditGate.Resolve`, unchanged; a second, opt-in arm routes a command marked `Audience = Customer` run by
a `Customer` — or anonymously where the marker allows it — to its own `CustomerActionAudit` table. The
admin arm wins, an Employee still lands nowhere, and the failure row's `ErrorCode` on **both** arms is
now the `BusinessErrorMessage` key rather than the field name — owner ruling Q-AUD-O2.)*
...
#### Later ruling — 2026-09-22, shipped 2026-09-23 {#audit-retention-2026-09-22}

> The owner set a **three-year default** for admin and cleaner audit rows. This supersedes D6's
> retain-without-auto-delete default below, while preserving the original decision as history.
> `DataRetentionBackgroundService` now runs `AdminActionAudits` and `EmployeeActionAudits` per
> company under `retention.admin_audit.years` and `retention.employee_audit.years`, deleting by each
> row's own `OccurredOn` and `CreatedOn` respectively. A subject's erasure still does not delete
> either trail. Age-based deletion is the additional, explicit exception to append-only storage.
> The catalogue currently accepts 1–100 years; whether these two minima must be three years remains
> an owner question. This ruling concerns the live tables and does not change sealed company archives.
> → [Current retention rules](/product/business-rules#customer-record),
> [ADR-0062 — later ruling](/decisions/adr-0062#audit-retention-2026-09-22)

#### Original decision (2026-06-22)
...
- The trail is **atomic** with the action (success path) — a completed admin mutation cannot exist
  without its record, and a record cannot exist for a rolled-back action. **Nine admin commands are a
  documented exception**: they flush inside the handler so a unique-index race can surface as a
  translated error instead of a 500, which puts their audit row in a second transaction. See
  [Later amendment (2026-09-10)](#later-amendment-2026-09-10-the-early-flush-carve-out).
...
Role files in `docs/domain/roles/`:
...
The entries below record the earlier questions. The admin and cleaner default window was later set
to three years on 2026-09-22; see [D6's dated amendment](#audit-retention-2026-09-22). The present
one-year configurable floor remains a separate owner question, not an unmade default-window decision.

Filed in `agents/archive/2026-08/backlog/questions/open.md`:
- **Q-AUDIT-01 (`pre-prod`, owner/legal)** — the regulatory **minimum-retention window** for
  ...
  *(Never re-filed after the 2026-08 backlog reset; **re-filed 2026-09-13 as Q-AUD-L1** by
  [ADR-0062](/decisions/adr-0062), whose customer table takes a three-years-per-row window, and
  **ruled on by the owner 2026-09-14 — "keep 3"** (ADR-0062 §Rulings). The admin table's "append-only,
  no auto-delete" default is unchanged by either.)*
...
## Later amendment (2026-09-10) — the early-flush carve-out

*The ADR above is accepted and unchanged. This section records a class of exception discovered while
implementing multicurrency, so that a reader who trusts the atomicity guarantee knows exactly where it
does not hold and why the alternative was rejected rather than overlooked.*

### What is true

D2 puts `AuditLogBehavior` inner to `UnitOfWorkPipelineBehavior`, so the audit row and the business
change ride the same `SaveChangesAsync`. That still describes the ordinary case — the great majority of
admin commands mutate, return, and are committed once by the pipeline.

**Nine admin commands flush inside the handler, before the pipeline commit.** For those, the business
change is durable in its own transaction and the audit row is written by a second one:

| Command | Why it flushes |
|---|---|
| `Features/AdminUsers/CreateAdminUser.cs:113` | unique email — the index is the arbiter |
| `Features/Currencies/CreateCurrency.cs:69` | unique `Code` |
| `Features/Currencies/UpdateCurrency.cs:79` | unique `Code` |
| `Features/Currencies/SetDefaultCurrency.cs:104-126` | filtered unique `IsDefault`, cleared then promoted |
| `Features/PromoCodes/Admin/CreatePromoCode.cs:151` | unique code |
| `Features/Packages/DeletePackage.cs:61` | FK restrict → `package.in_use` |
| `Features/Services/DeleteService.cs:61` | FK restrict → `service.in_use` |
| `Features/EmployeePayroll/GenerateInvoice.cs:131` | unique invoice number |
| `Features/EmployeePayroll/AssignInvoiceVariableSymbol.cs:122` | unique variable symbol |

`Features/EmployeePayroll/RegenerateInvoicePdf.cs:206` also flushes, but only on its failure path where
the row it writes is deliberately out of band. It is not part of this carve-out.

**They flush for one reason, and it is not convenience.** Each is a race the validator cannot close: a
uniqueness or referential rule whose only real arbiter is the database. To turn the resulting `23505`
or `23503` into a translated error key the admin can act on, the violation has to surface as a
catchable `DbUpdateException` at a point the handler still owns. Left to the pipeline's single commit
it surfaces as a plain-text 500 that the shared `HttpErrorInterceptorFn` cannot read — the admin sees
"An error occurred" for a duplicate currency code. For `SetDefaultCurrency` (and its twin
`Features/SavedAddresses/SetDefaultSavedAddress.cs`) it is stronger than ugly: a partial unique index
cannot be deferred in Postgres, and EF emits UPDATEs in change-tracker entry order, so a pipeline-only
commit makes the operation *impossible* rather than merely unfriendly.

### What is actually lost, and what is not

**Only atomicity is lost, not the record.** On the ordinary path both writes succeed and the trail is
complete; the gap is an infrastructure failure between them. In that window the business change is
durable and either `AuditFailureCaptureBehavior` writes a `Success = false` row — which is misleading,
because the action did succeed — or, with an already-cancelled token, nothing but a log line survives.

**Actor and time survive anyway for eight of the nine.** Every entity involved is `Auditable`, and
`CleansiaDbContext.CommitAsync` stamps `CreatedBy`/`UpdatedBy` and the timestamp from the same JWT
actor id during the *early* flush. So who did it and when is recoverable from the row itself.

**The exception is the two deletes.** `DeletePackage` and `DeleteService` remove the row, so there is
no stamped entity to recover from. If their audit write is the one that fails, the deletion is
unattributed. That is the only case where the loss is total, and it is the one worth knowing.

### The alternative was weighed and rejected

Moving the transaction up — having `UnitOfWorkPipelineBehavior` open one transaction spanning the
handler and the audit write — would restore atomicity everywhere at once. It changes the transaction
semantics of every `ICommand` in the backend simultaneously, across five hosts, and it breaks
`SetDefaultCurrency` and `SetDefaultSavedAddress` immediately, because both open their own
`BeginTransactionAsync` and Npgsql does not nest. The cost is a platform-wide change to how every
handler commits; the benefit is closing a window that requires an infrastructure failure between two
adjacent saves, in which the actor and time are usually still recoverable.

**Not worth it. Recorded rather than fixed** — so that the next reader spends no time rediscovering
it, and so that anyone who does decide to close it knows what it costs.
