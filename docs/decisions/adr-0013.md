---
title: ADR-0013 — Ios app architecture and port strategy
adr: ADR-0013
status: accepted
---
...
- Owner provisioning: the **APNs auth key/cert** (Apple Developer), a **Mapbox token only if** D6's fallback
  is ever taken, and the **Apple Developer account / signing** (an owner setup, not an agent step).
...
Role files in `docs/domain/roles/` (CRC cards — added when the Phase-0 tickets land so they reflect the
written code):
