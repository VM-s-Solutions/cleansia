---
title: ADR-0017 — Multi region expansion seam and its composition…
adr: ADR-0017
status: accepted
---
...
- **`null` TenantId = single-tenant mode** (CLAUDE.md, confirmed by the filter). Today the platform runs
  ...
  *(**Superseded 2026-09-13 by [ADR-0061](/decisions/adr-0061):** `TenantId` is NOT NULL on every stamped
  table, `null` is never a tenant, and a tenant is an operating company serving one or more countries —
  so D3's "two markets ⇒ two tenants" below is also superseded. The filter body, the claim and the
  tenancy=app / region=infra split in this ADR are unchanged.)*
