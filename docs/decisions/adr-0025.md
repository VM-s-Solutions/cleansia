---
title: ADR-0025 — Ios push display per platform apns alert with loc…
adr: ADR-0025
status: accepted
---
...
- **NEW — `docs/domain/roles/fcm-message-factory.md`** (created with this ADR): pure translator
...
space) and `docs/domain/roles/fcm-message-factory.md` (CRC card).
...
## Amendment A2 (2026-09-13, T-0721) — `amount` joins the lock-screen allowlist, for one key

Owner ruling 2026-09-13 (Q-MARKET-04, recorded in ADR-0060's amendment): the no-cleaner push names
the credit **with its currency**. ADR-0060 D1 had kept D3 closed on the ground that a loc-arg is a
string the server formats without a device locale; the owner ruled the figure matters more than the
locale formatting, so the figure ships and the string is locale-neutral.

**Amended:** D3's closed allowlist is now **`{orderNumber, count, amount}`**. `amount` is carried by
**`order.no_cleaner_refunded` only** — `FcmMessageFactory.ApnsDisplayMap[order.no_cleaner_refunded]
= ["orderNumber", "amount"]`; no other entry lists it, and the plain `order.cancelled` never carries
it. Its value is produced by one server-side rule, `CancelUnfilledOrders.FormatCreditAmount`: the
number in invariant culture with no trailing zeros, a space, the credit currency's `Symbol` — its
`Code` when there is none — so `"250 Kč"`, `"10 €"`, `"9.5 zł"`. It is formatted on the server because
the credit's currency is the credit's own (the customer's credit account in that currency), which the
device cannot derive from the order. Missing value → empty string, as for every other arg.

**The S6 line holds.** A money figure with a symbol identifies no person; it is the same class of
value as `orderNumber` and `count`. Internal ids, names, addresses and free text stay forbidden.

**Clients:** both iOS app catalogues carry `%2$@` in `push.order.no_cleaner_refunded.body` (five
locales); the Android customer template places the same arg (`args["amount"].orEmpty()`) into
`notification_order_no_cleaner_refunded_body`'s `%2$s`. Client-first held: the catalogue slots shipped
with the map entry.

**Verification:** TC-PUSH-APNS-5's union assertion widens by the one name; `FcmMessageFactoryTests`
pin the entry and that `amount` appears in no other `LocArgs` list; `CancelUnfilledOrdersTests` pin
the format. Every other verification above is untouched. The D3 comments on the map and in
`NotificationEventCatalog` record the reversal. Role card: `/domain/roles/fcm-message-factory`.
