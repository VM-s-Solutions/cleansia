---
title: ADR-0027 — Immediate user session cutoff on password reset via…
adr: ADR-0027
status: accepted
---
...
- The device directory's CRC card explicitly lists **"Users"** under *does NOT know* (ADR-0026 D9.4
  boundary — `docs/domain/roles/revoked-device-directory.md:47-48`). Folding user semantics in
...
- `docs/domain/roles/revoked-user-directory.md` — **created** (CRC card for the new singleton,
  the one new role this ADR introduces; sibling to `revoked-device-directory.md`).
- **Catalog edit at acceptance (not before):** `docs/architecture/security-rules.md` — the ADR-0024/
...
- **Catalog edit** — `docs/architecture/security-rules.md` (S2 neighbourhood): the ADR-0024/0026
...
- **Living doc** — `agents/architecture/decisions/auth-sessions.md`: flip the ADR-0027 markers from
  ...
  this change.)
- **CRC card** — `docs/domain/roles/revoked-user-directory.md`: flip proposed → accepted, add
