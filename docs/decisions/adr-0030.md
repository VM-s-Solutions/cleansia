---
title: ADR-0030 — Web admin access token ttl 15 min
adr: ADR-0030
id: ADR-0030
...
# ADR-0030 — Web.Admin access-token TTL is 15 minutes — the web revocation bound (supersedes-in-part ADR-0024 for the admin host)
...
## Effectiveness gate CLOSED — 2026-08-14 (cleanup track, `CL-051`)

*Appended per §"Accepted ADRs are immutable" — a dated note, not an edit. **No line above is changed.**
The decision it records was right; only its "OPEN" pre-flight has since been discharged, and an ADR
that overstates a live risk trains readers to discount ADRs.*

**The pre-flight recorded above as `OPEN, owner/T-0400` is closed, on both halves.**

| What the ADR recorded as open | State on 2026-08-14 |
|---|---|
| `environment.prod.ts` `authApiBaseUrl` points admin auth at `api.cleansia.cz`, the **partner** host | The key **does not exist anywhere in the tree**. `apps/cleansia-admin.app/src/environments/environment.prod.ts:5` pairs admin with `https://api-admin.cleansia.cz` — one base URL, correctly paired |
| The prod flip is therefore *"inert for the deployed admin app"* | It is **effective**. `Cleansia.Web.Admin/appsettings.Production.json:19` carries `AccessTokenExpMinutes: 15`, and `AccessTokenTtlConfigPinTests` pins it against the raw file |

So the 15-minute TTL this ADR decided is live in production, and the sentence above describing it as
inert should be read as history. Nothing about the decision changes.
