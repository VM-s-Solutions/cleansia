...
> ### ⚠️ Correction banner — 2026-09-12: D7's hard stop sits at invoice **approval**, not at issuance
>
> **D7 says the hard stop "moves to invoice/payout issuance": `EmployeeInvoice` generation refuses when
> the payout record is absent, `Scheme is null` or `Status != Provided`, recording an admin-visible
> `PayoutBlocked` reason.** What shipped puts the same three-condition gate at **`ApproveInvoice`**
> (`payroll.invoice.payout_details_missing`), judged before the T-0708 currency rule, and leaves
> generation ungated. The reason is the one T-0708 already used for the currency refusal: approval is
> the admin's commitment to key a manual transfer and the last point the platform can still say no,
> while generation is too early — refusing there withholds a sequence-numbered tax document from the
> cleaner for up to a month. Nothing D7 protects against happens before approval: a transfer to nowhere
> is only possible after it. And the document without a payment block is not the "knowingly defective"
> one D7 imagined — the PDF's payment block is already optional in the renderer, and
> `RegenerateInvoicePdf` reissues it under the same number once details arrive.
>
> **D7.1 and D7.2 are therefore not needed.** Both exist to mitigate a *withheld* invoice — the
> period-closed e-mail carrying the reason, and the reconciliation sweep re-enqueuing a pair it can
> never invoice. With generation ungated the invoice is issued on time and the pair leaves the
> candidate set the ordinary way; only the payment waits. Verification 7(a) holds at approval in
> `ApproveInvoicePayoutCurrencyTests`; 7(b) and 7(c) have no subject.
>
> **The decision stands and its wording is not edited** — a record that is absent, has no scheme or is
> not `Provided` still stops the money, and completeness is still decoupled from validity. Only *where*
> the stop sits moved. Read "issuance" in D7 and in Verification 7 as "approval".
>
> **Retires when:** D7 is superseded by a later ADR.

> **What shipped — 2026-09-22.** `PayoutDetailsStatus.NeedsReconfirmation` (D5's second status; the
> parked-value outcome in D7's migration table) has **no producer**: the one writer of the record,
> `UpdateBankDetails`, stamps `Provided` on both of its branches, and the only places the value appears
> are the enum, the admin's status label map and its spec, one test that arranges it to prove
> `ApproveInvoice` refuses it and one that proves `UpdateBankDetails` overwrites it to `Provided`. The
> member stays on the enum and the wire — Q-UI-05, default in force since 2026-09-20:
> neither build the producer nor strike the state until the owner rules.
