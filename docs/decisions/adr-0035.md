...
> **Dated cross-reference — 2026-09-26.** The oops window this ADR quotes as 15 minutes, with a dead
> 60-minute first-time arm, is since the owner's 2026-09-24 ruling **60 minutes for an entitled Plus
> member** and 15 for everyone else; the first-time arm is gone. A waiver is only ever reserved for an
> entitled member, so for one still entitled when they cancel, the zero-fee span in which AM-12 says a
> waiver can be lost is now 60 minutes. The decision stands — the quota is the bound, and AM-13's
> disclosure is what makes the loss fair. → [The oops window](/product/business-rules#oops-window)
...
> `agents/archive/2026-08/backlog/adr/challenges/0035-A-alternative.md`, `0035-B-exploit.md`, `0035-C-concurrency.md`.
...
`timestamp`).** ⚠️ **`ef-migration` — owner-only. the agent does not create it, and this amendment does
not create it.**
