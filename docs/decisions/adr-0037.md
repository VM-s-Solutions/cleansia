---
title: ADR-0037 — Order offerability is a payment qualified status…
adr: ADR-0037
status: accepted
---
...
- **Status:** `accepted` — **2026-08-03, by the lead of the defense panel** (`## Verdict`). Drafted
  ...
  **attacked by two challenger lanes — 19 findings, 8 of them marked blocking — and adjudicated
  ...
  a superseding ADR.
- ⚠️ **SUPERSEDED IN PART, 2026-09-08, by [ADR-0057](./adr-0057) — D1's STATUS TERM only.** D1 states
  the rule as "concretely `Confirmed`, plus `New` **only for `PaymentType.Cash`**". That clause exists
  only because `OrderStatus.Confirmed` was carrying a fact from the money axis: the Stripe webhook wrote
  it when payment settled, so `New` had to admit cash and nothing else. **Owner ruling 2026-09-08:**
  `Confirmed` means a cleaner took the job and nothing else, a paid card order rests at `New`, and the
  status term drops its payment qualifier — the money term was always asking that question. **Everything
  else in this record stands**: the two-axis shape, one rule in one place, `TakeOrder`'s status gate, the
  SQL/C# equivalence pin, and `Pending` being dead. Nothing above has been rewritten.
- ⚠️ **SUPERSEDED IN PART, 2026-08-22, by [ADR-0053](./adr-0053) — Fact 3's weekly-cap half only.**
  Fact 3 observes that the weekly order cap is applied status-blind, so a cancelled order still consumes
  a capped cleaner's allowance. That is **no longer true**: the cap became opt-in per cleaner and unset
  for everyone, and its count now filters slot-blocking statuses. **The rest of Fact 3 stands**, and so
  does every other Fact and Decision in this record — the cap is the only limb ADR-0053 touches. The
  dated record is at the END of this file; nothing above it has been rewritten.
...
(`docs/domain/roles/order-availability.md`) previously listed *"Payment state — it reads
...
`docs/domain/roles/order-availability.md` (its watch-list carried `Q-AVAIL-01` as unresolved), and
...
`agents/archive/2026-08/backlog/adr/challenges/0037-mechanism.md` (CH-M1…M10) and
`agents/archive/2026-08/backlog/adr/challenges/0037-experience.md` (CH-X1…X9). Summarised by id; the lead
...
## Superseded in part — 2026-08-22 (`architect`, lead mode) · ADR-0053

**What changed.** [ADR-0053](./adr-0053) removed the rating-tiered weekly order cap (3/6/10 by score,
applied to every cleaner automatically) and replaced it with `Employee.WeeklyOrderLimit` — a nullable
per-cleaner column an admin sets on one person, `null` meaning unlimited, which is every cleaner today.
The count behind it also gained a status term and no longer counts cancelled or completed work.

**What that supersedes here, precisely.** Fact 3's observation that the cap is status-blind, and only
that. The observation was correct when written and is now false. **Nothing else in Fact 3 is affected,
and no Decision in this record is.**

**What it does NOT supersede.** The prohibition on consulting the weekly cap from the preferred-cleaner
hold resolver stands untouched, and is if anything sharper: it rests on the count's window deriving from
`DateTime.UtcNow.Date`, which ADR-0053 left byte-identical, so at creation the count still answers about
a week that may not contain the booking. A cap that is null for almost everyone makes such a check look
free, which is exactly when a reader is most likely to add one.

**This record's status is unchanged (`accepted`) and its body has not been edited.** A decision that was
reversed in part is kept, not rewritten — code still cites it, and the reversal is part of the history.
