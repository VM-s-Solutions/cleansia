...
migration. The owner is dropping the database and regenerating; this rides that step. **The agent does
...
**There is no "startup twin".** The sentence above refers to `sql-scripts/insert_seed_data.sql`
"and its startup twin". The second copy — inside the retired `Cleansia.Infra.Scripts` project — was
removed on 2026-08-13 (`CL-039`), and startup now reads the repo-root file directly.

**The claim is unaffected and is now simpler to check:** the one remaining seed file contains no
`Orders` insert and no `CurrentStatus` reference.
