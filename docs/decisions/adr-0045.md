---
title: ADR-0045 — Favourite cleaner is a reservation the cleaner must…
adr: ADR-0045
status: accepted
---
...
> **Correction — 2026-09-16.** [ADR-0057](./adr-0057) supersedes this record's claims that
> `Confirmed` can mean money settled or cleaner acceptance, including the card-webhook
> `New → Confirmed` transition described below. `Confirmed` now records that a cleaner took the job;
> it does not guarantee a current assignee. A paid card order remains `New` + `PaymentStatus.Paid`.
>
> **Separate citation repair:** the fulfilment-axis discussion cites ADR-0037 D5 for a status rule.
> The status-term clause is in [ADR-0037 D1](./adr-0037#d1--the-ruling-offerability-is-a-two-axis-predicate),
> as partially superseded by ADR-0057; the single-source implementation is in ADR-0037 D3. D5
> declares `Pending` dead. References to D5 for that declaration still stand. The historical body is
> preserved; this banner corrects the status meaning and the factual miscitation separately.
...
> (`agents/archive/2026-08/backlog/questions/open.md:1981-1997`; the answer at `:1996-1997`). The post-booking screen
...
- `docs/domain/roles/preferred-cleaner-hold-resolver.md` — the resolver's "does NOT know" list
...
`agents/archive/2026-08/backlog/adr/challenges/NNNN-favourite-cleaner-reservation.md` — reviewed against HEAD, nine
