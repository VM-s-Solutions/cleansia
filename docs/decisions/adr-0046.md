---
title: ADR-0046 — Payout invoice variable symbol is a claimed number…
adr: ADR-0046
status: accepted
---
...
- **Living doc:** `agents/architecture/decisions/payout-invoice-references.md` (new, written at
  acceptance).
- **Role card:** `docs/domain/roles/payout-reference-allocator.md` (new, written at acceptance).
...
> ### ⚠️ Correction banner — 2026-09-12 (T-0709): a period now yields one invoice per currency, each with its own reference
>
> **§D1 item 4 sizes the counter at "one invoice per cleaner per period".** A payout invoice is one
> currency, so a cleaner whose pay in a period spans two currencies now receives two documents, and
> `GenerateInvoice` claims one variable symbol **per currency group** — every reference allocated before
> any invoice is staged, so a refused allocation leaves nothing half-written, and all of a pair's
> invoices flushed in one commit. The bound on the realized set is therefore
> (#cleaners × #periods × #currencies). With two currencies seeded and one operated, the capacity
> remark is as true as it was. The invoice-side reconciliation sweep (ADR-0002 D3.4), which re-enqueues
> the generation that claims these references, matches its anti-join on the pay row's currency too, so
> a pair whose CZK pay is invoiced and whose EUR pay is not is still re-enqueued under the unchanged
> `invoice:{PayPeriodId}:{EmployeeId}` key.
> → /flows/pay-and-payouts#the-path
>
> **§D1's third note and the CH-VS-6 ruling both cite "the `(EmployeeId, PayPeriodId)` unique index
> (`EmployeeInvoiceEntityConfiguration.cs:123-124`)" as the thing that bounds `|realized set|` at
> "(#cleaners × #periods) — one invoice per employee-period".** The index is now
> `IX_EmployeeInvoices_EmployeeId_PayPeriodId_CurrencyId`, unique on the triple, and the bound it
> states is the one above. The argument those passages make — the index bounds the set and makes
> nothing injective; density, not authorship, is the operative property — survives unchanged. Do not
> cite the wider index as if it helped either.
>
> **The decision stands and its wording is not edited** — the reference is still claimed before the row
> exists, stamped through the required constructor parameter, unique in one global namespace, printed
> unconditionally and refused as a business result. Only how many references one generation claims
> moved.
>
> **Retires when:** §D1 is superseded by a later ADR.

> ### ⚠️ Superseding note — 2026-09-15 (owner ruling Q-TENANCY-02, T-0757): each operating company numbers its own payout invoices
>
> **The owner answered the premise §D3.2 was contingent on.** ADR-0061 O-2 asked whether each legal
> entity must number its own invoices; the ruling was *"Separate everything — one holding and child
> companies per country."* With that, **Q-VS-03 is answered too**: the payer's account is *not* one
> account across the holding — each child company pays its own cleaners — so the namespace at least as
> wide as the payer is the company, and *global* is no longer "unconditionally at least that wide".
> The flip §D3.2 wrote down is what shipped, and it is recorded here because the decision text above
> is not edited. Three sections read differently:
>
> - **§D3.1 / §D3.2 — the namespace is the company, and the counter is keyed by it.**
>   `PayoutReferenceCounter` is `TenantAuditable` (stamped, FK'd to `Tenants`), keyed
>   **`(TenantId, Year, Scope)`** under `IX_PayoutReferenceCounters_Tenant_Year_Scope`, unique,
>   `NULLS NOT DISTINCT`; the repository takes `ITenantProvider` and writes the ambient company into
>   the arbiter, and with no ambient company the NOT NULL column refuses the row (`23502`) rather than
>   sharing one holding-wide sequence. `IX_EmployeeInvoices_VariableSymbol` became
>   **`(TenantId, VariableSymbol)` unique, `NULLS NOT DISTINCT`, filtered `"VariableSymbol" IS NOT NULL`**
>   — the exact shape §D3.2 named — and the inference channel §D3.2 accepted (a tenant admin reading
>   platform-wide volume from the gaps in their own sequence) is closed as a consequence, not a goal.
>   Two companies' first invoices of a year carry the same ten digits by construction; the
>   per-company index is what makes that a fact rather than a collision.
> - **§D2.1 — the `Scope` column is back, because the ADR can now name a second value for it.**
>   The 2026-08-09 verdict deleted `Scope` on the ground that *"this ADR cannot name a second value for
>   a scope"* (R4). It can now: **`VariableSymbol`** and **`InvoiceNumber`** are two independent series
>   on one table, the way `FiscalCounter.IssuerScope` keys one row per issuer. The statement is
>   `INSERT … ON CONFLICT ("TenantId", "Year", "Scope") DO UPDATE … WHERE "Value" < 999999 RETURNING
>   "Value"`; the cap, the self-commit exception, the empty-`RETURNING` guard and the gappiness contract
>   are unchanged. The nulls-distinct trap R4 feared is closed by the NOT NULL tenant term plus the
>   option on the index, not by the absence of a scope column.
> - **§D1 item 6 — `InvoiceNumber` is a per-company series, not a random suffix.** `INV-yyyyMM-XXXXX`
>   (five random hex) is replaced by **`INV-YYYY-NNNNNN`** — the `InvoiceNumber` scope of the same
>   counter, `PayoutReferenceAllocator.AllocateInvoiceNumberAsync` / `FormatInvoiceNumber`, refused
>   `payroll.invoice.reference_capacity_exhausted` at the cap like the symbol — under a
>   **`(TenantId, InvoiceNumber)` unique, `NULLS NOT DISTINCT`** index. `PaymentReference` still
>   mirrors the number; the dead `GenerateInvoiceNumber(prefix)` is deleted. The two references still
>   cannot coincide (one is ten digits, the other starts `INV-`), which is what item 6 required.
>
> **What did not move:** the VS format (`YYYYNNNNNN`, ten digits, the year of allocation — §D1 items
> 1–5), the allocate-before-construct rule and the required constructor parameter (§D2), no backfill
> (§D4), what prints (§D5 — the company on the PDF was already the ambient tenant's `CompanyInfo`),
> the duplicate-as-business-result rule (§D6). The proof moved with the key:
> `PayoutReferenceAllocatorTests` now asserts the counter **is** keyed per company
...
> `The_Counter_Table_Is_Keyed_Per_Company_Year_And_Scope`), the `NullsNotDistinctIndexModelTests`
> roster carries the three indexes, and the production census
> (`PayoutReferenceProductionCensusTests.The_Pay_Period_Batch_Numbers_Each_Companys_Invoices_From_Its_Own_Series`)
> drives the pay-period sweep across two companies and reads each company's own series back.
>
> → [ADR-0061](./adr-0061) D9 as amended · [PayoutReferenceAllocator](/domain/roles/payout-reference-allocator)
>
> **Retires when:** a later ADR supersedes §D3 outright.
...
`agents/archive/2026-08/backlog/adr/challenges/NNNN-payout-invoice-variable-symbol.md`** — twelve findings
...
filed at `agents/archive/2026-08/backlog/questions/open.md:2097`). The challenge file stays as the record; it is
not duplicated here.
...
- Role card `docs/domain/roles/payout-reference-allocator.md`. Its **"does NOT know"** list must
...
`docs/domain/roles/payout-reference-allocator.md`. **R5.3's `consistency.md` edit is recorded as a
...
**`Cleansia.Infra.Scripts` no longer exists.** The project was retired on 2026-08-13 (`CL-039`): it
compiled zero `.cs` files and existed only to carry SQL, and nothing referenced it. Its 18 remaining
scripts moved to `sql-scripts/seed/`.

The sentence above cites `Cleansia.Infra.Scripts.csproj:10-17` to establish that only
`insert_seed_data.sql` was copied to output, and therefore that the eight invoice-number literals in
the other scripts are hand-run rather than executed. **That finding is unchanged** — the other scripts
were never on any execution path, and still are not. Only the file proving it has gone; the proof now
