---
title: ADR-0047 — A server redacted field is rendered off its own…
adr: ADR-0047
status: accepted
---
...
>
> *(**Dated correction, 2026-09-22.** `ConfirmationCode` is no longer on the roster, because it is no
> longer on `OrderItem` at all. Blanking it for a **browsing** cleaner never helped: the same field was
> served in full to every **assigned** cleaner, and it was one third of the key the anonymous
> guest-cancellation endpoint accepted. The guest booking is re-keyed on a per-order access token and
> the code is now a printed reference that authenticates nothing — removing a field beats redacting it.
> The mixed-blanking rule below is unaffected; three string scalars now, not four.
> → [When the gate is a secret the wrong people hold](/architecture/security-rules#gate-secret-wrong-people))*
>
