...
**Tři podklady k obchodnímu modelu proti zdrojovému kódu · commit `6792f0c256e81e1473908308bd73881004853be4` (`master`, 22. 9. 2026)**
...
Odkaz `[Soubor.cs:123]` míří na řádek kódu auditovaného commitu. Odkaz „schéma s. 3", „strom s. 4" nebo „sešit, list Balíčky" míří do podkladu ve složce `colleague-documents/`. Rozhodnutí `→ Rn` pokračují v registru ze sesterských dokumentů; R22–R27 jsou nová.

## Shrnutí

Podklady popisují **model A** — zprostředkovatelskou platformu s úklidníky na IČO — a stojí na stavu aplikace ke commitu `9ad0b0ed` (16. 9.) a na ceníku v3. Kód auditovaného commitu se s nimi rozchází ve čtyřech nosných bodech, a každý z nich mění čísla v kalkulaci.

**Cena.** Sešit počítá ceny zpětně z plochy v m², z pracnosti v minutách na m² a z hodinové sazby. Kód nezná m² ani hodiny: cena je součet katalogových cen balíčků, služeb (`BasePrice + PerRoomPrice × (Rooms + Bathrooms)`) a položek, plus jediný příplatek — expres +20 %. Deset balíčků nového ceníku (velikost × ESSENTIAL/DEEP) nemá v katalogu protějšek; ten má osm balíčků s jednou cenou.
...
**DPH.** Sešit počítá s plátcovstvím a sazbou 21 %. Seed vede společnost jako neplátce, doklad tiskne „Nejsme plátci DPH" a DPH se z ceny **vyjímá, nepřičítá** — registrace by tedy žádnou katalogovou cenu nezvedla.
...
Nová rozhodnutí: R22–R27.

## 1. Co jsou ty tři podklady

| podklad | datum | z čeho vychází | co určuje |
|---|---|---|---|
| `Cleansia_cenik_a_kalkulace_v3.xlsx` | 21. 9. 2026 | zadání odměny 300 Kč/h, cílový příspěvek 18 % | ceny balíčků, doplňků a příplatků, bod zvratu |
...
Oba PDF podklady čtou aplikaci ke commitu `9ad0b0ed`; auditovaný strom je o 150 commitů dál, takže část jejich značek „rozpor" je vyřešena a část se posunula — §8.
...
| Provozní společnost vystavuje doklad zákazníkovi i samofakturu (schéma s. 1) | doklad nese `CompanyInfo` společnosti, faktura má dodavatele úklidníka — [insert_seed_data.sql:1145-1157](../../sql-scripts/insert_seed_data.sql#L1145-L1157) | platí | → R1 |
...
| Preferovaná uklízečka drží zakázku 10 % předstihu, max. 12 h (strom s. 3) | `PreferredHoldFraction`, `PreferredHoldCeilingHours` — [BookingPolicy.cs:216-217](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L216-L217) | platí | — |
| Vícečlenná posádka = samostatné účty a odměny (strom s. 3) | každý přiřazený úklidník dostane vlastní úplný výpočet — [CompleteOrder.cs:340-348](../../src/Cleansia.Core.AppServices/Features/Orders/CompleteOrder.cs#L340-L348) | platí, s důsledkem v §5 | → R13 |
...
| Smlouva o dílo vzniká při převzetí, podpis přejetím prstu, např. Signi (strom s. 5) | `StageAsync` uloží přijetí textu při převzetí; poskytovatel podpisu v kódu není — [TakeOrder.cs:356](../../src/Cleansia.Core.AppServices/Features/Orders/TakeOrder.cs#L356) | postaveno bez Signi | → R11 |

## 3. Ceník: struktura

| dimenze nového ceníku | co má kód | → |
|---|---|---|
| Plocha v m² (25–180) jako vstup | zakázka nese `Rooms` a `Bathrooms`, m² nikde — `grep -rn "SquareMeters|AreaM2" src/` → 0; [Order.cs:35-37](../../src/Cleansia.Core.Domain/Orders/Order.cs#L35-L37) | → R25 |
| Pracnost 3 / 5 / 7 min na m² | doba je součet `EstimatedTime` zvolených služeb — [OrderDuration.cs:29-31](../../src/Cleansia.Core.Domain/Orders/OrderDuration.cs#L29-L31) | → R25 |
| Balíček nese velikost, dispozici a hodiny | `Package` nese `Name`, `Description`, `Tagline`, `IsPopular`, `Translations` — [Package.cs:8-39](../../src/Cleansia.Core.Domain/Packages/Package.cs#L8-L39) | → R25 |
...
| Noční a víkendový úklid | +25 % | `grep "NightSurcharge|WeekendSurcharge" AppServices/` → 0; okno je `FirstWindowHour` až `LastWindowHour` — [BookingPolicy.cs:42-43](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L42-L43) | → R25 |
...
| Cena i odměna jsou na jednu zakázku (sešit, list Ekonomika) | odměnu dostane každý přiřazený úklidník celou; cena se počtem úklidníků nemění — [CompleteOrder.cs:340-348](../../src/Cleansia.Core.AppServices/Features/Orders/CompleteOrder.cs#L340-L348) | → R26 |
...
| ② zákazník platbu potvrdí, teprve pak doklad | doklad hotovostní objednávky vzniká už při objednání — [OrderPaymentDispatcher.cs:82-91](../../src/Cleansia.Core.AppServices/Features/Orders/OrderPaymentDispatcher.cs#L82-L91) | → R6 |
...
| ③ úklidník odvede do 7 dnů pod variabilním symbolem | `VariableSymbol` nese faktura firmy úklidníkovi, tedy opačný směr — [EmployeeInvoice.cs:74](../../src/Cleansia.Core.Domain/EmployeePayroll/EmployeeInvoice.cs#L74) | → R6 |
...
| Inkasní zmocnění úklidníka v rámcové smlouvě | `grep "DirectDebit|Mandate|inkas" src/` → 0; `CreateCheckoutSessionAsync` je jediná platební cesta — [IStripeClient.cs:16](../../src/Cleansia.Core.Clients.Abstractions/Stripe/IStripeClient.cs#L16) | → R6 |
...
| Základ DPH při zprostředkování (strom s. 6) | `Calculate` dostává celou `TotalPrice` zákazníka, ne provizi — [OrderFactory.cs:313-314](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L313-L314) | → R1 |
...
| Doklad zákazníkovi RCP-RRRR-NNNN od provozní společnosti (strom s. 4) | číselná řada a vystavitel sedí; účtenka neplátce tiskne „Nejsme plátci DPH" — [ReceiptService.cs:422](../../src/Cleansia.Core.AppServices/Services/ReceiptService.cs#L422) | → R24 |
...
| Řádky dokladu se nesčítají na uvedený součet (strom s. 4) | řádky nesou katalogové ceny bez slevy a příplatku — [ReceiptService.cs:395-404](../../src/Cleansia.Core.AppServices/Services/ReceiptService.cs#L395-L404) | → R1 |
...
| IČO a DIČ provozní společnosti | seed nese `12345678` a `CZ12345678` s poznámkou `REPLACE WITH ACTUAL` — [insert_seed_data.sql:1161-1162](../../sql-scripts/insert_seed_data.sql#L1161-L1162) | → R2 |
...
| Smlouvu o dílo teprve generovat při přijetí zakázky (strom s. 5) | postaveno, bez PDF a bez kvalifikovaného podpisu | [TakeOrder.cs:356](../../src/Cleansia.Core.AppServices/Features/Orders/TakeOrder.cs#L356) |
...
| K dostavbě: zrušení hostovských objednávek (schéma s. 2) | v kódu je — host ruší přes číslo objednávky, e-mail a kód | [CancelGuestOrder.cs:18](../../src/Cleansia.Core.AppServices/Features/Orders/CancelGuestOrder.cs#L18) |
...
| Členství není ve VOP; benefity končí první neúspěšnou platbou (strom s. 5) | platí; plány `PLUS_MONTHLY` 199 Kč a `PLUS_YEARLY` 2 030 Kč — [insert_seed_data.sql:1835-1836](../../sql-scripts/insert_seed_data.sql#L1835-L1836) | → R18 |
| Sleva Plus a věrnostní stupeň se sčítají se stropem 12 % (strom s. 5) | platí | [OrderFactory.cs:354-363](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L354-L363) |
...
| R24 | Plátcovství DPH provozní společnosti | zůstat neplátcem, dokud obrat dovolí (dnešní seed) · registrovat a ceny ponechat (marže klesne o sazbu) · registrovat a ceny zvednout podle sešitu |
...
| Cleansia Plus | 199 Kč měsíčně, 2 030 Kč ročně | totéž — [insert_seed_data.sql:1835-1836](../../sql-scripts/insert_seed_data.sql#L1835-L1836) |
| Kredit na objednávku | čerpá se automaticky, max. 70 % | `MaxCreditShareOfOrder` 0,70 — [BookingPolicy.cs:97](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L97) |
