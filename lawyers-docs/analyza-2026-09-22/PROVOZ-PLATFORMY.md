...
**Provozní popis podle zdrojového kódu · commit 6792f0c256e81e1473908308bd73881004853be4 (master, 22. 9. 2026)**
...
Jedna kartová objednávka podle kódu:

| Otázka | Odpověď | citace |
|---|---|---|
| Kdo vystavuje doklad | provozní společnost (`CompanyInfo` podle země adresy), číslo `RCP-{rok}-{NNNN}`; úklidník nikdy | [ReceiptService.cs:42-52](../../src/Cleansia.Core.AppServices/Services/ReceiptService.cs#L42-L52), [Constants.cs:83-85](../../src/Cleansia.Core.AppServices/Common/Constants.cs#L83-L85) |
...
| Kdo drží peníze z karet | jeden Stripe účet holdingu (`config.SecretKey`) | [StripeClient.cs:34](../../src/Cleansia.Infra.Clients/Stripe/StripeClient.cs#L34) |
| Kdo drží hotovost | úklidník; `MarkCashCollected` zapíše `Paid`, `CashCollectedAt`, `CollectedByEmployeeId`; žádné započtení | [Order.cs:693-700](../../src/Cleansia.Core.Domain/Orders/Order.cs#L693-L700) |
| Co si platforma bere | žádná provize; odměna `BasePay + max(0, pokoje−1)·ExtraPerRoom + koupelny·ExtraPerBathroom + km·DistanceRatePerKm`, ořez `ApplyMinMaxClamp`; seed sazba `ROUND(BasePrice * 0.5, 2)` | [PayCalculatorExtensions.cs:12-18](../../src/Cleansia.Core.Domain/Extensions/PayCalculatorExtensions.cs#L12-L18), [insert_seed_data.sql:781](../../sql-scripts/insert_seed_data.sql#L781) |

Otevřená rozhodnutí: → R1, R3–R10, R11.1–R11.7, R12, R13, R15, R17–R21.
...
- Index `Email` unikátní globálně přes společnosti; účet vázán `FK_Users_Tenants_TenantId` (`Restrict`) — [UserEntityConfiguration.cs:107-108](../../src/Cleansia.Infra.Database/EntityConfigurations/UserEntityConfiguration.cs#L107-L108), [20260920204705_Initial.cs:1072-1077](../../src/Cleansia.Infra.Database/Migrations/20260920204705_Initial.cs#L1072-L1077)
- Objednávka v jiném trhu: adresa vybírá operátora (`operatorTenantId`), účet patří dál své společnosti; operátora drží dnes jen CZE (`cleansia-cz`) → R19 — [CreateOrder.cs:933-935](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L933-L935), [insert_seed_data.sql:1014](../../sql-scripts/insert_seed_data.sql#L1014)
...
- `CreateOrder`: `AllowsAnonymousActor = true`, `[AllowAnonymous]` na webovém zákaznickém hostu — [CreateOrder.cs:27](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L27), [OrderController.cs:77-79](../../src/Cleansia.Web.Customer/Controllers/OrderController.cs#L77-L79)
- Mobilní zákaznický host má tytéž operace (`CancelGuest` … `GetWorkContract`) — [OrderController.cs:20-274](../../src/Cleansia.Web.Mobile.Customer/Controllers/OrderController.cs#L20-L274)
- `UserId = null` (`IsGuest`); přístup trojicí `DisplayOrderNumber` + `Email` + `ConfirmationCode` — [Order.cs:526](../../src/Cleansia.Core.Domain/Orders/Order.cs#L526), [GuestOrderAccess.cs:12-24](../../src/Cleansia.Core.AppServices/Features/Orders/GuestOrderAccess.cs#L12-L24)
- Kód 6 hex znaků velkými (`GenerateConfirmationCode`), uložen otevřeně — [OrderExtensions.cs:7](../../src/Cleansia.Core.Domain/Extensions/OrderExtensions.cs#L7)
...
- Jen trh operátora požadavku (`OrderCountryOperatorMismatch`); `TermsAccepted` povinné (`consent.terms_not_accepted`) → R5 — [CreateOrder.cs:176-180](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L176-L180), [CreateOrder.cs:319-322](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L319-L322)
- Bez kreditu (`userId` prázdné → 0), bez bodů (`UserId == null`) — [CreateOrder.cs:1107-1110](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L1107-L1110), [LoyaltyService.cs:40-43](../../src/Cleansia.Core.AppServices/Services/LoyaltyService.cs#L40-L43)
- Bez reklamace (`[Permission]` → 401) a bez stažení dokladu (`DownloadReceipt` = `Authenticated`) — [DisputeController.cs:17-18](../../src/Cleansia.Web.Customer/Controllers/DisputeController.cs#L17-L18), [OrderController.cs:170-171](../../src/Cleansia.Web.Customer/Controllers/OrderController.cs#L170-L171)
- K účtu se nikdy nepřipojí: `UserId` píše jen `Create` a `AnonymizeCustomerData` — grep -rn "UserId = " src/Cleansia.Core.Domain/Orders/Order.cs → 2 zápisy; [Order.cs:960](../../src/Cleansia.Core.Domain/Orders/Order.cs#L960)
...
| Google `GoogleAuth` / Apple `AppleAuth` | sign-in-or-register, jen zákaznický host, jen s `TermsAccepted`; `IsEmailConfirmed = true` | — | [GoogleAuth.cs:170-190](../../src/Cleansia.Core.AppServices/Features/Auth/GoogleAuth.cs#L170-L190), [AppleAuth.cs:201-217](../../src/Cleansia.Core.AppServices/Features/Auth/AppleAuth.cs#L201-L217) |
...
| tick na objednávce | `CreateOrder` | jen audit `OrderBookingEvidence` (`TermsVersionAccepted`); `UserConsents` nevzniká | grep -rn "consentService" CreateOrder.cs → 0; [CreateOrder.cs:1076-1079](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L1076-L1079) |
| verze při objednávce | `CreateOrder` | projde jakýkoli udělený souhlas (`IsGranted`, `WithdrawnAt is null`); verze přijatého znění se neporovnává → R3 | [CreateOrder.cs:325-329](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L325-L329) |
...
- `TakeOrder` vyžaduje `AcceptedWorkContractTextId` (`WorkContractNotAccepted`) patřící dokumentu (`WorkContractTextMismatch`) — [TakeOrder.cs:64-65](../../src/Cleansia.Core.AppServices/Features/Orders/TakeOrder.cs#L64-L65), [TakeOrder.cs:249-266](../../src/Cleansia.Core.AppServices/Features/Orders/TakeOrder.cs#L249-L266)
...
- Čte zákazník objednávky, úklidník svého přijetí (`ExistsForCallerAsync`), administrátor; host ne (`CanViewOrderDetail`) → R11.2 — [GetWorkContract.cs:48-63](../../src/Cleansia.Core.AppServices/Features/Orders/GetWorkContract.cs#L48-L63), [OrderController.cs:293-294](../../src/Cleansia.Web.Customer/Controllers/OrderController.cs#L293-L294)
...
| bez platové konfigurace | položka bez sazby v měně nejde objednat (`HavePayCoverageAsync`) | [CreateOrder.cs:209-210](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L209-L210) |
...
- Trh = obsluhovaná země s konfigurací aktivní měny (`DefaultCurrencyCode`, `IsActive`); `OperatorTenantId` NULL = nikdo neobsluhuje — [GetMarkets.cs:40-58](../../src/Cleansia.Core.AppServices/Features/Markets/GetMarkets.cs#L40-L58), [CountryConfiguration.cs:118-125](../../src/Cleansia.Core.Domain/Configuration/CountryConfiguration.cs#L118-L125)
- Měna objednávky = měna země adresy (`ResolveCurrencyForCountryAsync`), jiná odmítnuta; země musí být trh s operátorem (`TenantNotFound`) → R19 — [CreateOrder.cs:926-931](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L926-L931), [CreateOrder.cs:182-190](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L182-L190)
- `LoyaltyPointsDivisor`: body = floor(částka / divisor); NULL = bez bodů; kreditní účet per měna (`GetSpendableAsync`) — [Currency.cs:22-30](../../src/Cleansia.Core.Domain/Internationalization/Currency.cs#L22-L30), [CreateOrder.cs:1116-1121](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L1116-L1121)
...
- Minimální předstih `ExpressLeadTimeHours` 2 h (`CleaningDateBelowLeadTime`); pod `StandardLeadTimeHours` 4 h express — [BookingPolicy.cs:26](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L26), [BookingPolicy.cs:20](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L20)
- Příplatek `ExpressSurchargeRate` se počítá ze zlevněného mezisoučtu (`ApplyExpressSurcharge`); odpuštěn při `waiverApplies` (volný express Plus) — [BookingPolicy.cs:269-270](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L269-L270), [BookingPolicy.cs:176-179](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L176-L179)
...
- Posádka `RequiredEmployees` = ceil(minut / `MinutesPerEmployee` 120); `SpareSeatsPerOrder` 0 → R13 — [OrderDuration.cs:27](../../src/Cleansia.Core.Domain/Orders/OrderDuration.cs#L27), [BookingPolicy.cs:135](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L135)
- Max. délka `MaxBookableOrderSpanHours` 24 h (`OrderSpanExceedsMaximum`) — [BookingPolicy.cs:145-150](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L145-L150)
- Server ověřuje jen budoucnost a předstih (`IsBelowMinimumLeadTime`); `FirstWindowHour`/`LastWindowHour` bez čtenáře — grep -rn "FirstWindowHour|LastWindowHour" src/ → 0 čtení; [CreateOrder.cs:157-162](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L157-L162)
- Server cenu přepočítá; odeslaná hrubá `TotalPrice` musí sedět (`TotalPriceNotMatch`); cena > 0, minimální částka neexistuje — [CreateOrder.cs:641-646](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L641-L646), [CreateOrder.cs:164-166](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L164-L166)
- Nabídka vrací `TotalPrice`, `FinalPriceAfterDiscount`, zdroj slevy (`tierDiscount`, `membershipDiscount`); promo až v pokladně — [QuoteOrder.cs:458-466](../../src/Cleansia.Core.AppServices/Features/Orders/QuoteOrder.cs#L458-L466), [QuoteOrder.cs:401-405](../../src/Cleansia.Core.AppServices/Features/Orders/QuoteOrder.cs#L401-L405)
- Cena položky zmrazena na objednávce (`unitBasePrice`, `lineTotal`) → R12; `PaymentType` jen `Cash = 1`, `Card = 2` — [OrderFactory.cs:181-206](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L181-L206), [PaymentType.cs:8-9](../../src/Cleansia.Core.Domain/Enums/PaymentType.cs#L8-L9)

### 3.4 Slevy a skládání

| sleva | výpočet · skládání | citace |
|---|---|---|
| Plus | `DiscountPercentage` (seed `PLUS_MONTHLY` 5 %) × hrubý základ, bez minima | [OrderFactory.cs:108-115](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L108-L115), [insert_seed_data.sql:1805](../../sql-scripts/insert_seed_data.sql#L1805) |
...
| strop | Plus + tier ≤ `MaxCombinedDiscountFraction` 12 %; nad ním kráceny poměrně | [OrderFactory.cs:54](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L54), [OrderFactory.cs:354-364](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L354-L364) |
| promo | procentní, nebo pevná min(částka, základ); nahrazuje Plus + tier, je-li větší; nikdy nesčítá | [PromoCodeService.cs:264-277](../../src/Cleansia.Core.AppServices/Services/PromoCodeService.cs#L264-L277), [OrderFactory.cs:366-376](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L366-L376) |
| pořadí a zaokrouhlení | `ResolveLoy003Discount` → promo → `ApplyExpressSurcharge`; slevy 2 místa `AwayFromZero`, kredit `Math.Floor` | [OrderFactory.cs:121-130](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L121-L130), [OrderFactory.cs:361-362](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L361-L362) |
| kredit | platidlo, ne sleva: `TotalPrice`, DPH, body nezměněny; jen karta, stejná měna, max `MaxCreditShareOfOrder` 70 % | [Order.cs:236-239](../../src/Cleansia.Core.Domain/Orders/Order.cs#L236-L239), [BookingPolicy.cs:97](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L97) |
| snapshot | `TierDiscountAmount`, `TierAtPurchase`, `PromoDiscountAmount`, `PromoCodeId`, `MembershipDiscountAmount` | [Order.cs:305-341](../../src/Cleansia.Core.Domain/Orders/Order.cs#L305-L341) |
...
|---|---|---|
| `New` 0 | `OrderFactory` při vzniku; `ReturnToBoardIfUnstaffed` při návratu | [OrderFactory.cs:321](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L321), [Order.cs:833](../../src/Cleansia.Core.Domain/Orders/Order.cs#L833) |
...
| `Confirmed` 2 | `TakeOrder`, `AdminReassignOrder`, jen z `New` | [TakeOrder.cs:360-364](../../src/Cleansia.Core.AppServices/Features/Orders/TakeOrder.cs#L360-L364), [AdminReassignOrder.cs:136-139](../../src/Cleansia.Core.AppServices/Features/Orders/AdminReassignOrder.cs#L136-L139) |
...
| `InProgress` 4 | `StartOrder`, z `Confirmed` nebo `OnTheWay` | [StartOrder.cs:185-186](../../src/Cleansia.Core.AppServices/Features/Orders/StartOrder.cs#L185-L186) |
| `Completed` 5 | `CompleteOrder`, jen z `InProgress` | [CompleteOrder.cs:282-283](../../src/Cleansia.Core.AppServices/Features/Orders/CompleteOrder.cs#L282-L283) |
...
| `CurrentStatus` | poslední řádek historie; jediný zapisovatel `AddOrderStatus`; `Confirmed → New` jen `ReturnToBoardIfUnstaffed` při prázdné posádce | [Order.cs:666-680](../../src/Cleansia.Core.Domain/Orders/Order.cs#L666-L680), [Order.cs:826-835](../../src/Cleansia.Core.Domain/Orders/Order.cs#L826-L835) |

### 4.2 Osa platby (`PaymentStatus`)

| stav | kdo zapisuje | citace |
|---|---|---|
| `Pending` 1 | `OrderFactory` při vzniku | [OrderFactory.cs:161](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L161) |
...
| `Paid` 2 | `MarkCashCollected`; `ConfirmRecurringOrder` u hotovostního výskytu | [Order.cs:693-700](../../src/Cleansia.Core.Domain/Orders/Order.cs#L693-L700), [ConfirmRecurringOrder.cs:158](../../src/Cleansia.Core.AppServices/Features/Orders/ConfirmRecurringOrder.cs#L158) |
...
|---|---|---|---|
| vytvořit, nabídka `Quote`, seznam, detail | ano | `CreateOrder` anonymně; `Lookup` trojicí | [OrderController.cs:79](../../src/Cleansia.Web.Customer/Controllers/OrderController.cs#L79), [OrderController.cs:18-53](../../src/Cleansia.Web.Customer/Controllers/OrderController.cs#L18-L53) |
...
| promo, kredit, body, Plus, preferovaný úklidník, opakování | ano | ne (`PromoNamesASignedInCustomer` → `promo.requires_account`) | [CreateOrder.cs:667-669](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L667-L669), [OrderFactory.cs:88-91](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L88-L91) |
| přeplánování, změna rozsahu, spropitné, bankovní převod, poznámka → R12 | ne (poznámku přidá jen úklidník) | ne | grep -rn "Reschedule\|Tip\b\|BankTransfer\|AddOrderNote" src/Cleansia.Web.Customer/Controllers → 0; [OrderController.cs:20-300](../../src/Cleansia.Web.Customer/Controllers/OrderController.cs#L20-L300) |
| důvod storna | `SystemCancellationReason` ano; text administrátora nikdy | totéž | [OrderMappers.cs:283-288](../../src/Cleansia.Core.AppServices/Mappers/OrderMappers.cs#L283-L288) |

### 4.4 Preferovaný úklidník a obsazení

- Jen člen Plus s dokončenou objednávkou s úklidníkem v měně objednávky — [CreateOrder.cs:292-301](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L292-L301)
- Hold `ComputePreferredHold` = `PreferredHoldFraction` 10 % předstihu, strop `PreferredHoldCeilingHours` 12 h; pod 8 h žádný — [BookingPolicy.cs:216-217](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L216-L217), [BookingPolicy.cs:255-257](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L255-L257)
...
- Během holdu (`NotHeldFrom`) viditelná jen beneficientovi, ostatním `OrderNotFound`; `MaxPreferredOfferRounds` 2, nástěnce ≥ 80 % okna — [OrderVisibility.cs:74-79](../../src/Cleansia.Core.Domain/Orders/OrderVisibility.cs#L74-L79), [BookingPolicy.cs:231](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L231)
- Odmítnutí = `PreferredHoldUntilUtc = now`; nic se neeviduje — [DeclinePreferredOffer.cs:70-79](../../src/Cleansia.Core.AppServices/Features/Orders/DeclinePreferredOffer.cs#L70-L79)
- Sedadla `AvailableSpots = MaxEmployees − přiřazení`; souběh rozhodne unikátní index (`NoAvailableSpots`) — [Order.cs:128-129](../../src/Cleansia.Core.Domain/Orders/Order.cs#L128-L129), [TakeOrder.cs:373-404](../../src/Cleansia.Core.AppServices/Features/Orders/TakeOrder.cs#L373-L404)
- Objednávka v jiné měně než mzda úklidníka se nenabízí; preferovaný úklidník se partnerovi nezobrazuje — [OrderVisibility.cs:53-59](../../src/Cleansia.Core.Domain/Orders/OrderVisibility.cs#L53-L59), [Order.cs:343-353](../../src/Cleansia.Core.Domain/Orders/Order.cs#L343-L353)
...
- Vyžaduje aktivní Plus (`RecurringTemplateMembershipRequired`); bez placeného Plus se šablona přeskočí a jde push `NotificationEventCatalog.RecurringPaused` — [CreateRecurringBooking.cs:184-194](../../src/Cleansia.Core.AppServices/Features/Bookings/CreateRecurringBooking.cs#L184-L194), [MaterializeRecurringBookingTemplate.cs:121-143](../../src/Cleansia.Core.AppServices/Features/Bookings/MaterializeRecurringBookingTemplate.cs#L121-L143)
- Materializace `HorizonDays` 7 dní dopředu; krok 7 / 14 / 30 dní — [MaterializeRecurringBookings.cs:27](../../src/Cleansia.Core.AppServices/Features/Bookings/MaterializeRecurringBookings.cs#L27), [MaterializeRecurringBookingTemplate.cs:332-334](../../src/Cleansia.Core.AppServices/Features/Bookings/MaterializeRecurringBookingTemplate.cs#L332-L334)
- Výskyt bez express (`cleaningDateUtc: null`), doplňků, promo; tier/Plus sleva platí; `RecurringTemplateId` — [MaterializeRecurringBookingTemplate.cs:244-259](../../src/Cleansia.Core.AppServices/Features/Bookings/MaterializeRecurringBookingTemplate.cs#L244-L259)
...
| hotovost | `MarkCashCollected`: schválený přiřazený úklidník, jen `InProgress`; částka se neukládá; s kreditem odmítnuto (`CashNotCollectableOnCreditOrder`) | [MarkCashCollected.cs:80](../../src/Cleansia.Core.AppServices/Features/Orders/MarkCashCollected.cs#L80), [MarkCashCollected.cs:143-151](../../src/Cleansia.Core.AppServices/Features/Orders/MarkCashCollected.cs#L143-L151) |
...
| karta: `GenerateReceipt` po webhooku `Paid`; záloha při dokončení bez `Receipt` | [HandlePaymentNotification.cs:315-321](../../src/Cleansia.Core.AppServices/Features/Payments/HandlePaymentNotification.cs#L315-L321), [CompleteOrder.cs:288-297](../../src/Cleansia.Core.AppServices/Features/Orders/CompleteOrder.cs#L288-L297) |
| vystavitel `CompanyInfo` dle země adresy; PDF: LegalName, IČO, DIČ, adresa, IBAN → R1 | [ReceiptService.cs:42-52](../../src/Cleansia.Core.AppServices/Services/ReceiptService.cs#L42-L52), [ReceiptService.cs:423-442](../../src/Cleansia.Core.AppServices/Services/ReceiptService.cs#L423-L442) |
...
| řádky `Services`, `Packages` s `LineTotal` (ceníková); extras jen slug; sleva a express bez řádku → součet ≠ Total | [ReceiptService.cs:395-404](../../src/Cleansia.Core.AppServices/Services/ReceiptService.cs#L395-L404), [ReceiptPdfData.cs:12-16](../../src/Cleansia.Infra.Services/Pdf/Models/ReceiptPdfData.cs#L12-L16) |
| kredit `CreditApplied` „Paid with credit" pod Total (Total se nemění); DPH blok jen `IsVatPayer`, neplátce `NonVatPayerNotice` „Nejsme plátci DPH" česky | [DefaultReceiptLayoutBuilder.cs:342-346](../../src/Cleansia.Infra.Services/Pdf/Layouts/DefaultReceiptLayoutBuilder.cs#L342-L346), [ReceiptService.cs:422](../../src/Cleansia.Core.AppServices/Services/ReceiptService.cs#L422) |
| pole „VAT #" se tiskne podle `VatNumber` společnosti, tedy i neplátci | [DefaultReceiptLayoutBuilder.cs:161-162](../../src/Cleansia.Infra.Services/Pdf/Layouts/DefaultReceiptLayoutBuilder.cs#L161-L162) |
| „Payment Status" = `PaymentStatus.ToString()`; hotovostní doklad nese „Pending"; layout jen anglický (`DocumentTitle` „Receipt") | [ReceiptService.cs:410](../../src/Cleansia.Core.AppServices/Services/ReceiptService.cs#L410), [DefaultReceiptLayoutBuilder.cs:47](../../src/Cleansia.Infra.Services/Pdf/Layouts/DefaultReceiptLayoutBuilder.cs#L47) |
| e-mail s PDF na `order.CustomerEmail`; stažení `DownloadReceipt` jen `Authenticated`; dobropis ani firemní faktura neexistují | grep -rin "CreditNote\|dobropis" src/*.cs → jen komentáře; [OrderController.cs:170-171](../../src/Cleansia.Web.Customer/Controllers/OrderController.cs#L170-L171) |
...
- Registrace nese `TotalPrice`, `VatAmount`, měnu, IČO, DIČ, kontakt zákazníka; položky bez extras a slevy — [ReceiptService.cs:242-258](../../src/Cleansia.Core.AppServices/Services/ReceiptService.cs#L242-L258)
...
|---|---|---|
| bez `AssignedEmployees`; `OopsWindowMinutesStandard` 15 min od rezervace | 0 | [BookingPolicy.cs:333-336](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L333-L336), [BookingPolicy.cs:79](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L79) |
| `OopsWindowMinutesFirstTime` 60 min | nikdy (`IsFirstTimeCustomer` = false) → R4 | [BookingPolicy.cs:82](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L82), [CancellationAssessor.cs:32](../../src/Cleansia.Core.AppServices/Features/Orders/CancellationAssessor.cs#L32) |
| ≥ `FreeCancellationHours` 24 h před začátkem | 0 | [BookingPolicy.cs:64](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L64) |
| 4–24 h | `PartialCancellationFeeRate` 25 % | [BookingPolicy.cs:67](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L67) |
| < `PartialCancellationHours` 4 h | `LastMinuteCancellationFeeRate` 50 % | [BookingPolicy.cs:73](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L73), [BookingPolicy.cs:70](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L70) |
| sazba 100 %; člen Plus | neexistuje (`CancellationFeeRateFor`) → R4; `FreeCancellationWindowHours` (seed 4 h) nahrazuje 24 h, pak rovnou 50 % | [BookingPolicy.cs:355-360](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L355-L360), [CancellationPolicyResolver.cs:35-44](../../src/Cleansia.Core.AppServices/Services/CancellationPolicyResolver.cs#L35-L44) |
...
| hotovost | `FeeRate` zapsán, nikdy inkasován; vratka jen u `PaymentType.Card`; čtenář jen archiv | [CustomerOrderCancellation.cs:90-95](../../src/Cleansia.Core.AppServices/Features/Orders/CustomerOrderCancellation.cs#L90-L95), [CompanyArchiveService.cs:373](../../src/Cleansia.Core.AppServices/Services/CompanyArchiveService.cs#L373) |
...
| host `CancelGuestOrder` | číslo, e-mail, kód; aktor „System" | [CancelGuestOrder.cs:15-20](../../src/Cleansia.Core.AppServices/Features/Orders/CancelGuestOrder.cs#L15-L20) |
...
| spor `ResolveDispute` | `RefundAmount` > 0 → `DisputeResolution`; selhání refundu ignorováno, spor `Resolved` | [ResolveDispute.cs:89-98](../../src/Cleansia.Core.AppServices/Features/Disputes/ResolveDispute.cs#L89-L98), [ResolveDispute.cs:77-81](../../src/Cleansia.Core.AppServices/Features/Disputes/ResolveDispute.cs#L77-L81) |
...
| Stripe poplatek | odečítá se u `AdminDiscretion` (`PlatformAbsorbsStripeFee` false); `RefundStripeFeeRate` nikdo nezapisuje → 0 | [RefundPolicy.cs:49-50](../../src/Cleansia.Core.AppServices/Features/Refunds/RefundPolicy.cs#L49-L50), [CountryConfiguration.cs:91-100](../../src/Cleansia.Core.Domain/Configuration/CountryConfiguration.cs#L91-L100) |
...
| čerpání automatické, jen karta, `CapCreditForOrder` max `MaxCreditShareOfOrder` 70 % | [CreateOrder.cs:1107-1123](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L1107-L1123), [BookingPolicy.cs:97](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L97) |
...
| nikdy se nevyplácí → R4; kladný zůstatek blokuje výmaz (`GdprDeletionBlockedByCreditBalance`) | grep -rn "Payout" src/Cleansia.Core.AppServices/Features/Credit → 0; [GdprDeletionService.cs:157-160](../../src/Cleansia.Core.AppServices/Services/GdprDeletionService.cs#L157-L160) |
...
- `DisputeStatus`: `Pending`, `UnderReview`, `WaitingForResponse`, `Resolved`, `Closed`, `Escalated`; `Resolved`/`Closed` terminální — [DisputeStatus.cs:8-13](../../src/Cleansia.Core.Domain/Enums/DisputeStatus.cs#L8-L13), [Dispute.cs:103-112](../../src/Cleansia.Core.Domain/Disputes/Dispute.cs#L103-L112)
...
- Zamítnutí bez události; s refundem jen `order.refunded`; kredit jako náhrada jen ručně `IssueCustomerCredit` → R18 — [ResolveDispute.cs:100-125](../../src/Cleansia.Core.AppServices/Features/Disputes/ResolveDispute.cs#L100-L125), [IssueCustomerCredit.cs:14-17](../../src/Cleansia.Core.AppServices/Features/Credit/Admin/IssueCustomerCredit.cs#L14-L17)

### 7.3 Chargeback

- Události `charge.dispute.created/updated/closed` (`IsChargebackEvent`); zápis `Dispute` důvod `Chargeback`, `StripeDisputeId`, stav `Escalated`; objednávka beze změny — [Constants.cs:46-48](../../src/Cleansia.Core.AppServices/Common/Constants.cs#L46-L48), [HandlePaymentNotification.cs:475-505](../../src/Cleansia.Core.AppServices/Features/Payments/HandlePaymentNotification.cs#L475-L505)
...
- Evidence do Stripe se neposílá; výplata úklidníka nedotčena — grep -rn "DisputeService|SubmitEvidence" src/*.cs → 0; [CalculateOrderPay.cs:147-155](../../src/Cleansia.Core.AppServices/Features/EmployeePayroll/CalculateOrderPay.cs#L147-L155)
- Hostovská objednávka: `Dispute.UserId` FK required, `order.UserId ?? ""` → 23503, webhook 500 → R5 — [HandlePaymentNotification.cs:484-489](../../src/Cleansia.Core.AppServices/Features/Payments/HandlePaymentNotification.cs#L484-L489), [DisputeEntityConfiguration.cs:19-21](../../src/Cleansia.Infra.Database/EntityConfigurations/DisputeEntityConfiguration.cs#L19-L21)
...
- `ContractStatus`: `Pending = 1`, `Active = 2`, `Terminated = 3`, `Approved = 4`, `Rejected = 5`; zapisují jen `Approve`, `Reject` — [ContractStatus.cs:8-12](../../src/Cleansia.Core.Domain/Enums/ContractStatus.cs#L8-L12), [Employee.cs:324-350](../../src/Cleansia.Core.Domain/Users/Employee.cs#L324-L350)
...
| `RejectEmployee` | `RejectionReason`; uvolní budoucí `Confirmed` sedadla; bez push i e-mailu → R18 | grep -rn "EmployeeApproved\|EmployeeRejected" src/ → 0; [RejectEmployee.cs:113-116](../../src/Cleansia.Core.AppServices/Features/Employees/RejectEmployee.cs#L113-L116) |
...
| po začátku | `BoardBacklogHours` 2 h; bez horního limitu předstihu | [BookingPolicy.cs:129](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L129) |
...
| digest a zástup | `order.new_available` jen `Approved`/`Active`, `JobRadiusKm` 1–500 km; sedadlo se žádostí o zástup lze vzít vytlačením | [NewJobsDigestService.cs:79-94](../../src/Cleansia.Core.AppServices/Services/NewJobsDigestService.cs#L79-L94), [TakeOrder.cs:336-348](../../src/Cleansia.Core.AppServices/Features/Orders/TakeOrder.cs#L336-L348) |

### 8.4 Provedení

| brána | podmínka | citace |
|---|---|---|
| `NotifyOnTheWay` | z `Confirmed`, přiřazený, nejdříve `StartGraceWindowMinutes` 60 min před startem | [NotifyOnTheWay.cs:48-56](../../src/Cleansia.Core.AppServices/Features/Orders/NotifyOnTheWay.cs#L48-L56), [BookingPolicy.cs:61](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L61) |
...
| délka a fotky | `actualMinutes` = teď − start, min. 1 min; `Before = 1`, `After = 2`, smazat smí jen `AssignedEmployees`, i po dokončení → R7 | [CompleteOrder.cs:267](../../src/Cleansia.Core.AppServices/Features/Orders/CompleteOrder.cs#L267), [DeleteOrderPhoto.cs:56-71](../../src/Cleansia.Core.AppServices/Features/Orders/DeleteOrderPhoto.cs#L56-L71) |
| přístup k údajům | dokud drží sedadlo (`AssignedEmployees`), bez časového limitu; prohlížející `RedactForBrowsingCleaner` → R8 | [OrderAccessService.cs:88-94](../../src/Cleansia.Core.AppServices/Authentication/OrderAccessService.cs#L88-L94), [OrderPiiRedaction.cs:41-69](../../src/Cleansia.Core.AppServices/Features/Orders/OrderPiiRedaction.cs#L41-L69) |
| po dokončení | účtenka, push, e-mail, body, doporučení, `CalculateOrderPay` per úklidník | [CompleteOrder.cs:288-349](../../src/Cleansia.Core.AppServices/Features/Orders/CompleteOrder.cs#L288-L349) |
...
- Vzorec: `BasePay + max(0, pokoje−1)·ExtraPerRoom + koupelny·ExtraPerBathroom + km·DistanceRatePerKm`; výsledek ořízne `ApplyMinMaxClamp(totalPay, MinimumPay, MaximumPay)`; první pokoj je v `BasePay` — [PayCalculatorExtensions.cs:12-18](../../src/Cleansia.Core.Domain/Extensions/PayCalculatorExtensions.cs#L12-L18)
- `CalculateOrderPay` načítá jen `.Include(o => o.SelectedServices)`; prázdné `packageIds` → balíčky do zapsané odměny nevstupují — [CalculateOrderPay.cs:117-120](../../src/Cleansia.Core.AppServices/Features/EmployeePayroll/CalculateOrderPay.cs#L117-L120), [CalculateOrderPay.cs:149-155](../../src/Cleansia.Core.AppServices/Features/EmployeePayroll/CalculateOrderPay.cs#L149-L155)
- Odhad na nástěnce `OrderPayEstimator` balíčky počítá (`matchedPackageConfigs`, `CalculateAggregatedPay`) — obě čísla se liší; doplňky, express příplatek a spropitné do odměny nevstupují → R6 — [OrderPayEstimator.cs:88-107](../../src/Cleansia.Core.AppServices/Features/Orders/OrderPayEstimator.cs#L88-L107), [PayCalculatorExtensions.cs:8-23](../../src/Cleansia.Core.Domain/Extensions/PayCalculatorExtensions.cs#L8-L23)
- Sazba `EmployeePayConfig` jen v měně objednávky, per-úklidník override před celoplatformní; šablony 0,5 / 0,75 / 1,0 × cena — [CalculateOrderPay.cs:144-153](../../src/Cleansia.Core.AppServices/Features/EmployeePayroll/CalculateOrderPay.cs#L144-L153), [BulkCreateEmployeePayConfigs.cs:25-27](../../src/Cleansia.Core.AppServices/Features/PayConfig/BulkCreateEmployeePayConfigs.cs#L25-L27)
- Řádek `OrderEmployeePay` (`BasePay`, bonus, srážka, `TotalPay`, `IsApproved`) po schválení neměnný; hotovost placena stejně; jeden `CalculateOrderPay` na přiřazeného — [OrderEmployeePay.cs:130-206](../../src/Cleansia.Core.Domain/EmployeePayroll/OrderEmployeePay.cs#L130-L206), [CompleteOrder.cs:340-349](../../src/Cleansia.Core.AppServices/Features/Orders/CompleteOrder.cs#L340-L349)
...
- Nelze dokončit před výběrem (`OrderCashNotCollected`); do tržeb vstupuje jen `Paid` (`PaidAtSomePoint`) — [CompleteOrder.cs:193-202](../../src/Cleansia.Core.AppServices/Features/Orders/CompleteOrder.cs#L193-L202), [OrderRepository.cs:267-273](../../src/Cleansia.Infra.Database/Repositories/OrderRepository.cs#L267-L273)
...
| pravidlo | citace |
|---|---|
| plány seed `PLUS_MONTHLY`, `PLUS_YEARLY`; cena jen CZK, jinde `MembershipPlanNotPricedInCurrency`; `DiscountPercentage` 5 %, `FreeCancellationWindowHours` 4 h | [insert_seed_data.sql:1805](../../sql-scripts/insert_seed_data.sql#L1805), [CreateMembershipCheckoutSession.cs:95-99](../../src/Cleansia.Core.AppServices/Features/Memberships/CreateMembershipCheckoutSession.cs#L95-L99) |
| `TrialPeriodDays` 0; jiná hodnota odmítnuta (`MembershipPlanTrialNotPermitted`); web FAQ `faq_cancel_a` o zkušební době zobrazen vždy | [CreateMembershipPlan.cs:86-88](../../src/Cleansia.Core.AppServices/Features/Memberships/Admin/CreateMembershipPlan.cs#L86-L88), [cs.json:1664](../../src/Cleansia.App/apps/cleansia.app/src/assets/i18n/cs.json#L1664) |
| entitled = `Status == Active` ∧ `CurrentPeriodEnd > now`; jedno aktivní členství; události `IsSubscriptionEvent` (`customer.subscription.*`, `invoice.payment_failed`) | [UserMembershipRepository.cs:39-46](../../src/Cleansia.Infra.Database/Repositories/UserMembershipRepository.cs#L39-L46), [Constants.cs:57-61](../../src/Cleansia.Core.AppServices/Common/Constants.cs#L57-L61) |
...
| `GoldPolisher` 3 | 2000 | 10 % | 1000 Kč | [insert_seed_data.sql:1696](../../sql-scripts/insert_seed_data.sql#L1696), [insert_seed_data.sql:1716-1730](../../sql-scripts/insert_seed_data.sql#L1716-L1730) |
| `PlatinumSparkler` 4 | 5000 | 12 % | 1000 Kč | [insert_seed_data.sql:1707](../../sql-scripts/insert_seed_data.sql#L1707), [insert_seed_data.sql:1716-1730](../../sql-scripts/insert_seed_data.sql#L1716-L1730) |
...
- Storno odebírá celý zisk, částečný refund poměrně; `RecomputeTier` stupeň snižuje, web `spent_note` „stupeň už vám nikdy neklesne" → R17 — [LoyaltyAccount.cs:114-120](../../src/Cleansia.Core.Domain/Loyalty/LoyaltyAccount.cs#L114-L120), [cs.json:1344](../../src/Cleansia.App/apps/cleansia.app/src/assets/i18n/cs.json#L1344)
...
- Perky „Prioritní zákaznická podpora", „Vyhrazený tým uklízečů" bez implementace; ledger patří firmě účtu, ne firmě objednávky → R19 — grep -rn "PrioritySupport\|DedicatedPool" src/ --include=*.cs → 0; [LoyaltyAccount.cs:75-78](../../src/Cleansia.Core.Domain/Loyalty/LoyaltyAccount.cs#L75-L78)
...
|---|---|---|---|---|
| objednávka vytvořena | ne — žádná událost | ne | `admin.order.new` (feed + e-mail, Support+), jen nabízitelná | [NewOrderAdminNotifier.cs:10-16](../../src/Cleansia.Core.AppServices/Features/Orders/NewOrderAdminNotifier.cs#L10-L16) |
| platba potvrzena | `order.payment_confirmed` (feed + push) + e-mail s účtenkou; host jen e-mail | ne | ne | [HandlePaymentNotification.cs:315-336](../../src/Cleansia.Core.AppServices/Features/Payments/HandlePaymentNotification.cs#L315-L336) |
...
| úklidník převzal | `order.cleaner_assigned` (feed + push, bez jména) + e-mail „Confirmed" | ne | ne | [TakeOrder.cs:420-432](../../src/Cleansia.Core.AppServices/Features/Orders/TakeOrder.cs#L420-L432) |
| na cestě / zahájeno / dokončeno | `order.on_the_way`; `order.in_progress` + e-mail „Started"; `order.completed` + e-mail + účtenka | ne | ne | [CompleteOrder.cs:288-321](../../src/Cleansia.Core.AppServices/Features/Orders/CompleteOrder.cs#L288-L321) |
...
| zástup vyžádán / přijat | `order.cleaner_assigned` až při přijetí | `order.seat_open`; původní `order.assignment_revoked` | ne | [TakeOrder.cs:409-421](../../src/Cleansia.Core.AppServices/Features/Orders/TakeOrder.cs#L409-L421) |
...
| reklamace rozhodnuta → R18 | `order.refunded` jen při refundaci; zamítnutí nic | ne | ne | [ResolveDispute.cs:89-126](../../src/Cleansia.Core.AppServices/Features/Disputes/ResolveDispute.cs#L89-L126) |
...
| preferovaná nabídka propadla | `order.preferred_offer_closed` (push) | ne | ne | [NotificationEventCatalog.cs:46-57](../../src/Cleansia.Core.Domain/Notifications/NotificationEventCatalog.cs#L46-L57) |
...
| kanály, preference, e-mail | 12 přepínačů `NotificationCategory`, 8 klíčů úklidníka nevypnutelných, partner bez UI; stavové e-maily anglicky (`StatusTitle_Confirmed`); selhání → `DeadLetters` bez čtenáře | | | [NotificationEventCatalog.cs:228-253](../../src/Cleansia.Core.Domain/Notifications/NotificationEventCatalog.cs#L228-L253), [EmailService.cs:375-390](../../src/Cleansia.Core.AppServices/Services/EmailService.cs#L375-L390) |
...
| blokuje | čekající žádost; živá objednávka `New/Pending/Confirmed/OnTheWay/InProgress`; kladný kredit (`HasPositiveCreditBalanceAsync`) | [GdprDeletionService.cs:220-231](../../src/Cleansia.Core.AppServices/Services/GdprDeletionService.cs#L220-L231), [GdprDeletionService.cs:157-160](../../src/Cleansia.Core.AppServices/Services/GdprDeletionService.cs#L157-L160) |
| blokuje úklidníka | otevřená faktura, přiřazená živá zakázka, nevyplacená mzda | [GdprDeletionService.cs:165-178](../../src/Cleansia.Core.AppServices/Services/GdprDeletionService.cs#L165-L178) |
| smazáno | fotka profilu, dokumenty, bloby fotek, zařízení, notifikace, adresy, šablony, výplatní údaje, Stripe id | [GdprDeletionService.cs:308-309](../../src/Cleansia.Core.AppServices/Services/GdprDeletionService.cs#L308-L309), [GdprDeletionService.cs:470-483](../../src/Cleansia.Core.AppServices/Services/GdprDeletionService.cs#L470-L483) |
| anonymizováno | `User` (`deleted_{Id}@anonymized.local`), objednávka (`UserId = null`), sdílená adresa (`Address.Anonymize`) | [User.cs:427-449](../../src/Cleansia.Core.Domain/Users/User.cs#L427-L449), [Address.cs:55-64](../../src/Cleansia.Core.Domain/Users/Address.cs#L55-L64) |
...
| zachováno | text sporu do `TextRetainedUntil`; přijetí smlouvy trvale (`Pseudonymise`); souhlasy a tokeny odvolány | [GdprDeletionService.cs:439-440](../../src/Cleansia.Core.AppServices/Services/GdprDeletionService.cs#L439-L440), [WorkContractAcceptance.cs:98-101](../../src/Cleansia.Core.Domain/Contracts/WorkContractAcceptance.cs#L98-L101) |
...
| okno | dny | co | citace |
|---|---|---|---|
| ExpiredUserCodes; StaleDevices | zapnuto; 90 | nuluje prošlé kódy; maže zařízení podle `LastActiveAt` | [RetentionDefaults.cs:16](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L16), [RetentionDefaults.cs:17](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L17) |
| OldGdprRequests | 3 roky | nuluje `ProcessedBy` (`DefaultGdprRequestsYears`); řádek se nemaže | [RetentionDefaults.cs:18](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L18) |
| OrderCustomerPii | 2 roky | anonymizuje `Completed` (`DefaultOrderPiiYears`) a adresu; zrušené nikdy | [RetentionDefaults.cs:19](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L19), [DataRetentionBackgroundService.cs:172-174](../../src/Cleansia.Core.AppServices/Features/DataRetention/DataRetentionBackgroundService.cs#L172-L174) |
| WithdrawnConsents; SupersededDocuments | 3 roky; 365 dní | maže odvolané souhlasy; maže blob i řádek dokumentu | [RetentionDefaults.cs:20](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L20), [RetentionDefaults.cs:21](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L21) |
| UserNotifications; RefreshTokenCleanup | 90 dní (strop 500); 90 dní | maže feed; maže odvolané a prošlé tokeny | [RetentionDefaults.cs:22](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L22), [RefreshTokenCleanupService.cs:22](../../src/Cleansia.Core.AppServices/Features/DataRetention/RefreshTokenCleanupService.cs#L22) |
| CustomerActionAudits; DisputeText; metadata smlouvy; fotky → R7 | 3 roky; fotky žádné | audit zákazníka, text sporu (`DefaultDisputeTextRetentionYears`), IP a id zařízení; admin audit a kontejner `order-photos` bez okna | [RetentionDefaults.cs:23-25](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L23-L25), [storage.bicep:43-51](../../deploy/bicep/modules/storage.bicep#L43-L51) |
...
| `AccessInstructions` | přiřazený úklidník je dostane bez odhalovacího kroku a bez auditu; administrátor jen přes `RevealOrderAccessInstructions`, jinak `WithholdAccessInstructions` → R8 | [GetOrderDetails.cs:155-160](../../src/Cleansia.Core.AppServices/Features/Orders/GetOrderDetails.cs#L155-L160), [RevealOrderAccessInstructions.cs:26](../../src/Cleansia.Core.AppServices/Features/Orders/RevealOrderAccessInstructions.cs#L26) |
| prohlížející úklidník; zákazník | bez jména, kontaktů, adresy, kódu; zákazník jen vlastní objednávku | [OrderPiiRedaction.cs:41-69](../../src/Cleansia.Core.AppServices/Features/Orders/OrderPiiRedaction.cs#L41-L69), [OrderAccessService.cs:78-81](../../src/Cleansia.Core.AppServices/Authentication/OrderAccessService.cs#L78-L81) |
...
| trh → provozovatel `OperatorTenantId` zapisuje jen seed; trh nabízen jen s `OperatorTenant.IsActive` | [CountryConfiguration.cs:119-125](../../src/Cleansia.Core.Domain/Configuration/CountryConfiguration.cs#L119-L125), [CountryRepository.cs:40-48](../../src/Cleansia.Infra.Database/Repositories/CountryRepository.cs#L40-L48) |
...
| každá admin mutace auditována (`AdminActionAudit` bez IP a bez retence) | [AuditEntryFactory.cs:76-92](../../src/Cleansia.Core.AppServices/Auditing/AuditEntryFactory.cs#L76-L92) |
...
| pět politik (`CanViewCompanyLifecycle` … `CanArchiveCompany`) | `AdministratorOnly` | čtyři auditní klíče `company.deactivate|reactivate|wind_down|archive` | [PolicyBuilder.cs:219-223](../../src/Cleansia.Core.AppServices/Authentication/PolicyBuilder.cs#L219-L223), [DeactivateCompany.cs:26](../../src/Cleansia.Core.AppServices/Features/CompanyLifecycle/DeactivateCompany.cs#L26) |
| deaktivace výchozího trhu | — | společnost provozující výchozí trh deaktivovat nelze (`CompanyOperatesDefaultMarket`); `cleansia-cz` jej drží, takže celý řetězec je dnes nedosažitelný | [DeactivateCompany.cs:48-49](../../src/Cleansia.Core.AppServices/Features/CompanyLifecycle/DeactivateCompany.cs#L48-L49), [insert_seed_data.sql:1014](../../sql-scripts/insert_seed_data.sql#L1014) |
| deaktivace | Administrator | `IsActive = false`: trhy mizí z „serviced"; úklidníci odmítnuti (`auth.company_deactivated`), admini ne; nic neruší | [CountryRepository.cs:36-45](../../src/Cleansia.Infra.Database/Repositories/CountryRepository.cs#L36-L45), [CompanySignInGate.cs:17-41](../../src/Cleansia.Core.AppServices/Tenancy/CompanySignInGate.cs#L17-L41) |
...
| sweep útlumu | fronta | oznámení (`SendNoticesAsync`), zrušení `New/Confirmed/OnTheWay` (refund `ServiceNotRendered`), pauza šablon, zrušení Plus, odpis kreditu (`DischargeCreditAsync`), uzavření období | [CompanyWindDownService.cs:77-85](../../src/Cleansia.Core.AppServices/Services/CompanyWindDownService.cs#L77-L85), [CompanyWindDownService.cs:304-325](../../src/Cleansia.Core.AppServices/Services/CompanyWindDownService.cs#L304-L325) |
...
| `DataRetentionCleanup` | timer `0 0 3 * * 0` | 10 retenčních úloh per společnost | maže a anonymizuje (§11.3) | [DataRetentionTimerFunction.cs:9-11](../../src/Cleansia.Functions/Functions/DataRetentionTimerFunction.cs#L9-L11) |
...
| Okna (server nečte) | 60 min, 08:00–20:00 | `WindowDurationMinutes`, `FirstWindowHour` | [BookingPolicy.cs:37](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L37), [BookingPolicy.cs:42-43](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L42-L43) |
...
| Start nejdříve před termínem | 60 min | `StartGraceWindowMinutes` | [BookingPolicy.cs:61](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L61) |
| Storno zdarma; storno 4–24 h | ≥ 24 h; 25 % | `FreeCancellationHours`, `PartialCancellationFeeRate` | [BookingPolicy.cs:64](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L64), [BookingPolicy.cs:67](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L67) |
| Storno < 4 h; hranice | 50 %; 4 h | `LastMinuteCancellationFeeRate`, `PartialCancellationHours` | [BookingPolicy.cs:70](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L70), [BookingPolicy.cs:73](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L73) |
| „Oops" okno; okno prvního zákazníka | 15 min; 60 min (neaktivní) | `OopsWindowMinutesStandard`, `OopsWindowMinutesFirstTime` | [BookingPolicy.cs:79](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L79), [BookingPolicy.cs:82](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L82) |
| Max. podíl kreditu; max. délka objednávky | 70 %; 24 h = 1 440 min | `MaxCreditShareOfOrder`, `MaxBookableOrderSpanHours` | [BookingPolicy.cs:97](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L97), [BookingPolicy.cs:145-150](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L145-L150) |
| Nástěnka po začátku; rezervní sedadla | 2 h; 0 | `BoardBacklogHours`, `SpareSeatsPerOrder` | [BookingPolicy.cs:129](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L129), [BookingPolicy.cs:135](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L135) |
| Hold preferovaného; min. předstih | 10 % předstihu, strop 12 h; 8 h | `PreferredHoldFraction`, `ComputePreferredHold` | [BookingPolicy.cs:216-217](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L216-L217), [BookingPolicy.cs:255-257](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L255-L257) |
| Kola nabídky / okno nástěnce | 2 / 80 % | `MaxPreferredOfferRounds`, `MinimumOpenBoardShare` | [BookingPolicy.cs:231](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L231), [BookingPolicy.cs:241](../../src/Cleansia.Core.AppServices/Features/Orders/BookingPolicy.cs#L241) |
...
| Strop Plus + tier; zaokrouhlení | 12 %; 2 místa `AwayFromZero`, kredit `Math.Floor` | `MaxCombinedDiscountFraction`, `OrderFactory` | [OrderFactory.cs:54](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L54), [OrderFactory.cs:361-362](../../src/Cleansia.Core.AppServices/Features/Orders/OrderFactory.cs#L361-L362) |
...
| Storno administrátorem; důvod | sazba 0, plná vratka; ≤ 500 znaků | `feeRate: 0m`, `CancelOrder` | [PlatformOrderCancellation.cs:27-35](../../src/Cleansia.Core.AppServices/Services/PlatformOrderCancellation.cs#L27-L35), [CancelOrder.cs:81](../../src/Cleansia.Core.AppServices/Features/Orders/CancelOrder.cs#L81) |
| Refundační okno; Stripe fee | 14 dní; 0 | `RefundWindowDays`, `RefundStripeFeeRate` | [RefundPolicy.cs:17](../../src/Cleansia.Core.AppServices/Features/Refunds/RefundPolicy.cs#L17), [CountryConfiguration.cs:91-100](../../src/Cleansia.Core.Domain/Configuration/CountryConfiguration.cs#L91-L100) |
...
| Materializace; krok opakování | 7 dní; 7 / 14 / 30 dní | `HorizonDays` | [MaterializeRecurringBookings.cs:27](../../src/Cleansia.Core.AppServices/Features/Bookings/MaterializeRecurringBookings.cs#L27), [MaterializeRecurringBookingTemplate.cs:332-334](../../src/Cleansia.Core.AppServices/Features/Bookings/MaterializeRecurringBookingTemplate.cs#L332-L334) |
...
| DPH CZE; pojistné krytí | 21 % / 15 %; 1 000 000 Kč | seed `StandardVatRate`, `InsuranceCoverageAmount` | [insert_seed_data.sql:1010](../../sql-scripts/insert_seed_data.sql#L1010), [insert_seed_data.sql:1014](../../sql-scripts/insert_seed_data.sql#L1014) |
...
| Sleva Plus; Plus volné storno | 5 %; 4 h | seed `PLUS_MONTHLY`: `DiscountPercentage`, `FreeCancellationWindowHours` | [insert_seed_data.sql:1805](../../sql-scripts/insert_seed_data.sql#L1805) |
| Plus: express upgrady; zkušební doba | 1 měsíčně; 0 dní | seed `PLUS_MONTHLY`: `ExpressUpgradesPerMonth`, `TrialPeriodDays` | [insert_seed_data.sql:1806](../../sql-scripts/insert_seed_data.sql#L1806), [insert_seed_data.sql:1807](../../sql-scripts/insert_seed_data.sql#L1807) |
| Cena Plus; připomínka obnovy | 199 Kč/měsíc, 2030 Kč/rok; 2–4 dny před | seed `MembershipPlanPrices`, `RenewalLeadDaysLow/High` | [insert_seed_data.sql:1835-1836](../../sql-scripts/insert_seed_data.sql#L1835-L1836), [SendMembershipLifecycleNotifications.cs:26-27](../../src/Cleansia.Core.AppServices/Features/Memberships/SendMembershipLifecycleNotifications.cs#L26-L27) |
| Tiery: prahy / slevy / floor; referral | 0, 500, 2000, 5000 / 0, 5, 10, 12 % / 1000 Kč; 150 bodů, 90 dní | seed `LoyaltyTierConfigs`, `PointsPerSide` | [insert_seed_data.sql:1716-1730](../../sql-scripts/insert_seed_data.sql#L1716-L1730), [ReferralPolicy.cs:14](../../src/Cleansia.Core.AppServices/Features/Orders/ReferralPolicy.cs#L14) |
| Veřejný promokód; seed promo | 10 %, 30 dní, 1×; WELCOME15 15 %, SPRING20 20 % od 1500, LOYAL10 10 % od 800 | `FirstOrderDiscountPercent`, seed `PromoCodes` | [RequestPromoCode.cs:33-36](../../src/Cleansia.Core.AppServices/Features/PromoCodes/RequestPromoCode.cs#L33-L36), [insert_seed_data.sql:1749-1781](../../sql-scripts/insert_seed_data.sql#L1749-L1781) |
...
| Kód objednávky hosta | 6 hex znaků | `GenerateConfirmationCode` | [OrderExtensions.cs:7](../../src/Cleansia.Core.Domain/Extensions/OrderExtensions.cs#L7) |
| Objednávka: jméno / e-mail / telefon; adresa; instrukce | 2–100 / ≤ 150 / ≤ 20; ulice 5–255, město 2–100, PSČ 3–20; ≤ 2000 znaků | `CustomerName`, `Street`, `SpecialInstructions` | [CreateOrder.cs:102-125](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L102-L125), [CreateOrder.cs:127-155](../../src/Cleansia.Core.AppServices/Features/Orders/CreateOrder.cs#L127-L155) |
...
| Retence: zařízení / dokumenty / notifikace | 90 / 365 / 90 dní, strop 500 | `RetentionDefaults` | [RetentionDefaults.cs:17](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L17), [RetentionDefaults.cs:21](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L21) |
| PII dokončených objednávek | 2 roky | `DefaultOrderPiiYears` | [RetentionDefaults.cs:19](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L19) |
| Souhlasy, audit, spor, metadata smlouvy, GDPR žádosti | 3 roky | `DefaultGdprRequestsYears`, `DefaultWorkContractMetadataRetentionYears` | [RetentionDefaults.cs:18](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L18), [RetentionDefaults.cs:25](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L25) |
| Dávka sweepu; tokeny; retry výmazů | 100 řádků; 90 dní; 30 min | `BatchSize`, `StaleProcessingAfter` | [RetentionDefaults.cs:33](../../src/Cleansia.Core.AppServices/Features/DataRetention/RetentionDefaults.cs#L33), [AdminRetryUserDeletion.cs:28](../../src/Cleansia.Core.AppServices/Features/Gdpr/AdminRetryUserDeletion.cs#L28) |
...
| Účetní povrch / stampované tabulky | 21 typů / 48 | `AccountSurface`, `TenantAuditable` | [ArchivedCompanyWriteGuard.cs:26-49](../../src/Cleansia.Infra.Database/ArchivedCompanyWriteGuard.cs#L26-L49), [TenantAuditable.cs:3-11](../../src/Cleansia.Core.Domain/Common/TenantAuditable.cs#L3-L11) |
...
| Stripe Price `price_1TSiJ83KjMqxM0RBVaiKAF6r`; promo WELCOME15 / SPRING20 / LOYAL10 „Phase B" | sandbox seed — [insert_seed_data.sql:1835-1836](../../sql-scripts/insert_seed_data.sql#L1835-L1836), [insert_seed_data.sql:1733-1734](../../sql-scripts/insert_seed_data.sql#L1733-L1734) | produkční ceník Plus; existující kampaně |
...
| DEV administrátor `admin@cleansia.local` / `Admin123!`; DEV Postgres heslo; DEV crony `0 */2 * * * *` | seed a `local.settings.json` — [insert_seed_data.sql:1890](../../sql-scripts/insert_seed_data.sql#L1890), [local.settings.json:7-11](../../src/Cleansia.Functions/local.settings.json#L7-L11) | cokoli; frekvenci úloh |
