...
--
-- EUR is seeded ACTIVE and PRICELESS, deliberately (owner ruling 2026-09-08). It carries no catalogue
-- price rows, so once Wave B's fail-closed pricing lands it is correctly unofferable -- which makes
-- "the machinery is built, only CZK is reachable" provable by an integration test rather than asserted
-- in a fixture. Adding a market then means authoring its prices, not writing code.
--
...
VALUES
  ...
  (generate_ulid()::TEXT, true, false, 'system', CURRENT_TIMESTAMP, NULL, NULL, NULL, NULL, 'EUR', '€', 'Euro', 1.0);
