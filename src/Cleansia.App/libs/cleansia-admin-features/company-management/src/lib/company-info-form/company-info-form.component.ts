...
@Component({
  selector: 'cleansia-admin-company-info-form',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  providers: [CompanyInfoFormFacade],
})
export class CompanyInfoFormComponent implements OnInit, OnDestroy {
  ...
  readonly pageTitle = computed(() =>
    this.isEditMode
      ? this.translate.instant('pages.company_info.edit_title')
      : this.translate.instant('pages.company_info.create_title')
  );
  ...
  ngOnInit(): void {
    ...
    this.companyInfoId = this.route.snapshot.paramMap.get('companyInfoId');

    if (this.isEditMode && this.companyInfoId) {
      this.facade.loadCompanyInfo(this.companyInfoId);
    }
  }
  ...
  onSave(): void {
    ...
    if (this.isEditMode && this.companyInfoId) {
      this.facade.updateCompanyInfo(this.companyInfoId, data);
    } else {
      this.facade.createCompanyInfo(data);
    }
  }
