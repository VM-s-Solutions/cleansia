...
import { ReceiptTemplateFormFacade, ReceiptTemplateFormData } from './receipt-template-form.facade';

@Component({
  selector: 'lib-receipt-template-form',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './receipt-template-form.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [ReceiptTemplateFormFacade],
})
export class ReceiptTemplateFormComponent implements OnInit, OnDestroy {
  private readonly fb = inject(FormBuilder);
  private readonly route = inject(ActivatedRoute);
  private readonly router = inject(Router);
  private readonly translate = inject(TranslateService);
  protected readonly facade = inject(ReceiptTemplateFormFacade);
  ...
  readonly pageTitle = computed(() =>
    this.isEditMode()
      ? this.translate.instant('pages.template_management.edit_receipt_template')
      : this.translate.instant('pages.template_management.create_receipt_template')
  );
  ...
  ngOnInit(): void {
    ...
    if (this.isEditMode()) {
      ...
      if (templateId) {
        ...
        this.router.navigate([CleansiaAdminRoute.TEMPLATE_MANAGEMENT, 'receipt-templates']);
      }
    }
  }
  ...
  async onSave(): Promise<void> {
    ...
    const data: ReceiptTemplateFormData = {
