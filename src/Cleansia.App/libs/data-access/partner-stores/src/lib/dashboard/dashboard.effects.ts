...
export class DashboardEffects {
  ...
  loadDashboardStats$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ employeeId }) =>
        this.partnerClient.dashboardClient.getStats(employeeId).pipe(
          map((stats) => DashboardActions.loadDashboardStatsSuccess({ stats })),
          catchError((error) =>
            of(DashboardActions.loadDashboardStatsFailure({ error }))
          )
        )
      )
    )
      ...
      mergeMap(({ filter, sort, offset = 0, limit = 5 }) =>
        this.partnerClient.orderClient
          ...
          .pipe(
            ...
            catchError((error) =>
              of(DashboardActions.loadUpcomingOrdersFailure({ error }))
            )
          )
      )
    )
    ...
    this.actions$.pipe(
      ...
      mergeMap(({ employeeId, startDate, endDate }) =>
        this.partnerClient.dashboardClient
          .getEarningsAnalytics(employeeId, startDate, endDate)
          .pipe(
            map((data) =>
              DashboardActions.loadEarningsAnalyticsSuccess({ data })
            ),
            catchError((error) =>
              of(DashboardActions.loadEarningsAnalyticsFailure({ error }))
            )
          )
      )
    )
      ...
      mergeMap(({ employeeId, startDate, endDate }) =>
        this.partnerClient.dashboardClient
          ...
          .pipe(
            ...
            catchError((error) =>
              of(DashboardActions.loadTimeAnalyticsFailure({ error }))
            )
          )
      )
    )
      ...
      mergeMap(({ employeeId, startDate, endDate }) =>
        this.partnerClient.dashboardClient
          ...
          .pipe(
            ...
            catchError((error) =>
              of(DashboardActions.loadOrderAnalyticsFailure({ error }))
            )
          )
      )
    )
  ...
  loadProductivityMetrics$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ employeeId }) =>
        this.partnerClient.dashboardClient
          .getProductivityMetrics(employeeId)
          .pipe(
            map((data) =>
              DashboardActions.loadProductivityMetricsSuccess({ data })
            ),
            catchError((error) =>
              of(DashboardActions.loadProductivityMetricsFailure({ error }))
            )
          )
      )
    )
  ...
  refreshAllAnalytics$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(
        this.store.select(DashboardSelectors.selectSelectedDateRange)
      ),
