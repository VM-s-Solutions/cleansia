...
///
/// <para><b>No new predicate exists anywhere in this feature.</b> The surface is four existing
/// conjuncts and one equality, in the order the digest already uses: the beneficiary equality, the live
/// deadline, the seat arithmetic, and <c>OrderAvailability.IsOfferableSql</c> CONJOINED — never
/// extended. <c>OrderAvailability</c> answers "is this live work someone may take", a property of the
/// order alone; "is it reserved for me right now" is the (order, cleaner) pair. Had this needed a new
/// arm in <c>OrderAvailability</c>, that would have been the signal the design was wrong.</para>
///
