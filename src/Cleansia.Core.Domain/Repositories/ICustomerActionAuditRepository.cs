...
public interface ICustomerActionAuditRepository : IRepository<CustomerActionAudit, string>
{
    ...
    /// <summary>
    /// The subject's GUEST rows: acts with no <c>UserId</c> against an order that is now theirs — a booking
    /// made before the account existed. Reached by the order because nothing else links them. Same tracked
    /// walk, same single commit. Returns the number of rows touched.
    /// </summary>
