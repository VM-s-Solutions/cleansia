...
public class ExtraRepository(CleansiaDbContext context) : BaseRepository<Extra>(context), IExtraRepository
{
    ...
    // RESTRICT). ExtraPrices is Cascade and is not a use. Recurring templates carry services and
    // packages only. OrderExtra derives BaseEntity, so no tenant filter hides another tenant's line.
