...
public class PayoutReferenceProductionCensusTests(PostgresContainerFixture fixture) : BaseIntegrationTest(fixture)
{
    ...
    private sealed class SilentEmailService : IEmailService
    {
        ...
        public Task<string> SendOrderReceiptEmailAsync(string email, Order order, byte[]? pdfBytes = null, string fileName = "receipt.pdf", string languageCode = Constants.Language.English, CancellationToken ct = default, string? guestAccessToken = null) => Sent();
        ...
        public Task<string> SendOrderStatusUpdateEmailAsync(string email, Order order, string newStatus, string languageCode = Constants.Language.English, CancellationToken ct = default, decimal? refundedAmount = null, string? guestAccessToken = null) => Sent();
