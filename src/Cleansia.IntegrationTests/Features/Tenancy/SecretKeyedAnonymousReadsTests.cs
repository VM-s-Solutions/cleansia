...
/// stamped it. TC-TEN-LOOKUP — a guest order is found by its per-order access token from a request
/// with no market, and not by a token that was issued for nothing. TC-TEN-CONFIRM — a legacy 128-bit confirmation link confirms a
...
public sealed class SecretKeyedAnonymousReadsTests(PostgresContainerFixture fixture) : BaseIntegrationTest(fixture)
{
    ...
    [Fact]
    public async Task A_Guest_Order_Is_Found_By_Its_Secret_From_A_Request_With_No_Market_And_Not_By_Another_Token()
    {
        Order? order = null;
        ...
        await TestMethod(
            ...
            act: async provider =>
            {
                ...
                var found = await mediator.Send(new LookupOrder.Query(accessToken!));
                var otherToken = await mediator.Send(new LookupOrder.Query(
                    SecurityTokens.Generate(SecurityTokens.DurableTokenByteLength)));
                return (found, otherToken);
            },
            assert: (_, results) =>
            {
                Assert.True(results.found.IsSuccess, results.found.Error?.Message);
                Assert.Equal(order!.Id, results.found.Value.Id);
                Assert.False(results.otherToken.IsSuccess);
                Assert.Equal(BusinessErrorMessage.OrderNotFound, results.otherToken.Error?.Message);
