...
/// the three request-metadata columns the paged cut withholds, fails not-found when the tenant-filtered
/// repository yields nothing, and its DTO carries exactly the persisted columns — no <c>TenantId</c>.
/// </summary>
public class GetCustomerActionAuditByIdTests
{
    ...
    [Fact]
    public async Task Returns_The_Row_With_Its_Payload_And_Request_Metadata()
    {
        _repository
            .Setup(r => r.GetByIdAsync(AuditId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(Row("{\"feeRate\":0.5,\"currencyCode\":\"CZK\"}"));
    ...
    public void Detail_Dto_Carries_The_Persisted_Columns_And_No_TenantId()
    {
        var expected = new[]
        {
            ...
            "ResourceId", "Success", "ErrorCode", "OccurredOn", "PayloadJson", "CorrelationId",
        };
