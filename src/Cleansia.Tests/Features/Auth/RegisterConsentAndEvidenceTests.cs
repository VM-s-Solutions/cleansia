...
/// <summary>
/// ADR-0062 D4 — the e-mail registration writes the terms and privacy consents server-side, versioned,
/// when the client asserted the tick, and records the act as <c>customer.account.register</c> keyed on
/// the NEW user (the session has none yet). An old client that sends nothing is recorded as "not
/// asserted" and never refused (Q-AUD-L4). The evidence names the method, language, whether a referral
/// code was present and the two versions — never the person.
/// </summary>
public sealed class RegisterConsentAndEvidenceTests
{
    ...
    private Register.Handler CreateHandler() =>
        new(_cartRepository.Object, _userRepository.Object, new Mock<IReferralService>().Object, _pending,
            _consentService.Object, _auditContext, NullLogger<Register.Handler>.Instance);

    private static Register.Command Command(bool? termsAccepted, string? referralCode = null) =>
        new(Email, Password, "John", "Doe", Language, ReferralCode: referralCode, TermsAccepted: termsAccepted);
    ...
    [Fact]
    public async Task An_Asserted_Tick_Grants_Both_Legal_Consents_Under_The_Versions_In_Force()
    {
        ...
        _consentService.Verify(
            s => s.TryGrantAsync(_createdUser!.Id, ConsentType.TermsOfService, LegalDocumentVersions.CustomerTerms, It.IsAny<CancellationToken>()),
            Times.Once);
        _consentService.Verify(
            s => s.TryGrantAsync(_createdUser!.Id, ConsentType.PrivacyPolicy, LegalDocumentVersions.CustomerPrivacy, It.IsAny<CancellationToken>()),
            Times.Once);
    ...
    public async Task A_Re_Registration_Of_An_Unconfirmed_Account_Grants_On_The_Existing_User()
    {
        ...
        _consentService.Verify(
            s => s.TryGrantAsync(existing.Id, ConsentType.TermsOfService, LegalDocumentVersions.CustomerTerms, It.IsAny<CancellationToken>()),
            Times.Once);
