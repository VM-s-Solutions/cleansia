...
public sealed class NotifyLapsedPreferredOffersSweepTests : IDisposable
{
    ...
    public async Task Each_Tenant_Groups_Rows_Are_Stamped_With_Their_Own_Tenant()
    {
        ...
        var other = ReservedOrder(
            "01HZL0000000000000000011", lapsesAt: DateTime.UtcNow.AddMinutes(-2), userId: "user-other");
        ...
        Assert.Equal(
            TestTenants.Default,
            Assert.Single(dispatches, m => m.MessageKey.Contains("user-other")).TenantId);
    }
