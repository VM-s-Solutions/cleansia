...
/// <summary>
/// The Stripe-webhook per-source-IP rate-limit window, proven on the real
...
public class WebhookRateLimitTests
{
    ...
    // Per-source-IP isolation on the webhook route: IP A exhausts the window → A's next
    ...
    // Independence: flooding "webhook" to 429 consumes NO "auth"/"interactive"
    ...
    // Reverse direction — exhausting "auth" consumes NO "webhook" allowance.
    [Fact]
    ...
    // A rejected webhook returns 429 + a positive integer Retry-After (inherits the shared
    // OnRejected / RejectionStatusCode; QueueLimit=0 → immediate reject, never queued).
    [Fact]
    ...
    // The D8 rejection metric is dimensioned by the policy name "webhook" ONLY; the partition key
