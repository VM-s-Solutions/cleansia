...
public class AdminCompanyLifecycleController(IMediator mediator) : ApiController(mediator)
{
    ...
    [HttpPost("wind-down")]
    [Permission(Policy.CanWindDownCompany)]
    [ProducesResponseType(typeof(WindDownCompany.Response), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [EnableRateLimiting("auth")]
    public async Task<IActionResult> WindDown([FromBody] WindDownCompany.Command command, CancellationToken cancellationToken)
    {
