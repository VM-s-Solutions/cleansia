...
[ApiController]
public class OrderController(IMediator mediator) : CustomerMobileApiController(mediator)
{
    [AllowAnonymous]
    ...
    {
        var result = await Mediator.Send(new LookupOrder.Query(token), cancellationToken);
