...
[ApiController]
public class NotificationController(IMediator mediator) : MobileApiController(mediator)
{
    [HttpGet("Paged")]
    ...
    {
        request.Audience = NotificationFeedAudience.Partner;
    ...
    public async Task<IActionResult> UnreadCount(CancellationToken cancellationToken)
    {
        var result = await Mediator.Send(
            new GetUnreadNotificationCount.Query(NotificationFeedAudience.Partner), cancellationToken);
    ...
    {
        var result = await Mediator.Send(
            command with { Audience = NotificationFeedAudience.Partner }, cancellationToken);
