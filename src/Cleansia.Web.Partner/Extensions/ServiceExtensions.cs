...
using Cleansia.Web.Partner.Middlewares;
using Cleansia.Web.Partner.SwaggerSchemaFilters;
using Microsoft.IdentityModel.Tokens;
using Microsoft.AspNetCore.Authentication.JwtBearer;
using Cleansia.Core.AppServices.Authentication;
using Cleansia.Infra.Common.Configuration.Interfaces;

namespace Cleansia.Web.Partner.Extensions;
    ...
    public static IServiceCollection AddJwt(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        .AddJwtBearer(options =>
        {
            options.TokenValidationParameters = new TokenValidationParameters
            {
                ...
                ValidAudience = JwtAudiences.Partner,
            ...
            options.Events = new JwtBearerEvents()
            {
                ...
                // Cookie fallback for the partner web SPA. See the Customer
                // host's matching comment for the migration rationale.
                OnMessageReceived = context =>
                {
                    if (string.IsNullOrEmpty(context.Token)
                        && context.Request.Cookies.TryGetValue("partner_token", out var fromCookie))
                    {
