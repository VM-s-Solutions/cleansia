...
/**
 * Build the AddSavedAddress command. Returns null when [latitude] or [longitude]
 * is missing — the backend's binder would coerce nulls to 0.0 and stash the
 * pin off the African coast, so the call site is expected to surface a "pick
 * a location" error instead of submitting. See [AddressRepository.upsert].
 */
internal fun UserAddress.toAddCommand(setAsDefault: Boolean): AddSavedAddressCommand? {
    val lat = latitude ?: return null
    val lng = longitude ?: return null
    return AddSavedAddressCommand(
        label = label,
        street = street,
        city = city,
        zipCode = zipCode,
...
/**
 * Build the UpdateSavedAddress command. Same coordinate guard as
 * [toAddCommand] — see comment there.
 */
internal fun UserAddress.toUpdateCommand(savedAddressId: String): UpdateSavedAddressCommand? {
    val lat = latitude ?: return null
    val lng = longitude ?: return null
    return UpdateSavedAddressCommand(
