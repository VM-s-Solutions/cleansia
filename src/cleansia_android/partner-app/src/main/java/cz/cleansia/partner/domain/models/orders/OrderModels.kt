...
/**
 * Order status enum — matches backend integer values.
...
/**
 * Request to mark an order as "on my way" — fires a heads-up push to
 * the customer. Backend appends an OnTheWay status track between
 * Confirmed and InProgress. Optional step in the workflow; the cleaner
 * can also skip directly to Start.
 */
@Serializable
data class NotifyOnTheWayRequest(
    val orderId: String,
    val employeeId: String
)
...
/**
 * Response from NotifyOnTheWay. newStatus = 6 (OrderStatus.OnTheWay).
 */
@Serializable
data class NotifyOnTheWayResponse(
    val orderId: String? = null,
    val newStatus: Int? = null
)
