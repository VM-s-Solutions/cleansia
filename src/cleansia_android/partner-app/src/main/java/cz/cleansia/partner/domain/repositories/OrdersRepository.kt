...
interface OrdersRepository {
    ...
    /**
     * Optional heads-up between Take and Start. Sends `order.on_the_way`
     * push to the customer; status flips to OnTheWay. Cleaner can still
     * call [startOrder] from either Confirmed OR OnTheWay.
     */
...
class OrdersRepositoryImpl @Inject constructor(
    ...
    override suspend fun notifyOnTheWay(orderId: String): ApiResult<Unit> {
        val employeeId = tokenManager.getUserId()
        if (employeeId.isNullOrBlank()) {
            return ApiResult.Error(cz.cleansia.partner.core.network.ApiError.Unknown("Employee ID not found"))
        }
        return safeApiCall(json) {
            apiService.notifyOnTheWay(NotifyOnTheWayRequest(orderId, employeeId))
        }.let { result ->
            when (result) {
                is ApiResult.Success -> ApiResult.Success(Unit)
                is ApiResult.Error -> result
