...
import cz.cleansia.partner.features.profile.viewmodels.IdentificationSectionViewModel
...
fun IdentificationSectionScreen(
    ...
    SectionScaffold(
        ...
        headerSlot = if (!onboarding) null else ({
            cz.cleansia.partner.features.profile.components.OnboardingChainHeader(
                currentSection = cz.cleansia.partner.features.orders.viewmodels.ProfileSection.Identification,
                state = chainState,
            )
        }),
    ) {
