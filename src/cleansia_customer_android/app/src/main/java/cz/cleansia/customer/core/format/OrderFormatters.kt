...
/**
 * Shared formatting helpers for Order DTOs. Reused across the Orders tab,
 ...
 *   Confirmed=2        → primary-solid (Sky600)
 *   InProgress=3       → light blue (Sky400)
 *   Completed=4        → green success
 *   Cancelled=5        → neutral slate text
 *   null / unknown     → outlineVariant
 *
 * Consumers typically use this as a tint + `.copy(alpha = 0.12f)` for the bg.
 */
@Composable
fun orderStatusColor(statusValue: Int?): Color = when (statusValue) {
    0, 1 -> WarningStar
    2 -> StatusConfirmedBg
    3 -> StatusInProgressBg
    4 -> StatusCompletedText
    5 -> StatusCancelledText
