...
object BookingPricing {
     ...
     * larger, then clamps to zero. Discounts cannot stack: backend's
     * `CreateOrder.Handler` runs the same best-wins rule, so the displayed
     * total here matches what the user actually pays.
     *
    ...
    fun finalTotal(
        ...
        val withSurcharge = basePrice + expressSurchargeAmount(basePrice, cleaningAt, now)
        val bestDiscount = maxOf(tierDiscount, promoDiscount)
        return (withSurcharge - bestDiscount).coerceAtLeast(0.0)
    }
}
