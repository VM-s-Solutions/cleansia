...
private struct AddressReviewPane: View {
    ...
    ///
    /// Both halves have been wrong once. The un-normalised ISO code made every country unfindable, and
    /// the fix for it wrongly folded "absent from the list" into the silent branch — so the banner went
    /// from showing on every address on earth to showing on none of them. The first is worse (a client
    /// refusing cities the server accepts is the one direction `CityNameMatch` forbids) but both are
    /// wrong, and they are distinguished here rather than in the caller.
    private func resolveServiceability() async {
        ...
        // genuinely unknown case — the two must not collapse into one silent branch, and briefly did:
        // San Francisco showed nothing at all.
