...
/// additive load-more (the Slice-A paged-list archetype). Opening marks the
/// fetched rows seen via the watermarked mark-all (FD-AC6) — rows created after
/// the fetch stay unread; the rows fetched unread keep their indicator for the
/// viewing session. Tapping an unread row fires the idempotent single
/// mark-read with an optimistic row/badge flip, and a resolvable row emits the
/// push-tap destination (FD-AC9).
@MainActor
final class NotificationsInboxViewModel: ViewModel {
    ...
    /// and navigates. A target-less row (missing args, promo) only marks read
    /// and the sheet stays open — the Android `onRowClick` behavior. The VM
    /// never navigates itself.
    let tapped = PassthroughSubject<CustomerNotificationDestination, Never>()
    ...
    func tap(id: String) async {
        ...
        if let destination = CustomerNotificationDeepLink.resolve(
            eventKey: item.eventKey,
            orderId: nonEmpty(item.args[CustomerNotificationDeepLink.orderIdField]),
            disputeId: nonEmpty(item.args[CustomerNotificationDeepLink.disputeIdField])
        ) {
