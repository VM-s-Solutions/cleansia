...
/// A customer locked out after fumbling their own password on their own handset is let through by a
/// still-valid refresh token from a previous session. This drives the customer app's REAL sign-in
/// screen through the auth spine it is wired to, and asserts the bytes that reach the wire — the
/// customer app has no login request of its own, so the only way to know its login carries the marker
/// is to send one.
@MainActor
final class CustomerTrustedDeviceLoginTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        suiteName = "CustomerTrustedDeviceLoginTests.\(UUID().uuidString)"
        ...
        spine = try AuthApiClient(
            ...
            headerAdapter: HeaderAdapter(
                deviceIdProvider: CustomerTrustedDeviceId(),
    ...
    /// sign-up screen, which reaches the customer-only `Register` path.
    func testSigningUpFromAPopulatedStoreCarriesNoMarker() async throws {
        tokenStore.save(tokens(refreshToken: storedRefresh))

        await signUp()

        let wire = try XCTUnwrap(AuthWireRecorder.body(ofPath: "/api/Auth/Register"))
        XCTAssertFalse(wire.contains("trustedDeviceToken"), wire)
    ...
    /// the production encoder produced — the server grants both consents off it in the same commit.
    func testSigningUpPutsTheTermsTickOnTheRegisterBody() async throws {
        await signUp()
    ...
    private func signIn() async {
        ...
        viewModel.onSignInEmailChange("customer@example.com")
        viewModel.onSignInPasswordChange("submitted-password")
    ...
    private func signUp() async {
        ...
        viewModel.onSignUpEmailChange("customer@example.com")
    ...
    private func makeViewModel() -> CustomerAuthViewModel {
        CustomerAuthViewModel(
            ...
            socialProvider: SilentSocialSignIn(),
...
private struct CustomerTrustedDeviceId: DeviceIdProviding {
    var deviceId: String {
        "device-customer"
    }
}
...
@MainActor
private final class SilentSocialSignIn: SocialSignInProviding {
    ...
    private static func answer(for request: URLRequest) -> (Int, Data) {
        ...
        case "/api/Auth/Login":
            ...
            {"token":"header.payload.signature","isEmailConfirmed":true,"userId":"u-1",
             "email":"customer@example.com","refreshToken":"r-1","refreshTokenExpiresAt":"2099-01-01T00:00:00Z"}
            """.utf8))
        default:
            (200, Data("true".utf8))
        }
