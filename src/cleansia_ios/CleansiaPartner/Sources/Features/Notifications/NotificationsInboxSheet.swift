...
/// The notifications inbox behind the dashboard bell — the T-0393 server-backed
/// feed. Newest-first pages of 20 with load-more; opening fires the watermarked
/// mark-all; the mascot empty state is the zero-rows state. Tapping a row that
/// resolves a destination dismisses the sheet and hands it to the shell; a
/// target-less row just marks read and stays put.
struct NotificationsInboxSheet: View {
    @Environment(\.dismiss) private var dismiss
    @StateObject private var vm: NotificationsInboxViewModel
    private let onDestination: (PartnerNotificationDestination) -> Void

    init(
        client: NotificationFeedClient,
        badge: NotificationBadgeModel,
        snackbar: SnackbarController,
        onDestination: @escaping (PartnerNotificationDestination) -> Void
    ) {
        ...
        static var previews: some View {
            VStack(spacing: Spacing.s) {
                NotificationFeedRowCard(row: NotificationFeedRow(
                    ...
                    body: "New jobs near you: 3. Tap to view.",
                ...
                NotificationFeedRowCard(row: NotificationFeedRow(
                    id: "n-2",
                    title: "New jobs available",
                    body: "New jobs near you: 1. Tap to view.",
