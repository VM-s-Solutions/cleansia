...
/// translation stops fitting. `dashboard` is the tight one: `ru` already spends 88 of its 90pt.
@MainActor
...
private struct TabLabelHarness: View {
    var body: some View {
        TabView {
            ForEach(Array(ShellTab.allCases.enumerated()), id: \.offset) { _, tab in
                Color.clear
